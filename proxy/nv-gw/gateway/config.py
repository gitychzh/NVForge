...
NVCF_PEXEC_MODELS = {
    "kimi_nv": {
        # R-bugfix (2026-07-29): f966661c (nvquery-kimi-k2_6) 已从账户删除 (NVCF status=INACTIVE,
        #   pexec/integrate 均 404 "Not found for account"). integrate 自动路由 23d4f03a 也 404.
        #   kimi 在此 NVCF 账户已完全不可用. 美国IP+本地IP 一致, 非 IP 问题. 保留待恢复.
    ...
    "dsv4p_nv": {
        # R-channel (通路模式): 3 pexec fids + integrate = 4 channels, per-attempt key+channel rotation.
        # Ch0: 74f02205 (ai-deepseek-v4-pro, 2026-08-04 pexec 404 account-wide — may recover)
        # Ch1: 12acbc62 (dynamo-mn, INACTIVE)
        # Ch2: 8915fd28 (sglang-deepseek-v4-pro, INACTIVE)
        # Ch3: integrate (function_id=null, via integrate.api.nvidia.com)
        # When k fails on channel N, try k+1 on channel (N+1)%4. All 4ch×5key exhausted → 502.
        "function_ids": [
    ...
    "dsv4f_nv": {
        # R-dsv4f-newfid (2026-08-04): old FID 6166b605 (0731-deepseek-v4-flash) now INACTIVE.
        # New FID 52e1ddb6 (ai-deepseek-v4-flash, ACTIVE) - pexec NOW works (previously 404).
        # Test: pexec SR ~25%, integrate SR ~15%, 529 overloaded dominant. pexec latency 1-3s, integrate 2-4s.
        "function_ids": [os.environ.get("NVCF_DEEPSEEK_FLASH_FUNCTION_ID",
                                        "52e1ddb6-c745-4802-93f5-ba012d04c336")],
        "strip_params": ["reasoning_effort", "stream_options", "thinking"],
        "inject": {},
    },
    "dsv4f0731_nv": {
        # R-fid0731 (2026-08-07): 换用专用 FID 281478d0 (ai-deepseek-v4-flash-0731, ACTIVE,
        #   created 2026-08-05, ownedByDifferentAccount=NVIDIA官方). 实测 pexec 5/5 US IP 全通,
        #   ttfb 0.4-1.8s 总耗时 <2.3s, 比旧 52e1ddb6 (10-28s) 快 10x.
        #   注意: 此 FID 的 pexec body model 必须用 deepseek-ai/deepseek-v4-flash-0731
        #   (普通名 404 Inference error); integrate 端点则只认普通名 → pexec_model_id 区分.
        # env 可覆盖首选: NVCF_DEEPSEEK_FLASH_0731_FUNCTION_ID (独立于 dsv4f_nv).
        "function_ids": [os.environ.get("NVCF_DEEPSEEK_FLASH_0731_FUNCTION_ID",
                                        "281478d0-f307-49f4-9e0f-080b63b16c47")],
        "pexec_model_id": "deepseek-ai/deepseek-v4-flash-0731",
        "strip_params": ["reasoning_effort", "stream_options"],
...
# ─── R cc_s3: Per-caller fixed key binding (probe NVIDIA per-key quota) ────
# NVU_CALLER_KEY_MAP: "caller_name:key_idx;caller_name:key_idx" (0-based key idx).
# 当 caller 命中此 map 时, 该 caller 的请求只用绑定的那一个 key (不走 5-key 轮转),
# nv_gw 也不跨 key 重试 (max_attempts=1). 未命中的 caller 走默认 5-key 轮转.
# caller 来源: handlers.py metrics["caller"] (X-Caller header 优先).
_caller_key_raw = os.environ.get("NVU_CALLER_KEY_MAP", "").strip()
NVU_CALLER_KEY_MAP = {}
if _caller_key_raw:
    for tok in _caller_key_raw.split(";"):
        tok = tok.strip()
        if not tok or ":" not in tok: continue
        cname, kstr = tok.split(":", 1)
        cname = cname.strip()
        if kstr.strip().lstrip("-").isdigit() and cname:
            NVU_CALLER_KEY_MAP[cname] = int(kstr)
if NVU_CALLER_KEY_MAP:
    print(f"[NV-CONFIG] NVU_CALLER_KEY_MAP loaded: {NVU_CALLER_KEY_MAP}", file=sys.stderr, flush=True)

# R-keyretry: per-caller fixed-key same-key retry (2026-07-27).
# 用户需求: NVCF key1 失败后不立刻 fallback ms_gw, 同 key 间隔重试 3 次 (2s→4s→8s),
# 每次记详细日志. 3 次全败才 all_keys_exhausted → ms_gw fallback.
# 回滚: env NVU_CALLER_RETRY=0 (禁用同 key 重试, 回退 max_attempts=1).
NVU_CALLER_RETRY = int(os.environ.get("NVU_CALLER_RETRY", "0"))
NVU_CALLER_RETRY_INTERVALS = [float(x) for x in os.environ.get(
    "NVU_CALLER_RETRY_INTERVALS", "2,4,8").split(",") if x.strip()]
if NVU_CALLER_RETRY > 0:
    print(f"[NV-CONFIG] NVU_CALLER_RETRY={NVU_CALLER_RETRY} "
          f"intervals={NVU_CALLER_RETRY_INTERVALS}", file=sys.stderr, flush=True)
...
# R704: 旧已下架 tier 移除.
NV_MODEL_TIERS = ["kimi_nv", "dsv4p_nv", "dsv4f_nv", "dsv4f0731_nv", "glm5_2_nv"]
...
# R-glm52split: glm5_2_nv 专用 per-key fid/mode 绑定 (独立 env, 非全局, 不影响 dsv4p_nv).
# KEY_MODE_BINDING 是全局陷阱 (影响所有 model), 故新增 NV_GLM52_KEY_MODE_BIND 仅 glm5_2_nv 读.
# NV_GLM52_KEY_FID_BIND: "key_idx:fid_position;..." 0-based. k1/3/5 → fid 候选列表第 0/1/2 位 (fid1/2/3).
# NV_GLM52_KEY_MODE_BIND: "key_idx:mode_name;..." k2/4→integrate_us_rr, k1/3/5→pexec_us_rr.
def _parse_key_int_map(raw):
    m = {}
    if not raw: return m
    for tok in raw.split(";"):
        tok = tok.strip()
        if not tok or ":" not in tok: continue
        kstr, vstr = tok.split(":", 1)
        for k in kstr.split(","):
            if k.strip().lstrip("-").isdigit() and vstr.strip():
                m[int(k)] = vstr.strip() if not vstr.strip().lstrip("-").isdigit() else int(vstr.strip())
                break
    return m
NV_GLM52_KEY_FID_BIND = _parse_key_int_map(os.environ.get("NV_GLM52_KEY_FID_BIND", "").strip())
NV_GLM52_KEY_MODE_BIND = _parse_key_int_map(os.environ.get("NV_GLM52_KEY_MODE_BIND", "").strip())
...
MODEL_MAP = {
    ...
    "dsv4f0731_nv": "dsv4f0731_nv",
    "deepseek-v4-flash-0731": "dsv4f0731_nv",
    "deepseek-ai/deepseek-v4-flash-0731": "dsv4f0731_nv",
    "glm5_2_nv": "glm5_2_nv",
    "glm5.2": "glm5_2_nv",
    "z-ai/glm-5.2": "glm5_2_nv",
}

# ─── R1254: NVU_ACTIVE_TIERS 模型白名单 (按容器瘦身) ─────────────────────
# 40006/40066/40666 三容器 bind-mount 共用同一份 config.py, 各容器原本全量
# 暴露 5 个模型 (health 列表 + 路由), 但实际只服务自己的专职模型.
# NVU_ACTIVE_TIERS (逗号分隔内部 tier 名) 声明容器只暴露哪些模型; 空=全部 (默认兼容).
# 过滤 NVCF_PEXEC_MODELS / NV_MODEL_TIERS / NV_MODEL_IDS / MODEL_MAP,
# 未声明模型在 health 列表消失 + 请求路由 fallback 到 DEFAULT.
# 例: 40006 设 NVU_ACTIVE_TIERS=glm5_2_nv, 40666 设 dsv4f0731_nv, 40066 设 dsv4p_nv.
NVU_ACTIVE_TIERS = [t.strip() for t in os.environ.get("NVU_ACTIVE_TIERS", "").split(",") if t.strip()]
if NVU_ACTIVE_TIERS:
    _active_set = set(NVU_ACTIVE_TIERS)
    NVCF_PEXEC_MODELS = {k: v for k, v in NVCF_PEXEC_MODELS.items() if k in _active_set}
    NV_MODEL_TIERS = [t for t in NV_MODEL_TIERS if t in _active_set]
    NV_MODEL_IDS = {k: v for k, v in NV_MODEL_IDS.items() if k in _active_set}
    MODEL_MAP = {k: v for k, v in MODEL_MAP.items() if v in _active_set}
    if DEFAULT_NV_MODEL not in _active_set:
        DEFAULT_NV_MODEL = NV_MODEL_TIERS[0] if NV_MODEL_TIERS else ""
        print(f"[NV-CONFIG] R1254: NVU_ACTIVE_TIERS={NVU_ACTIVE_TIERS} DEFAULT_NV_MODEL→{DEFAULT_NV_MODEL}",
              file=sys.stderr, flush=True)
    print(f"[NV-CONFIG] R1254: NVU_ACTIVE_TIERS={NVU_ACTIVE_TIERS} → tiers={NV_MODEL_TIERS}",
          file=sys.stderr, flush=True)
...
MODEL_INPUT_TOKEN_SAFETY = {
    ...
    "dsv4p_nv": 1048576,
    "dsv4f_nv": 1048576,  # R-dsv4f-0731: to be verified # R-bugfix: pexec 实测 1K-1048576 全 200, 支持 1M (fid 12acbc62 dynamo-mn)
    "dsv4f0731_nv": 1048576,  # R-dsv4f0731: 同 dsv4f_nv, 1M context
    "glm5_2_nv": 200000,  # R-bugfix-L: NVCF 实测 max context=202752, 改为 200K (旧 1M 导致大请求被拒)
}
...
NVU_FORCE_STREAM_EXCLUDE_MODELS = [m for m in os.environ.get('NVU_FORCE_STREAM_EXCLUDE_MODELS', 'dsv4p_nv,dsv4f_nv,glm5_2_nv').split(',') if m]
...
# R-rebuild: if set, completely disable ms_gw fallback even in execute_request path.
# NVU_DISABLE_MS_FALLBACK=1 overrides NVU_MS_FALLBACK_ENABLED for glm5_2_nv.
NVU_DISABLE_MS_FALLBACK = os.environ.get('NVU_DISABLE_MS_FALLBACK', '0') == '1'
...
# 对 cc4101-primary caller 的流式请求, 不立即转发 chunks 给 CC, buffer 在内存里
# 直到 judge_stream() 判定三者齐全 (content + finish_reason + [DONE]) 再一次性 flush.
# 每 30s 发 event:ping 占位符重置 SDK idle timer. 失败同 key 重试, 3 次全败发 error.
# 总预算 580s (150+200+200+余量), 在 R2254 watchdog 600s 之内.
# 回滚: NVU_BUFFER_CALLERS="" (空=对所有 caller 禁用 buffer 模式)
NVU_BUFFER_CALLERS = {c.strip() for c in os.environ.get("NVU_BUFFER_CALLERS", "").split(",") if c.strip()}
NVU_BUFFER_MAX_RETRIES = int(os.environ.get("NVU_BUFFER_MAX_RETRIES", "2"))  # R-buf2key: 2 key x 150s
NVU_BUFFER_TIMEOUT_STAIRS = [int(x) for x in os.environ.get("NVU_BUFFER_TIMEOUT_STAIRS", "150,150").split(",")]  # R-buf2key: 150s x 2
NVU_BUFFER_PING_INTERVAL_S = int(os.environ.get("NVU_BUFFER_PING_INTERVAL_S", "30"))
NVU_BUFFER_TOTAL_DEADLINE_S = int(os.environ.get("NVU_BUFFER_TOTAL_DEADLINE_S", "600"))  # R-buf2key: 150x2 + overhead
