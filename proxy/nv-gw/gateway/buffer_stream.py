...
class BufferStreamSession:
    ...
    def _drain_upstream(self, resp, conn, timeout_s):
        ...
        while True:
            ...
            # R-ttfb-20260816: 首个数据字节到达 → 记录 TTFB (对齐 passthrough handlers.py:2172).
            # 此前 buffer 路径(glm5_2_nv 流式主力)从不设 ttfb_ms → nv_requests 成功请求 93% 缺失.
            if self.metrics.get("ttfb_ms") is None:
                self.metrics["ttfb_ms"] = int((time.time() - self.t_start) * 1000)
    ...
    def run(self):
        ...
        for attempt in range(self.max_retries):
            ...
            if _all_cooling:
                ...
                # R2429: Record breaker + trigger fid discovery (same as AKE fail-fast)
                _nv_breaker_record_failure()
                try:
            ...
            if self._consecutive_ake_count >= _ake_fast_threshold:
                _log("NV-BUFFER-AKE-FASTM",
                     ...
                     f"(state={_nv_breaker_state()}, req={_rid})")
                # R2429: Record breaker failure HERE (not just at line 650).
                # Without this, AKE fail-fast break exits the loop before
                # _nv_breaker_record_failure() at line 650, so the breaker
                # never opens and every request wastefully retries NVCF 5 times
                # before falling to MS.
                _nv_breaker_record_failure()
                _log("NV-BREAKER-RECORD",
                     f"({self.request_model}) AKE fail-fast, recording nv failure "
                     f"(state={_nv_breaker_state()}, req={_rid})")
                # R2429: Trigger immediate fid discovery (non-blocking background)
                try:
                    from . import fid_discovery
                    fid_discovery.trigger_immediate()
                except Exception:
