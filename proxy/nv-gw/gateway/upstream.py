...
from .config import (
    ...
    NV_GLM52_KEY_FID_BIND, NV_GLM52_KEY_MODE_BIND,  # R-glm52split: per-key fid/mode (glm5_2_nv only, 非全局)
...
def _km_mark_conn(tier_model, key_idx, error_type):
    """R-rebuild Phase 2: 连接异常 → KeyManager 记录。"""
    try:
        from .key_manager import KeyManager
        KeyManager.mark_conn_error(tier_model, key_idx, error_type)
    except Exception:
        pass

# R-nvonly: transport-level error -> short penalty, no conn_count
def _km_mark_transport(tier_model, key_idx, error_type):
    """R-nvonly: RemoteDisconnected/SSL EOF -> short penalty (5-10s), do not freeze key."""
    try:
        from .key_manager import KeyManager
        KeyManager.mark_transport_error(tier_model, key_idx, error_type)
    except Exception:
        pass
...
def _try_integrate_keys(oai_body, tier_model, request_id, metrics, t_start,
    ...
    for attempt_idx in range(_n_iter):
        ...
        try:
            ...
            if resp.status >= 400:
                ...
                should_cycle = resp.status in (401, 403, 429, 408, 500, 502, 503, 504, 529, 202)
                if should_cycle:
                    cycle_reason = ("401_integrate_auth_failed" if resp.status == 401 else
                                    ...
                                    "504_integrate_gateway_timeout" if resp.status == 504 else "529_integrate_overloaded" if resp.status == 529 else "202_integrate_async_hang")
    ...
    # R-bugfix-C: all_429 需要检查至少试了 NVU_NUM_KEYS 个 key, 否则 per-key 模式
    # (key_filter 只试 1 个 key) 时单 key 429 就误触发整条 path 冷却.
    all_429 = (all(a.get("error_type") == "429_integrate_rate_limit" for a in tier_attempts)
               if tier_attempts else False)
    _is_full_key_scan = len(tier_attempts) >= NVU_NUM_KEYS
    all_429 = all_429 and _is_full_key_scan  # 只有全 key 都试过且都 429 才标 path 冷却
...
# ─── R-dsv4f-dynamic: dsv4f_nv per-key pexec↔integrate 动态切换 ─────────
# 当 pexec 失败 → 下一个 key 切 integrate; integrate 失败 → 切 pexec.
# 5 key 全失败后 fallback 逻辑不变 (execute_request all-tiers-exhausted → ms_fb).
_dsv4f_dynamic_rr_counter = 0
_dsv4f_dynamic_rr_lock = __import__("threading").Lock()

def _try_dsv4f_dynamic_keys(oai_body, tier_model, request_id, metrics, t_start,
                             is_stream, prior_cycle_attempts, upstream_timeout_override=None):
    """dsv4f_nv 动态链路: 5 key 交替 pexec↔integrate, 失败自动换路径.

    attempt 0: key_start → pexec
    attempt 1: key_next → integrate
    attempt 2: key_next → pexec
    attempt 3: key_next → integrate
    attempt 4: key_next → pexec
    (529 cycling 仍工作: should_cycle 包含 529)

    Returns: UpstreamResult (success=True + resp/conn on 200-non-empty; else failure).
    """
    global _dsv4f_dynamic_rr_counter
    ...
    # FID selection: use func_health to pick healthy function
    ...
    # Build request body (same for pexec and integrate — strip_params + inject)
    req_body = _build_pexec_body(oai_body, tier_model, nvcf_config, pexec_model=True)
    req_data = json.dumps(req_body).encode("utf-8")

    # Starting key from per-tier persistent counter
    with _dsv4f_dynamic_rr_lock:
        start_key_idx = _dsv4f_dynamic_rr_counter % NVU_NUM_KEYS
        _dsv4f_dynamic_rr_counter += 1

    # R-dsv4f-adaptive: pexec-first strategy variables
    _integ_consecutive_529 = 0
    _pexec_consecutive_529 = 0
    _INTEG_529_SKIP = int(os.environ.get("NVU_DSV4F_INTEG_529_SKIP", "2"))
    _PEXEC_CONSEC_529 = int(os.environ.get("NVU_DSV4F_PEXEC_CONSEC_529", "3"))
    _integrate_skipped = False
    _integrate_tried = False

    _log("NV-DSV4F-DYNAMIC", f"Starting dsv4f_nv adaptive mode: pexec-first, "
                             f"integrate after {_PEXEC_CONSEC_529} consecutive 529s, "
                             f"skip integrate after {_INTEG_529_SKIP} consecutive 529s, "
                             f"start_key=k{start_key_idx+1} fid={function_id[:12]}...")
    ...
    PEXEC_TIMEOUT_FASTBREAK = int(os.environ.get("NVU_PEXEC_TIMEOUT_FASTBREAK", "3"))
    EMPTY_200_FASTBREAK = int(os.environ.get("NVU_EMPTY_200_FASTBREAK", "1"))
    consecutive_empty_200 = 0

    # R814: tier-level DEGRADED short-circuit
    if is_tier_degraded(tier_model):
        _log("NV-DSV4F-DYNAMIC-DEGRADED-SKIP", f"tier={tier_model} in DEGRADED cooldown, short-circuit")
        result.all_keys_exhausted = True
        result.final_error_json = {"error": {"type": "nvcf_tier_degraded",
                                  "message": f"NVCF function for tier {tier_model} is DEGRADED (short-circuited)"}}
        result.final_resp_status = 400
        result.key_cycle_attempts = key_cycle_attempts
        result.elapsed_ms = int((time.time() - t_start) * 1000)
        return result

    _max_attempts = NVU_NUM_KEYS + 2  # 7 attempts max (5 keys + 2 extra for wrap)
    _tried_keys = set()

    # R-dsv4f-adaptive: pexec-first strategy with adaptive integrate skipping
    # Data: integrate 0% SR (all 529, 120s avg), pexec 50% SR (34s avg)
    # Strategy: default pexec; try integrate only after pexec N consecutive 529s;
    #           skip remaining integrate after M consecutive integrate 529s.
    for attempt_idx in range(_max_attempts):
        key_idx = (start_key_idx + attempt_idx) % NVU_NUM_KEYS
        # R-dsv4f-adaptive: pexec-first, try integrate only after threshold
        if _integrate_skipped:
            use_integrate = False
        elif not _integrate_tried and _pexec_consecutive_529 >= _PEXEC_CONSEC_529:
            use_integrate = True
            _integrate_tried = True
        else:
            use_integrate = False
        t_attempt_start = time.time()

        # Dedup: skip if already tried this key on same path
        _path_key = (key_idx, use_integrate)
        if _path_key in _tried_keys:
            continue
        _tried_keys.add(_path_key)

        # Tier timeout budget check
        elapsed_in_tier = time.time() - tier_budget_start
        if elapsed_in_tier >= tier_budget_s:
            _log("NV-DSV4F-DYNAMIC-BUDGET", f"tier={tier_model} budget {tier_budget_s}s exceeded after {elapsed_in_tier:.1f}s, breaking")
            break
        remaining_budget = tier_budget_s - elapsed_in_tier
        if remaining_budget < MIN_ATTEMPT_TIMEOUT:
            _log("NV-DSV4F-DYNAMIC-BUDGET", f"tier={tier_model} remaining {remaining_budget:.1f}s < {MIN_ATTEMPT_TIMEOUT}s, breaking")
            break
        per_attempt_timeout = max(MIN_ATTEMPT_TIMEOUT,
                                  min(upstream_timeout_override if upstream_timeout_override else UPSTREAM_TIMEOUT,
                                      remaining_budget - CONNECT_RESERVE_S))

        # Skip keys in cooldown or auth-failed
        integ_tier = _integrate_tier_name(tier_model)
        _cooling_tier = integ_tier if use_integrate else tier_model
        if is_key_cooling(_cooling_tier, key_idx) or is_key_auth_failed(key_idx):
            _log("NV-DSV4F-DYNAMIC", f"tier={tier_model} k{key_idx+1} {'integrate' if use_integrate else 'pexec'} cooling/auth-failed, skipping")
            continue
        ...
        # Proxy selection
        if use_integrate:
            # Integrate: use NV_INTEGRATE_PROXY_URLS (US IPs)
            if NV_INTEGRATE_PROXY_URLS:
                proxy_url = NV_INTEGRATE_PROXY_URLS[key_idx % len(NV_INTEGRATE_PROXY_URLS)]
            else:
                proxy_url = NVU_PROXY_URLS[key_idx] if key_idx < len(NVU_PROXY_URLS) else ""
            req_path = NV_INTEGRATE_PATH
            req_host = NV_INTEGRATE_HOST
            upstream_type = "nv_integrate"
            _fid_label = "integrate"
        else:
            # Pexec: use per-key NVU_PROXY_URLS
            proxy_url = NVU_PROXY_URLS[key_idx] if key_idx < len(NVU_PROXY_URLS) else ""
            req_path = f"/v2/nvcf/pexec/functions/{function_id}"
            req_host = nvcf_host
            upstream_type = "nvcf_pexec"
            _fid_label = function_id[:12] if function_id else "none"
        is_direct = (not proxy_url) or (proxy_url.strip() == "")

        # Throttle before first outbound
        if attempt_idx == 0:
            throttle_outbound()

        # Header camouflage (same for both paths)
        hdr_extra = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
            "Accept-Language": "en-US,en;q=0.9",
            "Accept": "application/json, text/plain, */*",
        ...
        _log("NV-DSV4F-DYNAMIC-ATTEMPT", f"tier={tier_model} attempt {attempt_idx+1}/{_max_attempts}: "
                                         f"k{key_idx+1} path={'integrate' if use_integrate else 'pexec'} "
                                         f"fid={_fid_label}... "
                                         f"{'DIRECT' if is_direct else 'via ' + proxy_url}")

        try:
            t_connect_start = time.time()
            conn = _make_nvcf_proxy_conn(proxy_url, nvcf_host=req_host, timeout=per_attempt_timeout)
            connect_elapsed = time.time() - t_connect_start
            post_connect_remaining = tier_budget_s - (time.time() - tier_budget_start)
            if post_connect_remaining < MIN_ATTEMPT_TIMEOUT:
                _log("NV-DSV4F-DYNAMIC-BUDGET", f"tier={tier_model} k{key_idx+1} after connect "
                                                 f"({connect_elapsed:.1f}s) remaining {post_connect_remaining:.1f}s, aborting")
                try: conn.close()
                except Exception: pass
                break
            read_timeout = min(per_attempt_timeout, post_connect_remaining)
            conn.request("POST", req_path, body=req_data, headers=headers_out)
            if conn.sock:
                conn.sock.settimeout(read_timeout)
            resp = conn.getresponse()
            ...
            if resp.status >= 400:
                ...
                # All errors are cycling (including 529, 404, 400)
                should_cycle = resp.status in (401, 403, 429, 408, 500, 502, 503, 504, 529, 202, 404, 400)
                if should_cycle:
                    cycle_reason = ("404_nv_function_not_found" if resp.status == 404 else
                                    "400_nv_bad_request" if resp.status == 400 else
                                    ...
                                    "504_nv_gateway_timeout" if resp.status == 504 else
                                    "529_nv_overloaded" if resp.status == 529 else "202_nv_async_hang")
                    key_cycle_attempts.append({
                        "tier": tier_model,
                        "nv_key_idx": key_idx,
                        "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                        "error_body": err_str[:500],
                        "error_type": cycle_reason,
                        "upstream_type": upstream_type,
                        "function_id": function_id if not use_integrate else "integrate",
                        "path": "integrate" if use_integrate else "pexec",
                    })
                    if resp.status in (401, 403):
                        mark_key_cooling(_cooling_tier, key_idx)
                        mark_key_auth_failed(key_idx)
                        _log("NV-DSV4F-DYNAMIC-AUTH-FAIL", f"tier={tier_model} k{key_idx+1} {resp.status} auth failed, cycling")
                    elif resp.status == 429:
                        mark_key_cooling(_cooling_tier, key_idx)
                        _log("NV-DSV4F-DYNAMIC-COOLDOWN", f"tier={tier_model} k{key_idx+1} marked cooling after 429")
                    # R-dsv4f-adaptive: track consecutive 529 per path
                    if resp.status == 529:
                        if use_integrate:
                            _integ_consecutive_529 += 1
                            if _integ_consecutive_529 >= _INTEG_529_SKIP:
                                _integrate_skipped = True
                                _log("NV-DSV4F-DYNAMIC-INTEG-SKIP", f"tier={tier_model} integrate consecutive "
                                     f"529={_integ_consecutive_529} >= {_INTEG_529_SKIP}, skipping integrate")
                        else:
                            _pexec_consecutive_529 += 1
                    else:
                        _pexec_consecutive_529 = 0
                        _integ_consecutive_529 = 0
                    _log("NV-DSV4F-DYNAMIC-CYCLE", f"tier={tier_model} k{key_idx+1} {'integrate' if use_integrate else 'pexec'} "
                                                     f"→ {resp.status} ({cycle_reason}), cycling")
                    consecutive_pexec_timeout = 0
                    continue

                # Non-cycling error → report
                _log("NV-DSV4F-DYNAMIC-NONCYCLE-ERR", f"tier={tier_model} k{key_idx+1} resp.status={resp.status} "
                                                         f"non-cycling, aborting. body={err_str[:200]}")
                if resp.status == 400 and "DEGRADED" in err_str.upper():
                    _cd = mark_tier_degraded(tier_model)
                    _log("NV-DSV4F-DYNAMIC-DEGRADED", f"tier={tier_model} marked DEGRADED cooldown {_cd:.0f}s")
                result.final_error_json = error_json
                result.final_resp_status = resp.status
                result.key_cycle_attempts = key_cycle_attempts
                result.elapsed_ms = int((time.time() - t_start) * 1000)
            ...
            if is_empty:
                ...
                mark_key_cooling(_cooling_tier, key_idx)
                _log("NV-DSV4F-DYNAMIC-EMPTY", f"tier={tier_model} k{key_idx+1} empty 200, cycling")
                consecutive_empty_200 += 1
                if EMPTY_200_FASTBREAK > 0 and consecutive_empty_200 >= EMPTY_200_FASTBREAK:
                    _log("NV-DSV4F-DYNAMIC-EMPTY-FASTBREAK", f"tier={tier_model} {consecutive_empty_200} consecutive empty_200, fast-break")
                    break
                consecutive_pexec_timeout = 0
                try: conn.close()
                except Exception: pass
            ...
            result.tier_model = tier_model
            result.nv_key_idx = key_idx
            result.upstream_type = upstream_type
            result.function_id = function_id if not use_integrate else "integrate"
            if use_integrate:
                _eg_port = proxy_url.strip().rsplit(":", 1)[-1] if proxy_url and ":" in proxy_url else "direct"
                result.egress_route = f"integrate-mihomo-{_eg_port}" if not is_direct else "integrate-direct"
                _, result.egress_ip = egress_info_for_integrate_key(key_idx)
            ...
            result.nv_model_label = f"{upstream_type}_{nv_model_id}_k{key_idx+1}"
            result.key_cycle_attempts = key_cycle_attempts
            result.fallback_tiers_used = [tier_model]
            reset_key429_count(_cooling_tier, key_idx)
            metrics["upstream_type"] = upstream_type
            metrics["tier_model"] = tier_model
            metrics["nv_key_idx"] = key_idx
            metrics["litellm_model"] = result.nv_model_label
            if key_cycle_attempts:
                metrics["key_cycle_429s_before_success"] = len(key_cycle_attempts)
                _log("NV-DSV4F-DYNAMIC-SUCCESS", f"tier={tier_model} k{key_idx+1} {'integrate' if use_integrate else 'pexec'} "
                                                   f"succeeded after {len(key_cycle_attempts)} cycle attempts "
                                                   f"(pexec_529={_pexec_consecutive_529}, integ_529={_integ_consecutive_529}, "
                                                   f"integ_skipped={_integrate_skipped})")
            else:
                _log("NV-DSV4F-DYNAMIC-SUCCESS", f"tier={tier_model} k{key_idx+1} {'integrate' if use_integrate else 'pexec'} "
                                                   f"succeeded on first attempt")
            return result

        except socket.timeout as e:
            attempt_elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-DSV4F-DYNAMIC-TIMEOUT", f"tier={tier_model} k{key_idx+1} {'integrate' if use_integrate else 'pexec'} "
                                              f"timeout: attempt={attempt_elapsed_ms}ms")
            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error_type": "NVCFDynamicTimeout",
                "elapsed_ms": attempt_elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if not use_integrate else "integrate",
                "path": "integrate" if use_integrate else "pexec",
            ...
            consecutive_pexec_timeout += 1
            if consecutive_pexec_timeout >= PEXEC_TIMEOUT_FASTBREAK:
                _log("NV-DSV4F-DYNAMIC-FASTBREAK", f"tier={tier_model} {consecutive_pexec_timeout} consecutive "
                                                   f"timeouts → fast-break")
                break
            continue

        except (ConnectionRefusedError, http.client.RemoteDisconnected) as e:
            attempt_elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-DSV4F-DYNAMIC-CONN", f"tier={tier_model} k{key_idx+1} connection error: {e}")
            consecutive_conn_err += 1
            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error_type": f"NVCFDynamic{type(e).__name__}",
                "elapsed_ms": attempt_elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if not use_integrate else "integrate",
                "path": "integrate" if use_integrate else "pexec",
            ...
            if consecutive_conn_err >= CONN_ERR_FAST_BREAK:
                _log("NV-DSV4F-DYNAMIC-CONN-BREAK", f"tier={tier_model} {consecutive_conn_err} consecutive "
            ...
            error_class = type(e).__name__
            elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-DSV4F-DYNAMIC-ERR", f"tier={tier_model} k{key_idx+1} {error_class}: {e}")

            is_ssl_err = (error_class == "SSLEOFError" or error_class == "SSLError" or
                         error_class == "SSLZeroReturnError")
            if is_ssl_err:
                _log("NV-DSV4F-DYNAMIC-SSL", f"tier={tier_model} k{key_idx+1} SSL error ({elapsed_ms}ms), cycling")
            else:
                _log("NV-DSV4F-DYNAMIC-ERR", f"tier={tier_model} k{key_idx+1} {error_class} ({elapsed_ms}ms), cycling")

            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error_type": f"NVCFDynamic{error_class}",
                "elapsed_ms": elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if not use_integrate else "integrate",
                "path": "integrate" if use_integrate else "pexec",
            ...
            if is_ssl_err:
                _ssl_penalty = float(os.environ.get("NVU_SSLEOF_RETRY_DELAY_S", "1.0"))
                if _ssl_penalty > 0:
                    time.sleep(_ssl_penalty)
            continue

    # All keys exhausted
    has_429 = any("429" in a.get("error_type", "") for a in key_cycle_attempts if a.get("tier") == tier_model)
    has_empty = any(a.get("error_type") == "empty_200" for a in key_cycle_attempts if a.get("tier") == tier_model)
    result.all_keys_exhausted = True
    result.all_429 = has_429 and not has_empty
    result.empty_200 = has_empty
    result.final_resp_status = 429 if has_429 and not has_empty else 502
    result.key_cycle_attempts = key_cycle_attempts
    result.elapsed_ms = int((time.time() - t_start) * 1000)
    _log("NV-DSV4F-DYNAMIC-FAIL", f"tier={tier_model} all keys exhausted "
                                   f"(attempts={len([a for a in key_cycle_attempts if a.get('tier') == tier_model])}, "
                                   f"elapsed={result.elapsed_ms}ms)")
    return result

# ─── R-channel: dsv4p_nv 通路模式 (4 channel × 5 key rotation) ─────────
# 4 channels: Ch0=pexec(74f02205), Ch1=pexec(12acbc62), Ch2=pexec(8915fd28), Ch3=integrate(null)
# When key K fails on channel N, try key K+1 on channel (N+1)%4.
# Total attempts = min(4*5, 20) with dedup. All exhausted → all_keys_exhausted.
# unify pexec+integrate into one per-attempt loop, rotating both key and channel.
_dsv4p_channel_rr_counter = 0
_dsv4p_channel_rr_lock = __import__("threading").Lock()

def _try_dsv4p_channel_keys(oai_body, tier_model, request_id, metrics, t_start,
                             is_stream, prior_cycle_attempts, upstream_timeout_override=None):
    """dsv4p_nv 通路模式: 4 channels (3 pexec fids + 1 integrate) × 5 keys, per-attempt rotation.

    R-channel (2026-08-04): NVCF pexec 404 account-wide for all 3 dsv4p fids.
    Old code: 404 = non-cycling → abort tier after 1 key. No channel switching.
    New: 404 on channel N → advance to channel (N+1)%4 + next key. Tries all 4 channels
    (pexec fids + integrate) before giving up. Integrate treated as channel 3 (fid=null).

    Returns: UpstreamResult (success=True + resp/conn on 200-non-empty; else failure).
    """
    global _dsv4p_channel_rr_counter
    ...
    # Channel definitions: (name, function_id, channel_type)
    # Ch3 = integrate (fid=None, type=integrate)
    _channels = []
    for fid in nvcf_config.get("function_ids", []):
        _channels.append(("pexec", fid, "nvcf_pexec"))
    _channels.append(("integrate", None, "nv_integrate"))
    _num_channels = len(_channels)
    ...
    # Starting key from per-tier persistent counter
    with _dsv4p_channel_rr_lock:
        start_key_idx = _dsv4p_channel_rr_counter % NVU_NUM_KEYS
        _dsv4p_channel_rr_counter += 1
    # Starting channel = 0 (pexec first fid). func_health may skip unhealthy fids.
    start_channel = 0

    _log("NV-CHANNEL", f"Starting dsv4p_nv channel mode: {_num_channels} channels × {NVU_NUM_KEYS} keys, "
                        f"start_key=k{start_key_idx+1} start_ch={start_channel} "
                        f"channels={[c[0]+':'+(c[1][:8] if c[1] else 'integrate') for c in _channels]}")
    ...
    # R814: tier-level DEGRADED short-circuit
    if is_tier_degraded(tier_model):
        _log("NV-CHANNEL-DEGRADED-SKIP", f"tier={tier_model} in DEGRADED cooldown, short-circuit")
        result.all_keys_exhausted = True
        result.final_error_json = {"error": {"type": "nvcf_tier_degraded",
                                  "message": f"NVCF function for tier {tier_model} is DEGRADED (short-circuited)"}}
        result.final_resp_status = 400
    ...
    _max_attempts = _num_channels * NVU_NUM_KEYS  # 4×5=20 max
    _tried_pairs = set()  # dedup (key_idx, channel_idx)

    for attempt_idx in range(_max_attempts):
        key_idx = (start_key_idx + attempt_idx) % NVU_NUM_KEYS
        channel_idx = (start_channel + attempt_idx) % _num_channels
        t_attempt_start = time.time()

        # Dedup: skip if already tried this (key, channel) pair
        pair = (key_idx, channel_idx)
        ...
        elapsed_in_tier = time.time() - tier_budget_start
        if elapsed_in_tier >= tier_budget_s:
            _log("NV-CHANNEL-BUDGET", f"tier={tier_model} budget {tier_budget_s}s exceeded after {elapsed_in_tier:.1f}s, breaking")
            break
        remaining_budget = tier_budget_s - elapsed_in_tier
        if remaining_budget < MIN_ATTEMPT_TIMEOUT:
            _log("NV-CHANNEL-BUDGET", f"tier={tier_model} remaining {remaining_budget:.1f}s < {MIN_ATTEMPT_TIMEOUT}s, breaking")
            break
        per_attempt_timeout = max(MIN_ATTEMPT_TIMEOUT,
                                  min(upstream_timeout_override if upstream_timeout_override else UPSTREAM_TIMEOUT,
                                      remaining_budget - CONNECT_RESERVE_S))
        ...
        # Skip keys in cooldown or auth-failed
        integ_tier = _integrate_tier_name(tier_model)
        _cooling_tier = integ_tier if channel_idx == _num_channels - 1 else tier_model
        if is_key_cooling(_cooling_tier, key_idx) or is_key_auth_failed(key_idx):
            _log("NV-CHANNEL", f"tier={tier_model} k{key_idx+1} ch{channel_idx} cooling/auth-failed, skipping")
            continue
        ...
        nv_key = NVU_KEYS[key_idx]
        channel_name, function_id, upstream_type = _channels[channel_idx]
        is_integrate = (channel_name == "integrate")

        # Proxy selection
        if is_integrate:
            # Integrate: use NV_INTEGRATE_PROXY_URLS (US IPs, geographic requirement)
            if NV_INTEGRATE_PROXY_URLS:
                proxy_url = NV_INTEGRATE_PROXY_URLS[key_idx % len(NV_INTEGRATE_PROXY_URLS)]
            else:
                proxy_url = NVU_PROXY_URLS[key_idx] if key_idx < len(NVU_PROXY_URLS) else ""
        ...
        _log("NV-CHANNEL-ATTEMPT", f"tier={tier_model} attempt {attempt_idx+1}/{_max_attempts}: "
                                   f"k{key_idx+1} ch{channel_idx}({channel_name}) "
                                   f"fid={function_id[:12] if function_id else 'integrate'}... "
                                   f"{'DIRECT' if is_direct else 'via ' + proxy_url}")
        ...
        try:
            ...
            post_connect_remaining = tier_budget_s - (time.time() - tier_budget_start)
            if post_connect_remaining < MIN_ATTEMPT_TIMEOUT:
                _log("NV-CHANNEL-BUDGET", f"tier={tier_model} k{key_idx+1} ch{channel_idx} after connect "
                                         f"({connect_elapsed:.1f}s) remaining {post_connect_remaining:.1f}s, aborting")
                try: conn.close()
                except Exception: pass
                break
            ...
            if resp.status >= 400:
                ...
                # R-channel: ALL errors are cycling (including 404 which was non-cycling before)
                # 404 = function not found in account → try next channel (different fid)
                # 401/403 = auth fail → mark key, cycle
                # 429 = rate limit → mark key cooling, cycle
                # 500/502/503/504 = server error → cycle
                # 202 = async hang → cycle
                should_cycle = resp.status in (401, 403, 429, 408, 500, 502, 503, 504, 202, 404, 400)
                if should_cycle:
                    cycle_reason = ("404_nv_function_not_found" if resp.status == 404 else
                                    "400_nv_bad_request" if resp.status == 400 else
                                    "401_nv_auth_failed" if resp.status == 401 else
                                    ...
                                    "502_nv_error" if resp.status == 502 else
                                    "503_nv_error" if resp.status == 503 else
                                    "504_nv_gateway_timeout" if resp.status == 504 else "529_nv_overloaded" if resp.status == 529 else "202_nv_async_hang")
                    key_cycle_attempts.append({
                        "tier": tier_model,
                        "nv_key_idx": key_idx,
                        "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                        ...
                        "error_type": cycle_reason,
                        "upstream_type": upstream_type,
                        "function_id": function_id if function_id else "integrate",
                        "channel_idx": channel_idx,
                    })
                    if resp.status in (401, 403):
                        mark_key_cooling(_cooling_tier, key_idx)
                        mark_key_auth_failed(key_idx)
                        _log("NV-CHANNEL-AUTH-FAIL", f"tier={tier_model} k{key_idx+1} ch{channel_idx} {resp.status} auth failed, cycling")
                    elif resp.status == 429:
                        mark_key_cooling(_cooling_tier, key_idx)
                        _log("NV-CHANNEL-COOLDOWN", f"tier={tier_model} k{key_idx+1} ch{channel_idx} marked cooling after 429")
                    elif resp.status == 404:
                        # 404 = function not in account → mark this fid unhealthy in func_health
                        if function_id:
                            func_health.record_result(function_id, False)
                        _log("NV-CHANNEL-404", f"tier={tier_model} k{key_idx+1} ch{channel_idx} fid={function_id[:12] if function_id else 'N/A'} 404, cycling to next channel")
                    _log("NV-CHANNEL-CYCLE", f"tier={tier_model} k{key_idx+1} ch{channel_idx} → {resp.status} ({cycle_reason}), cycling")
                    consecutive_pexec_timeout = 0
                    continue

                # Non-cycling error → report
                _log("NV-CHANNEL-NONCYCLE-ERR", f"tier={tier_model} k{key_idx+1} ch{channel_idx} resp.status={resp.status} non-cycling, aborting. body={err_str[:200]}")
                if resp.status == 400 and "DEGRADED" in err_str.upper():
                    _cd = mark_tier_degraded(tier_model)
                    _log("NV-CHANNEL-DEGRADED", f"tier={tier_model} marked DEGRADED cooldown {_cd:.0f}s")
                result.final_error_json = error_json
                result.final_resp_status = resp.status
                result.key_cycle_attempts = key_cycle_attempts
                result.elapsed_ms = int((time.time() - t_start) * 1000)
            ...
            if is_empty:
                ...
                mark_key_cooling(_cooling_tier, key_idx)
                _log("NV-CHANNEL-EMPTY", f"tier={tier_model} k{key_idx+1} ch{channel_idx} empty 200, cycling")
                consecutive_empty_200 += 1
                if EMPTY_200_FASTBREAK > 0 and consecutive_empty_200 >= EMPTY_200_FASTBREAK:
                    _log("NV-CHANNEL-EMPTY-FASTBREAK", f"tier={tier_model} {consecutive_empty_200} consecutive empty_200, fast-break")
                    break
                consecutive_pexec_timeout = 0
                try: conn.close()
                except Exception: pass
            ...
            result.nv_key_idx = key_idx
            result.upstream_type = upstream_type
            result.function_id = function_id if function_id else "integrate"
            result.egress_route, result.egress_ip = (egress_info_for_key(key_idx) if not is_integrate
                                                      else egress_info_for_integrate_key(key_idx))
            result.nv_model_label = f"{upstream_type}_{nv_model_id}_k{key_idx+1}"
            result.key_cycle_attempts = key_cycle_attempts
            result.fallback_tiers_used = [tier_model]
            reset_key429_count(_cooling_tier, key_idx)
            ...
            metrics["nv_key_idx"] = key_idx
            metrics["litellm_model"] = result.nv_model_label
            metrics["channel_idx"] = channel_idx
            if key_cycle_attempts:
                metrics["key_cycle_429s_before_success"] = len(key_cycle_attempts)
                _log("NV-CHANNEL-SUCCESS", f"tier={tier_model} k{key_idx+1} ch{channel_idx}({channel_name}) succeeded after "
                                            f"{len(key_cycle_attempts)} cycle attempts")
            else:
                _log("NV-CHANNEL-SUCCESS", f"tier={tier_model} k{key_idx+1} ch{channel_idx}({channel_name}) succeeded on first attempt")
            return result

        except socket.timeout as e:
            attempt_elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-CHANNEL-TIMEOUT", f"tier={tier_model} k{key_idx+1} ch{channel_idx} timeout: attempt={attempt_elapsed_ms}ms")
            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error_type": f"{'Integrate' if is_integrate else 'NVCFPexec'}Timeout",
                "elapsed_ms": attempt_elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if function_id else "integrate",
                "channel_idx": channel_idx,
            ...
            consecutive_pexec_timeout += 1
            if consecutive_pexec_timeout >= PEXEC_TIMEOUT_FASTBREAK:
                _log("NV-CHANNEL-FASTBREAK", f"tier={tier_model} {consecutive_pexec_timeout} consecutive timeouts, fast-break")
                break
            continue

        except (ConnectionRefusedError, http.client.RemoteDisconnected) as e:
            attempt_elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-CHANNEL-CONN", f"tier={tier_model} k{key_idx+1} ch{channel_idx} connection error: {e}")
            consecutive_conn_err += 1
            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error_type": f"{'Integrate' if is_integrate else 'NVCFPexec'}{type(e).__name__}",
                "elapsed_ms": attempt_elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if function_id else "integrate",
                "channel_idx": channel_idx,
            ...
            if consecutive_conn_err >= CONN_ERR_FAST_BREAK:
                _log("NV-CHANNEL-CONN-BREAK", f"tier={tier_model} {consecutive_conn_err} consecutive connection errors, fast-break")
                break
            continue
            ...
            error_class = type(e).__name__
            elapsed_ms = int((time.time() - t_attempt_start) * 1000)
            _log("NV-CHANNEL-ERR", f"tier={tier_model} k{key_idx+1} ch{channel_idx} {error_class}: {e}")
            is_ssl_err = (error_class == "SSLEOFError" or error_class == "SSLError" or
                          error_class == "SSLZeroReturnError")
            if is_ssl_err:
                _log("NV-CHANNEL-SSL-CYCLE", f"tier={tier_model} k{key_idx+1} ch{channel_idx} SSL error ({elapsed_ms}ms), cycling")
                continue
            key_cycle_attempts.append({
                "tier": tier_model,
                "nv_key_idx": key_idx,
                "litellm_model": f"{upstream_type}_{nv_model_id}_k{key_idx+1}",
                "error": str(e)[:200],
                "error_type": f"{'Integrate' if is_integrate else 'NVCFPexec'}{error_class}",
                "elapsed_ms": elapsed_ms,
                "upstream_type": upstream_type,
                "function_id": function_id if function_id else "integrate",
                "channel_idx": channel_idx,
    ...
    # ─── All channels × all keys exhausted ───
    tier_attempts = [a for a in key_cycle_attempts if a.get("tier") == tier_model]
    all_429 = all(a.get("error_type") in ("429_nv_rate_limit", "429_integrate_rate_limit") for a in tier_attempts) if tier_attempts else False
    all_empty = all(a.get("error_type") == "empty_200" for a in tier_attempts) if tier_attempts else False
    ...
    fail_summary = (f"404={sum(1 for a in tier_attempts if '404' in a.get('error_type',''))}, "
                    f"429={sum(1 for a in tier_attempts if '429' in a.get('error_type',''))}, "
                    f"empty200={sum(1 for a in tier_attempts if a.get('error_type')=='empty_200')}, "
                    f"timeout={sum(1 for a in tier_attempts if 'Timeout' in a.get('error_type',''))}, "
                    f"other={sum(1 for a in tier_attempts if a.get('error_type') not in ('429_nv_rate_limit','429_integrate_rate_limit','empty_200','404_nv_function_not_found') and 'Timeout' not in a.get('error_type',''))}")
    _log("NV-CHANNEL-FAIL", f"tier={tier_model} all {_num_channels} channels × {NVU_NUM_KEYS} keys failed: {fail_summary}, "
                             f"elapsed={result.elapsed_ms}ms")

    if all_429:
        for k in range(NVU_NUM_KEYS):
            mark_key_cooling(tier_model, k, duration_s=int(TIER_COOLDOWN_S))
        _log("NV-CHANNEL-GLOBAL-COOLDOWN", f"tier={tier_model} all keys 429. Marking all cooling {TIER_COOLDOWN_S:.0f}s")

    _log_error_detail({
        "request_id": request_id,
        "timestamp": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%S+00:00"),
        "error_subcategory": f"channel_{tier_model}_all_keys_failed",
        "tier_model": tier_model,
        "tier_attempts": tier_attempts,
        "all_429": all_429,
...
def _try_tier_keys(oai_body, tier_model, request_id, metrics, t_start,
    ...
    pexec_body = _build_pexec_body(oai_body, tier_model, nvcf_config, pexec_model=True)
...
# R-bugfix (2026-07-29): fid 独立轮询计数器, 与 key 轮转解耦.
# key 由 buffer_stream _buf_rr_counter 轮转, fid 由本计数器独立轮转.
# 两者互不影响, 让日志数据覆盖所有 key×fid 组合, 供后续分析决定优先顺序.
_fid_rr_counter = 0
_fid_rr_lock = __import__("threading").Lock()

def _next_fid_idx(num_fids):
    """返回下一个 fid index (0~num_fids-1), 独立于 key 轮转."""
    global _fid_rr_counter
    with _fid_rr_lock:
        idx = _fid_rr_counter % num_fids
        _fid_rr_counter += 1
        return idx
...
def _glm52_single_attempt(oai_body, tier_model, request_id, metrics, t_start,
    ...
    req_body = _build_pexec_body(oai_body, tier_model, nvcf_config,
                                 pexec_model=(channel == "pexec"))
    req_data = json.dumps(req_body).encode("utf-8")

    is_direct = (not proxy_url) or (proxy_url.strip() == "")
    if channel == "pexec":
        # R-glm52split: per-key fid 绑定 (仅 glm5_2_nv, 不影响 dsv4p_nv).
        # NV_GLM52_KEY_FID_BIND={key_idx: fid_position} 0-based. k1/3/5 → fid1/2/3.
        # 未绑定的 key (含 dsv4p_nv 全部) → 原 _fid_rr 轮转逻辑.
        _candidates = nvcf_config.get("function_ids") or [nvcf_config.get("function_id")]
        if tier_model == "glm5_2_nv" and key_idx in NV_GLM52_KEY_FID_BIND:
            _fid_pos = NV_GLM52_KEY_FID_BIND[key_idx]
            if 0 <= _fid_pos < len(_candidates):
                function_id = _candidates[_fid_pos]
            else:
                function_id = _candidates[0]
            # R1253: KEY_FID_BIND 命中也检查健康度; 绑定 fid surge/429 时 (func_health 判定不健康)
            # 也自动切到候选里健康的另一个 fid, 不再让绑定 fid 全链路陪葬.
            if len(_candidates) > 1 and not func_health.is_healthy(function_id):
                function_id = func_health.select_healthy_function(tier_model, _candidates)
        else:
            # R-bugfix (2026-07-29): fid 独立轮询 (与 key 轮转解耦).
            # key 由 buffer_stream _buf_rr_counter 轮转; fid 由 _next_fid_idx 独立轮转.
            # 两者互不影响, 5key×3fid=15 组合均匀覆盖, 日志数据供后续分析决定优先顺序.
            # 若轮到的 fid 在 func_health 中明确不健康 (surge/DEGRADING), 跳到下一个健康的.
            _fid_idx = _next_fid_idx(len(_candidates))
            function_id = _candidates[_fid_idx]
            if len(_candidates) > 1 and not func_health.is_healthy(function_id):
                function_id = func_health.select_healthy_function(tier_model, _candidates)
        result.function_id = function_id
        nvcf_host = NVCF_BASE_URL
    ...
    _log("NV-GLM52-ATTEMPT", f"tier={tier_model} mode={mode_name} k{key_idx+1} channel={channel} "
                             f"fid={function_id[:8]} {'DIRECT' if is_direct else 'via ' + proxy_url} timeout={per_attempt_timeout:.0f}s")
    ...
    try:
        ...
        _km_mark_conn(tier_model, key_idx, "timeout")
        # R-bugfix-A: 关闭 conn 防 socket leak. 旧: 异常分支不关 conn,
        # NVCF 间歇性 timeout 高发期每 attempt 泄漏一个 socket.
        try: conn.close()
        except Exception: pass
        result.key_cycle_attempts = all_attempts
        result.elapsed_ms = int((time.time() - t_start) * 1000)
        ...
        _km_mark_transport(tier_model, key_idx, type(e).__name__)
        # R-bugfix-A: 关闭 conn 防 socket leak
        try: conn.close()
        except Exception: pass
        result.key_cycle_attempts = all_attempts
        result.elapsed_ms = int((time.time() - t_start) * 1000)
        ...
        if is_ssl: _km_mark_transport(tier_model, key_idx, error_class)
        # R-bugfix-A: 关闭 conn 防 socket leak
        try: conn.close()
        except Exception: pass
        result.key_cycle_attempts = all_attempts
...
def _try_glm52_mode_chain(oai_body, tier_model, request_id, metrics, t_start,
    ...
    # R-bugfix-B: buffer_stream 通过 metrics["nv_start_key_override"] 传入指定的 start_key,
    # 让 buffer 的 _KEY_ROTATION 表真正生效 (旧: caller=cc4101-primary 不在 map 中,
    # 走 RR, _KEY_ROTATION 的 key_idx 被忽略)
    _override_start_key = metrics.get("nv_start_key_override") if isinstance(metrics, dict) else None
    if _override_start_key is not None and 0 <= _override_start_key < NVU_NUM_KEYS:
        start_key = _override_start_key
        _chain_max_attempts = 1  # 只试 buffer 指定的这一个 key
        _log("NV-GLM52-CHAIN", f"tier={tier_model} BUFFER_OVERRIDE start_key=k{start_key+1} "
                               f"(buffer _KEY_ROTATION, NVCF 1 attempt)")
    elif _bound_key is not None and 0 <= _bound_key < NVU_NUM_KEYS:
    ...
    # R-bugfix-B: override 路径也固定 key (只试 1 个, 不做 RR 轮转)
    _is_key_fixed = _is_caller_bound or (_override_start_key is not None and 0 <= _override_start_key < NVU_NUM_KEYS)
    for attempt in range(_chain_max_attempts):
        ...
        # R-glm52split: NV_GLM52_KEY_MODE_BIND 优先 (仅 glm5_2_nv 读, 非全局, 避免 KEY_MODE_BINDING 拐 dsv4p_nv).
        _bound_mode_name = NV_GLM52_KEY_MODE_BIND.get(key_idx) or KEY_MODE_BINDING.get(key_idx)
        ...
        # 跳过冷却/auth-fail 的 key (caller-bound / buffer-override 同 key 重试时跳过此检查)
        if not _is_key_fixed:
        ...
        # R1246 (2026-08-10): pexec 不可达时, 同 key 立即用 integrate + 5 US IP (rr_us) 兜底一次,
        # 兜底成功即返回; 兜底也 fail 则照常 cooldown + 换下一 key (下一 key 回 pexec 主链).
        # ⚠️ 仅当 attempts 是 RR (非 caller-bound / 非 buffer-override 单 key) 且本 attempt 走 pexec 时触发.
        # integrate 兜底用 rr_us 轮换 (不定 key 绑定 proxy), 保持 5 US IP 多样性.
        if channel == "pexec" and not _is_key_fixed:
            _fb_proxy = _glm52_resolve_proxy("rr_us", attempt, None)
            r_fb = _glm52_single_attempt(oai_body, tier_model, request_id, metrics, t_start,
                                         is_stream, key_idx, "integrate_us_rr", "integrate",
                                         _fb_proxy, list(all_attempts), upstream_timeout_override,
                                         attempt_idx=attempt)
            all_attempts = r_fb.key_cycle_attempts
            fb_ok = r_fb.success and not r_fb.empty_200
            _log("NV-GLM52-INTEGRATE-FB", f"tier={tier_model} k{key_idx+1} pexec→integrate 兜底 "
                                          f"success={fb_ok} proxy={_fb_proxy}")
            if fb_ok:
                glm52_save_mode_idx(mode_idx)
                r_fb.fallback_tiers_used = [tier_model]
                metrics["tier_model"] = r_fb.tier_model
                metrics["fallback_tiers_used"] = r_fb.fallback_tiers_used
                metrics["glm52_mode"] = "integrate_us_rr"
                metrics["nv_key_idx"] = key_idx
                if r_fb.function_id:
                    metrics["function_id"] = r_fb.function_id
                func_health.record_result(r_fb.function_id, True)
                _next_nv_key(tier_model)
                return r_fb
            # integrate 兜底也失败: 记录并回落 (channel 保持 pexec 语义供下方日志, cooldown 同 key)
        if _is_caller_bound:
...
def execute_request(handler, oai_body, mapped_model, request_id, metrics, t_start, upstream_timeout_override=None):
    ...
    for retry_idx in range(2):
        ...
        for tier_idx, tier_model in enumerate(tier_order):
            ...
            # R-dsv4f-dynamic: dsv4f_nv per-key pexec↔integrate 动态切换.
            _dsv4f_dynamic_done = False
            if is_first_tier and tier_model in ("dsv4f_nv", "dsv4f0731_nv") and not _chain_failed:  # R1247: dsv4f0731 复用 pexec→integrate dynamic 兜底
                dynamic_result = _try_dsv4f_dynamic_keys(oai_body, tier_model, request_id, metrics, t_start,
                                                           is_stream, all_attempts, upstream_timeout_override)
                if dynamic_result.success and not dynamic_result.empty_200:
                    dynamic_result.fallback_tiers_used = [tier_model]
                    metrics["tier_model"] = dynamic_result.tier_model
                    metrics["fallback_tiers_used"] = dynamic_result.fallback_tiers_used
                    if dynamic_result.function_id:
                        metrics["function_id"] = dynamic_result.function_id
                    if retry_idx > 0:
                        _log("NV-STARTUP-RETRY-SUCCESS", f"Startup retry #{retry_idx} succeeded (dsv4f dynamic mode)")
                        metrics["startup_retry"] = retry_idx
                    if dynamic_result.function_id and dynamic_result.function_id != "integrate":
                        func_health.record_result(dynamic_result.function_id, True)
                    return dynamic_result
                # Dynamic mode failed → set tier_result, skip normal pexec/integrate branches
                _log("NV-DSV4F-DYNAMIC-FAIL-BRANCH", f"req={request_id} tier={tier_model} dynamic mode all-failed")
                all_attempts = list(dynamic_result.key_cycle_attempts)
                if dynamic_result.function_id and dynamic_result.function_id != "integrate":
                    func_health.record_result(dynamic_result.function_id, False)
                all_tier_summaries.append({
                    "tier": tier_model,
                    "all_429": dynamic_result.all_429,
                    "all_empty_200": dynamic_result.empty_200,
                    "all_cooldown": False,
                    "num_attempts": len([a for a in dynamic_result.key_cycle_attempts if a.get("tier") == tier_model]),
                    "elapsed_ms": dynamic_result.elapsed_ms,
                    "dynamic_mode": True,
                })
                tier_result = dynamic_result
                _dsv4f_dynamic_done = True
            else:
                _dsv4f_dynamic_done = False

            # R-channel (通路模式): dsv4p_nv 4-channel rotation (3 pexec fids + 1 integrate).
            _dsv4p_channel_done = False
            if is_first_tier and tier_model == "dsv4p_nv" and not _chain_failed:
                channel_result = _try_dsv4p_channel_keys(oai_body, tier_model, request_id, metrics, t_start,
                                                           is_stream, all_attempts, upstream_timeout_override)
                if channel_result.success and not channel_result.empty_200:
                    channel_result.fallback_tiers_used = [tier_model]
                    metrics["tier_model"] = channel_result.tier_model
                    metrics["fallback_tiers_used"] = channel_result.fallback_tiers_used
                    if channel_result.function_id:
                        metrics["function_id"] = channel_result.function_id
                    if retry_idx > 0:
                        _log("NV-STARTUP-RETRY-SUCCESS", f"Startup retry #{retry_idx} succeeded (dsv4p channel mode)")
                        metrics["startup_retry"] = retry_idx
                    func_health.record_result(channel_result.function_id, True)
                    return channel_result
                # Channel mode failed → set tier_result, skip normal pexec/integrate branches
                _log("NV-CHANNEL-FAIL", f"req={request_id} tier={tier_model} channel mode all-failed")
                all_attempts = list(channel_result.key_cycle_attempts)
                if channel_result.function_id:
                    func_health.record_result(channel_result.function_id, False)
                all_tier_summaries.append({
                    "tier": tier_model,
                    "all_429": channel_result.all_429,
                    "all_empty_200": channel_result.empty_200,
                    "all_cooldown": False,
                    "num_attempts": len([a for a in channel_result.key_cycle_attempts if a.get("tier") == tier_model]),
                    "elapsed_ms": channel_result.elapsed_ms,
                    "channel_mode": True,
                })
                tier_result = channel_result
                _dsv4p_channel_done = True
            else:
                _dsv4p_channel_done = False
            ...
            if (not _dsv4p_channel_done and not _dsv4f_dynamic_done and is_first_tier and NV_INTEGRATE_ENABLED
            ...
            elif (not _dsv4p_channel_done and not _dsv4f_dynamic_done and is_first_tier and NV_INTEGRATE_ENABLED and tier_model in NV_INTEGRATE_MODELS
                    and not _integrate_is_path_cooling()):
                integ_result = _try_integrate_keys(oai_body, tier_model, request_id, metrics, t_start,
                                                    is_stream, all_attempts, upstream_timeout_override)
                if integ_result.success and not integ_result.empty_200:
            ...
            if _dsv4p_channel_done or _dsv4f_dynamic_done:
                pass  # tier_result already set by channel mode
            elif _chain_failed:
    ...
    _log_error_detail({
        ...
        # R-dsv4f-log: record function_id + per-attempt details for long-term analysis
        "function_id": metrics.get("function_id", ""),
        "key_cycle_attempts": [{"nv_key_idx": a.get("nv_key_idx"), "error_type": a.get("error_type"),
                                  "function_id": a.get("function_id", ""), "upstream_type": a.get("upstream_type", ""),
                                  "path": a.get("path", ""), "tier": a.get("tier", "")}
                                 for a in all_attempts if isinstance(a, dict)],
    })
