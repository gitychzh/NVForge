...
NVCF_PEXEC_MODELS = {
    ...
    "dsv4f_nv": {
        # 2026-08-05: NVCF ai-deepseek-v4-flash (52e1ddb6, ACTIVE). pexec + integrate 均可用.
        # 多链路实测: pexec US avg 3.7s (5/5), integrate US avg 5.5s (4/5). 最稳定最快.
        # strip 同 dsv4p_nv (reasoning_effort/stream_options/thinking), inject 空, 普通模式秒回.
    ...
    "dsv4f0731_nv": {
        ...
        #   created 2026-08-07, ownedByDifferentAccount=NVIDIA官方). 实测 pexec 5/5 US IP 全通,
        #   ttfb 0.9-7.3s (avg ~3.2s), 比旧 52e1ddb6 (10-28s) 快且稳.
        #   注意: 此 FID 的 pexec body model 必须用 deepseek-ai/deepseek-v4-flash-0731
        #   (普通名 404 Inference error); integrate 端点只认普通名 → pexec_model_id 区分.
    ...
    "minimax_m3_nv": {
        # 2026-08-05: NVCF ai-minimax-m3 (87ea0ddc, ACTIVE). pexec 404, 仅 integrate 路径可用.
        # model id on integrate = "minimaxai/minimax-m3". key5 + US proxy 7899 验证 200 OK.
        # NV_KEY_INTEGRATE_KEYS=minimax_m3_nv:5 已在 env, 全 key 走 integrate.
...
# R704: 旧已下架 tier 移除.
NV_MODEL_TIERS = ["kimi_nv", "dsv4p_nv", "dsv4f_nv", "dsv4f0731_nv", "glm5_2_nv", "minimax_m3_nv"]

NV_MODEL_IDS = {
    "kimi_nv": "moonshotai/kimi-k2.6",
    "dsv4p_nv": "deepseek-ai/deepseek-v4-pro",
    "dsv4f_nv": "deepseek-ai/deepseek-v4-flash",
    "dsv4f0731_nv": "deepseek-ai/deepseek-v4-flash",  # 普通 model_id (integrate 用); pexec 用 pexec_model_id 带 -0731 后缀
    "glm5_2_nv": "z-ai/glm-5.2",
    "minimax_m3_nv": "minimaxai/minimax-m3",
}

# R0810: dsv4f0731_nv (281478d0, ACTIVE) 最稳定, 设为默认兜底.
# 旧 dsv4f_nv (52e1ddb6) 已 INACTIVE → FID 404. dsv4p_nv EOL 2026-08-07. kimi 全死.
# glm5_2_nv (3b9748d8) 可用但 pexec 3/5 成功率, integrate 超时/EOF.
DEFAULT_NV_MODEL = os.environ.get("DEFAULT_NV_MODEL", "dsv4f0731_nv")
