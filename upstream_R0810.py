...
from . import big_input_breaker  # R1695: big input breaker
...
def _try_tier_keys(oai_body, tier_model, request_id, metrics, t_start,
    ...
    if is_tier_degraded(tier_model):
        ...
        result.elapsed_ms = int((time.time() - t_start) * 1000)
        return result
    for attempt_idx in range(probe_key_limit if probe_key_limit else (NVU_NUM_KEYS + 2)):
...
def _glm52_single_attempt(oai_body, tier_model, request_id, metrics, t_start,
    ...
    # Body: reuse _build_pexec_body (strip + inject). R-fid0731: pexec 路径用 pexec_model_id.
    req_body = _build_pexec_body(oai_body, tier_model, nvcf_config, pexec_model=(channel == "pexec"))
...
def execute_request(handler, oai_body, mapped_model, request_id, metrics, t_start, upstream_timeout_override=None):
    ...
    # R1695: BIG_INPUT breaker OPEN -> return all_tiers_exhausted immediately
    # (peer-fallback in handlers.py will try HM2)
    ...
    if (big_input_breaker.is_big_input(_bi_input)
        ...
        _log("NV-BIGINPUT-FB-OPEN", f"big_input breaker OPEN for {mapped_model} "
                                   ...
                                   f"returning all_tiers_exhausted for peer-fallback "
                                   f"(state={big_input_breaker.big_input_breaker_state()})")
        final_result = UpstreamResult()
        final_result.success = False
        final_result.all_keys_exhausted = True
        final_result.all_429 = False
        final_result.empty_200 = False
        final_result.key_cycle_attempts = []
        final_result.tier_attempts = []
        final_result.fallback_tiers_used = [mapped_model]
        final_result.elapsed_ms = int((time.time() - t_start) * 1000)
        final_result.final_resp_status = 502
        return final_result

    # R2349: HALF-OPEN probe — set probe_key_limit=1 when breaker is HALF_OPEN
    _probe_key_limit = None
    if (big_input_breaker.is_big_input(_bi_input)
            and mapped_model in big_input_breaker.NVU_BIG_INPUT_MODELS
            and big_input_breaker.is_big_input_half_open()):
        _probe_key_limit = big_input_breaker.get_max_probe_keys()
        _log("NV-BIGINPUT-HALF-OPEN", f"big_input breaker HALF_OPEN for {mapped_model} "
              f"input={_bi_input}c (req={request_id}), "
              f"probe_key_limit={_probe_key_limit}, "
              f"state={big_input_breaker.big_input_breaker_state()}")
    ...
    for retry_idx in range(2):
        ...
        for tier_idx, tier_model in enumerate(tier_order):
            ...
            tier_result = _try_tier_keys(oai_body, tier_model, request_id, metrics, t_start,
                                         is_stream, all_attempts, upstream_timeout_override,
                                         probe_key_limit=_probe_key_limit)
            ...
            if tier_result.success and not tier_result.empty_200:
                ...
                # R1695: 超大 input nv success -> big_input breaker CLOSED
                _bi_input = metrics.get("total_input_chars", 0) or 0
                if big_input_breaker.is_big_input(_bi_input):
        ...
        _log("NV-ALL-TIERS-FAIL", f"All {len(tier_order)} tiers failed "
                                   ...
                                   f"elapsed={int((time.time() - t_start) * 1000)}ms")

        has_429 = any(s.get("all_429") for s in all_tier_summaries)
        has_empty = any(s.get("all_empty_200") for s in all_tier_summaries)

        # R-DSV4P-ADAPTIVE: pexec 全挂时, dsv4p_nv 尝试 integrate 路径作为跨链路 fallback.
        # pexec (74f02205) 和 integrate (integrate.api.nvidia.com) 是独立故障域 —
        # NVCF pexec surge 时 integrate 路径常仍可用 (今天实测 CN 直连 integrate 5/5).
        # 仅 dsv4p_nv + pexec 已试过 + 非 429 (429 是 key 级限流, 跨链路不增加 key 池).
        # 不影响 glm5_2_nv (已有 MODE_CHAIN) / kimi_nv / minimax_m3_nv (已走 integrate).
        if (mapped_model == "dsv4p_nv" and not has_429
                and NV_INTEGRATE_ENABLED
                and not _integrate_is_path_cooling()):
            _log("NV-DSV4P-ADAPTIVE", f"pexec all-failed, attempting integrate fallback "
                                       f"for {mapped_model} (req={request_id})")
            _integ_result = _try_integrate_keys(oai_body, tier_model, request_id, metrics,
                                                 t_start, is_stream, all_attempts,
                                                 upstream_timeout_override)
            if _integ_result.success and not _integ_result.empty_200:
                _integ_result.fallback_tiers_used = [tier_model]
                metrics["tier_model"] = _integ_result.tier_model
                metrics["fallback_tiers_used"] = _integ_result.fallback_tiers_used
                metrics["dsv4p_adaptive_path"] = "integrate_fallback"
                _log("NV-DSV4P-ADAPTIVE", f"integrate fallback SUCCESS for {mapped_model} "
                                           f"(req={request_id})")
                if _integ_result.conn:
                    try:
                        _integ_result.conn.close()
                    except Exception:
                        pass
                return _integ_result
            _log("NV-DSV4P-ADAPTIVE", f"integrate fallback also failed for {mapped_model} "
                                       f"(req={request_id})")
            all_attempts = list(_integ_result.key_cycle_attempts)
            all_tier_summaries.append({
                "tier": tier_model,
                "path": "nv_integrate_fallback",
                "all_429": _integ_result.all_429,
                "all_empty_200": _integ_result.empty_200,
                "num_attempts": len([a for a in _integ_result.key_cycle_attempts
                                     if a.get("tier") == tier_model]),
                "elapsed_ms": _integ_result.elapsed_ms,
            })
    ...
    # R2349: _probe_key_limit set earlier in execute_request (HALF-OPEN probe logic)
    # R1695: 超大 input nv hang -> big_input breaker failure
    _bi_input = metrics.get("total_input_chars", 0) or 0
    if (big_input_breaker.is_big_input(_bi_input)
            and not final_result.all_429):
        if final_result.empty_200:
            _bi_err = "empty_200"
        elif _probe_key_limit is not None and _probe_key_limit == 1:
            # R2349: HALF-OPEN single-key probe failure → timeout
            _bi_err = "pexec_timeout"
        else:
            _bi_err = "all_keys_exhausted"
