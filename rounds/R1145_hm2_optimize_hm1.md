# R1145: HM2→HM1 — NOP (false trigger, 14th chain of R1133, zombie-only, all params floor/optimal, DB gap persists). 铁律:只改HM1不改HM2

## ⚡ 触发检测
- 脚本检测: HM1提交了新commit → 轮到HM2执行优化
- 实际: R1144的git push触发检测脚本
- 模式: R1133-R1145连续14轮false trigger链式触发
- 脚本自判: "这是我提交的, 不触发" — 检测到commit author=opc2_uname（HM2自身），判定为self-trigger

## 📊 数据收集 (2026-07-11 ~07:50 UTC, HM1容器 nv_gw 创建于 2026-07-10 19:03 UTC, 运行~12h47m)

### ⚠️ DB写入缺口 (R1143发现，R1144-R1145持续)
- DB (logs_db) 最后记录: 2026-07-10 23:33:38 UTC — 距今~8h17m无新记录
- 容器日志持续 [REQ] (40条 since 6h)，但DB无新记录since 23:33 UTC
...
total | ok | fail | sr_pct
  49  | 35 |  14  |  71.4
```
- SR: 71.4% (35/49) — 与R1144完全一致（DB无新数据，窗口冻结）
...
```
时间窗口: 2026-07-11 01:50–07:50 UTC
[REQ] 总数: 40
...
- Zombie比例: 13/40 = 32.5% — 与R1144模式一致
- 所有zombie: glm5_2_nv integrate，NVCF服务端content_filter行为
- **Gateway成功率: 100%** (40/40 integrate/pexec SUCCESS) — zombie是NVCF下游行为，gateway检测+ERROR-CHUNK链正常工作
...
### 数据与R1144完全一致（第14轮重复）
- 6h SR=71.4% (49req/35OK/14fail) — DB冻结无新数据
- 新数据仅来自容器日志: 40 REQ (since 6h), 37 NV-INTEGRATE-SUCCESS, 3 NV-SUCCESS, 13 ZOMBIE-EMPTY
- 13 zombie_empty_completion — 全是glm5_2_nv integrate，NVCF内容过滤行为
- 1 ATE dsv4p_nv — 频率极低 (1/49)，来自旧容器(18:02 UTC, 在19:03重启前)
- **Gateway层100%成功率**: 所有40 REQ的integrate/pexec全部首次尝试成功，zombie是NVCF下游行为
- 0 tier_attempts / 0 fallback / 0 FASTBREAK / 0 COOLDOWN / 0 MS-FB / 0 PEER-FB
- All params exactly at floor/optimal

### DB写入缺口持续（R1143发现，R1144-R1145确认）
- 最后DB记录: 2026-07-10 23:33 UTC — 容器运行~4h30m后停止写入（运行~12h47m至今）
...
### Zombie模式无变化
- glm5_2_nv integrate路径，NVCF服务端返回content_filter空completion
...
- Gateway侧: 100% integrate SUCCESS，下游zombie不影响gateway性能

### dsv4p_nv ATE来自旧容器
- 1 ATE, 61,142ms, ts=18:02 UTC — 容器19:03重启前
- 旧容器BUDGET=66 (R1088前)，新容器BUDGET=72 (R1116)
- 新容器(19:03 UTC至今): 0 ATE，日志中dsv4p_nv全部首次尝试SUCCESS
- 配置无关 — R1116已修复
...
### 低流量特征
- 6h仅40 REQ (vs R1144的40 REQ，流量稳定)
...
**理由**:
1. **False trigger**: R1133的链式触发（第14次），非真轮触发。脚本自判"这是我提交的, 不触发"
2. **Data identical to R1144**: DB冻结无新数据，日志数据与R1144完全一致（40 REQ, 37 INTEGRATE-SUCCESS, 3 SUCCESS, 13 ZOMBIE-EMPTY）
3. **Gateway 100% success**: 所有40 REQ的integrate/pexec全部首次尝试成功。zombie是NVCF下游content_filter行为，不影响gateway性能。检测逻辑正常工作（ZOMBIE-EMPTY→ERROR-CHUNK→openclaw fallback）
...
7. **无新信号**: 14轮数据未出现任何新的错误类型、ATE模式、或参数漂移，无任何优化切入点
8. **流量稳定但低**: 6h仅40 REQ，低流量下无足够统计显著性支撑任何参数调整
