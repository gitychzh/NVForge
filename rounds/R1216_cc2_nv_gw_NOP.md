# R1216 cc2 nv_gw — NOP 巡检轮 (SR 100%, 0 错误, fallback 0%)

> 轮: R1216  |  日期: 2026-08-08  |  容器: nv_gw /health ok (5 keys, dsv4f0731_nv 单模式)
> 主链 fid: 281478d0-f307 稳定, dsv4f0731_nv 单模式 (active 流量)
> 上一轮: R1215 (NOP — SR 100%, 0 错误, fallback 0%)

## 判定: NOP — 无改码条件

## 依据 (轮前注入链路分析 + 活查确认)

- **30min cc2-primary (nv_requests)**: 活查 `200|99`, **无 502/4xx** → SR=**100% (99/99)**。
- **30min 错误分类 (活查)**: 空 (0 rows) → 无 buffer_exhausted / stream_total_deadline / 其他。
- **30min 注入全量 dsv4f0731_nv**: 169/169 SR=**100%** (含 hermes 69)。
...
- **per-key tier** (nv_tier_attempts 活查): k2 NVCFPexecRemoteDisconnected×1, 其余全
  pexec_success → 均为 attempt 级瞬时抖动, 被重试自愈到 status 200, 非净新增
  (与 R1212-R1215 同签名的链路单点瞬抖)。
- **buffer 日志 (活查)**: 绝大多数请求 attempt=1 即 success_text/success_tool_call。一条
  `d63f7c8a` req 08:00:11 k4 `execute_failed` (all_keys_exhausted=True) → 5s backoff →
  attempt 2 success_tool_call 200 (~16s)。连续重试自愈, 防御链按设计拦截。无 WAIT- 阻塞,
  无 buffer_exhausted。
...
- **容器健康 (活查)**: nv_gw /health ok (5 keys + pexec_models 含 dsv4f0731_nv), cc4101 ok,
  ms_gw ok。参数与 R1215 一致 (见参数快照), 无重配置迹象 → 非配置回归。

## 验证
活查 30min cc2-primary 99/99 (0 错误), dsv4f0731_nv 全量 169/169 SR 100%、fallback 0%;
容器 health ok、参数无漂移。k4 1 次 execute_failed + k2 1 次 RemoteDisconnected 均为
attempt 级瞬时抖动被重试自愈到 200, 无任何净新增。
→ 无改码条件, NOP。
