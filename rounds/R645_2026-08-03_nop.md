...
- dsv4p_nv 30min (hermes caller): 28 req, 25×200 + 3×502 (SR=89.3%)
  - vs R644 83.3% / R643 85.7% / R642 78.9% / R641 70.6% / R639 61.5% / R634-R637 连续 4 轮 37.5%
    → **本轮 +6.0pp vs R644 (83.3%), 仍在 R617-R644 正常波动区间 (37-91%) 上沿, 配额持续恢复, 趋势向好, 无突变恶化**
    → 6h 趋势: 29-87% 波动, 04:00 回升 86.8%, 06:56 本轮 89.3%, 配额型故障模式未变
...
- 错误分类 2 类 (无新模式):
  - `all_tiers_exhausted` ×2 (avg_dur 20346ms)
    - 502×2 NVCF 慢响应 (peer-fb skip → 本地 502 返, cc4101 ms_gw 兜底)
    → R624→...→R634-R644: 3→...→3→4→5→5→5→5→5→5→4→3→3→2 (持平正常区间 3-6 下沿, -1 vs R644, 趋势向好)
  - `NVStream_IncompleteRead` ×1 (avg_dur 36755ms)
    → 502 路径里的不完整流读, NVCF 慢响应/连接中断子类型, **R644 已观察 1 例, 本轮仍 1 例, 未演变为模式 (阈值 >=3/30min 未触及)**
...
- 波动 29-89%, 模式不变: NVCF 账户级配额耗尽 (429 无 retry-after, 全 5key 429 → TIER_COOLDOWN 180s)
- 04:00 回升 86.8%, 06:56 本轮 89.3% (本轮为 6h 最高) → 配额型故障模式确认, 非 nv_gw 侧可改

## 持续观察点 (沿用 R644, 不改码) — dsv4p_nv 配额型 429 全挂
- 6h SR 波动 29-89%, 04:00 回升 86.8%, 06:56 本轮 89.3% (6h 最高) → 非 nv_gw 侧可改
...
- NVStream_IncompleteRead ×1 = 502 路径的不完整流读, NVCF 慢响应子类型, 连续 2 轮单发, 未演变, 非 nv_gw 故障

## 本轮改动
- 无 (NOP). 根因不变: NVCF 账户级配额耗尽 (429 无 retry-after), 非 nv_gw 侧可改.

## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R644)
- dsv4p_nv SR=89.3% +6.0pp vs R644 (83.3%), 仍在正常波动区间 (37-91%) 上沿, 趋势向好, 无突变恶化
- 6h SR 趋势 29-89% 波动, 04:00 回升 86.8%, 06:56 本轮 89.3% (6h 最高) → 配额型故障模式确认
- all_tiers_exhausted ×2 (-1 vs R644 ×3), 仍在正常波动区间 3-6 下沿, 趋势向好
- NVStream_IncompleteRead ×1 = 502 路径的不完整流读, 连续 2 轮单发, 未演变为模式 (阈值 >=3/30min 未触及)
...
- 配置无漂移 (env 全项匹配 R644 快照, NVU_DISABLE_MS_FALLBACK=0/ms_gw fallback 已恢复)
