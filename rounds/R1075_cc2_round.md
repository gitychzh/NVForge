# R1075 — cc2 NOP 巡检轮 (2026-08-07 20:15 CST)

## 结论: NOP, 不改码

cc2 主链 (cc4101-primary → nv_gw:40006, dsv4f0731_nv) 完全健康:
30min = **107/107 = 100% SR, 0 bad**; cc_requests 106 条全走主链, fallback 0 次 (0.0%);
...
per-key 全 5 key pexec_success (0:22 / 1:18+1 / 2:26 / 3:19 / 4:22), key1 一次 NVCFPexecRemoteDisconnected 后 18 次 success (常态单键抖动);
buffer 日志全 attempt=1 即 success_tool_call (7-21s, input 66-72K tokens), 无 fail/WAIT/KEYMGR 死锁;
容器 /health 全 200 (40006 nv_gw, 40066 dsv4p, 4101 cc4101), nv_gw Up 17h, cc4101 Up 16h.

## 依据

- 注入轮前链路分析 (20:10 CST): cc4101-primary|dsv4f0731_nv|200|107 (100%, 0 bad);
  dsv4f0731_nv 整体 SR=98.9% (179/181), 2×502 zombie_empty_completion (avg_dur 4459ms); per-key 0/2/3/4 pexec_success, key1 18 success + 1 RemoteDisconnected.
- 独立 DB 复核: zombie×2 归属 host_machine=opc2sname-dsv4f40666 (越界 40666 hermes 线),
  request_id JOIN cc_requests 无行 → 非 cc2 主链; nv_gw caller 分布 = cc4101-primary|200|107, hermes|200|66 + 502|2.
- 30min cc_requests (106 条) = sr 100.0%, fallback 0 次 (0.0%), 全走主链。
- buffer 日志抽查 20:11: 4 条 BUFFER-START 全部 attempt=1 即 success_tool_call 并 flushed (7-21s), verdict=success_tool_call, 无 fail/WAIT/KEYMGR。
...
| 30min cc_requests | 106 条, sr 100%, fallback 0 次 (0.0%), 全走主链 | ✅ |
| dsv4f0731_nv 整体 | 179/181 = 98.9% (作参考) | ✅ |
| hermes 越界宿主 (40666, 非 cc2) | zombie_empty_completion×2, 铁证 host_machine=opc2sname-dsv4f40666 | ⚠️ 非主链 |
| per-key | 全 5 key pexec_success; key1 一次 RemoteDisconnected 后 18 次 success | ✅ |
