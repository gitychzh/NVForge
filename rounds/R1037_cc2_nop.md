# R1037 cc2 NOP — cc2 primary 100% clean (110/110), scoped errors 0; bad 2 hermes (zombie_empty_completion/NVStream_IncompleteRead); fallback 0 (0/1982); 145th clean round (R893-R1037)

## 判定: NOP 巡检轮/不改码

cc2 主链路 (cc4101-primary → nv_gw:40006) 连续第 **145** 轮 (R893-R1037) 100% 干净。
...
- **30min cc4101-primary (主 nv_gw:40006) = 110/110 全 200 = 100% SR, 0 bad**
  (`SELECT caller,status,count(*) FROM nv_requests WHERE ts>now()-interval'30 min' GROUP BY 1,2`:
  cc4101-primary|200|110, hermes|200|49, hermes|502|2)。注入窗口 113 条 (17:35), live 复核微幅回落至 110, 全 200。
- **主链专属错误 (caller=cc4101-primary, status!=200) = 0 rows**
  (`SELECT caller,error_type,count(*) ... WHERE ts>now()-interval'30 min' AND status!=200 GROUP BY 1,2` 为空, 无 cc4101-primary 行)。
- **30min total bad = 2 条**: zombie_empty_completion ×1 + NVStream_IncompleteRead ×1,
...
- **nv_tier_attempts per-key 健康**: 全 5 key (k0-k4) 以 pexec_success 为主 (k0=25/k1=19/k2=26/k3=22/k4=21),
...
本轮 2 条 bad (zombie_empty_completion/NVStream_IncompleteRead) 经 caller 铁证归属全属 **hermes** 越界宿主,
与主链 host 分离完全干净 — 主链 110/110 全 200。
