# R944 cc2 NOP 巡检轮 (HM2 nv_gw 主链路)

> 轮次: R944 | 日期: 2026-08-07 CST | 类型: NOP (不改码)
> 结论: cc2 主链路连续 **52** 轮 100% 干净 (R893→R944);
> cc4101-primary 30min = **129/129 = 100% SR, 0 bad**;
...
- 30min cc4101-primary (主 nv_gw:40006) = **129/129 全 200, 0 bad (100% SR)**。
...
- **caller 列实拉铁证 (带 request_id)**: 4 bad 全 caller=hermes (105fb55f, 57772fd8, e4f8ea10, 4dd0664d), 0 个属于 cc2 主链 (host-separated)。
- zombie_empty_completion 保持 **1** (R941 首现 1 → R944 连续保持 1, 稳定未扩散, 越界 hermes 不构成 cc2 风险)。
- fallback (cc_requests 30min) = **0 次** (129 req, fb=0, SR=100.0)。
- nv_tier_attempts 30min: pexec_success 128 (约) + NVCFPexecRemoteDisconnected + empty_200 + NVCFPexecTimeout。
...
- 容器 UP: nv_gw 8h (与 R943 持平, 平稳, 非重启回归), cc4101 7h (固定节奏), dsv4p_nv40066 2d。

### 本轮数据

| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **129/129 = 100% SR, 0 bad** (实拉) | ✅ |
...
| zombie_empty_completion | 1 (R941 首现 → R944 连续保持 1 未扩散) | ⚠️ 观察 |
| tier 瞬态错误 | RemoteDisconnected + empty_200 + Timeout, 全被吸收 | ✅ |
| scoped health | 4101/40006/40066 全 ok (200) | ✅ |
| 容器 UP | nv_gw 8h (平稳) / cc4101 7h / dsv4p 2d | ✅ |

### 验证
- curl 4101/40006/40066 → 全 ok (200)。
- 30min nv_requests cc4101-primary 实拉 = 129/129 (0 bad); 专属错误 0 rows。
...
### 关键判断
cc2 主链路连续第 **52** 轮 (R893-R944) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
bad 请求 100% 属 hermes (caller 列实拉铁证带 request_id, 未进 cc2 主链), fallback 0 次, 无新 cc2 主链错误类。
zombie_empty_completion 连续第 4 轮保持单例 (R941→R942→R943→R944), 未扩散, 属 hermes 越界。
...
> ⚠️ 观察项 (R944): nv_gw Up = 8h (与 R943 持平, 平稳无重启回归);
