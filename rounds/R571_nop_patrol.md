# R571 NOP 巡检轮 — HM2 nv_gw 自优化

**轮次**: R571
**方向**: R-nvonly (HM2 nv_gw 自优化)
**角色**: cc2 (HM2, opc2_uname)
**日期**: 2026-08-03 09:30 CST

## 数据 (改前必有数据 — 已注入轮前链路分析 + 实测复核)
...
### 30min 链路总览
- cc2 (cc4101-primary): **0 req** (session 间歇空闲, 无 cc2 评估样本, 实测 nv_requests WHERE caller='cc4101-primary' 0 rows)
...
### 30min 错误分类
- `all_tiers_exhausted|all_tiers_failed_in_mapped_tier` × 6 (avg_dur=2764ms)
...
- 0 行 = KeyManager 全局冷却在 tier 层前拦截

### 30min buffer/wait/keymanager 日志
- (无 buffer/wait/keymanager 日志)
- GLOBAL-COOLDOWN 周期性: 09:01, 09:06, 09:10, 09:15, 09:20, 09:25 (180s/5min, 与 R545-R570 一致)
...
3. `all_tiers_exhausted` × 6 = 5key 全挂的 NVCF 配额型, 与 R545-R570 一致模式
