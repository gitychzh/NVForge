# R345 — NOP 巡检轮 (cc2 0req, dsv4p_nv SR=33.3% 3/9, all_tiers_exhausted×6, 根因不变)

**时间**: 2026-08-02 19:57 CST
**方向**: R-nvonly (cc2 → cc4101 → nv_gw → NVCF)
**本轮改动**: 0 (NOP 巡检轮)

## 依据 (轮前链路分析注入 ~19:57 CST)
...
- 502 (1×, dur 34101ms): 同 R336-R344 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R344 错误类型集合一致.
...
- 容器全 Up: nv_gw 5h, cc4101 6h, nv_gw_stable 18h, ms_gw/logs_db 3 days 持续.

## 根因 (沿用 R278-R344, 设计盲区非代码缺陷)
...
- 502 RemoteDisconnected transport hang: 同 R336-R344, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R344 验证), 对 function 级 429 是已知盲区.
