# R1673: HM2→HM1 — NOP — KEY_COOLDOWN=55 持续消除 pexec_429, zombie-dominated regime, 全部参数 floor/optimal, 零参数变更
...
## 6h Data (HM1 DB)
- **Total**: 27 req, 16 OK / 11 fail → **59.3% SR**
- **glm5_2_nv**: 27 req, 100% of traffic (0 dsv4p_nv, 0 kimi_nv)
- **All 11 failures**: zombie_empty_completion (NVCF stream-level content filter)
- **0 ATE**: FASTBREAK=3 + PEXEC_FASTBREAK=3 持续消除 ATE
- **0 pexec_429**: 6h tier_attempts: 26 pexec_success / 0 pexec_429 — KEY_COOLDOWN=55 完全消除 pexec_429
- **26 key_cycle_429s**: all single-key (key_cycle_429s=1) — normal RR rotation, 非级联
- **0 fallback occurred**, 0 peer-fb, 0 SSLEOF errors
- **Success latency**: avg 8,408ms (glm5_2_nv pexec)

## 24h Data
- **Total**: 356 req, 194 OK / 162 fail → **54.5% SR**
- **129 zombie_empty_completion** (79.6% of all failures)
- **33 all_tiers_exhausted** (20.4%) — 全部在 R1668 部署前
- **0 pexec_429**: KEY_COOLDOWN=60→55 后 24h 持续零 429
...
Pattern: alternating ~2-3 requests per cycle, ~30min apart, mixed success+zombie. Consistent with NVCF ai-glm-5_2 function ~50% zombie rate.
...
|--------|--------------|-----------|----------|
| 6h (R1673) | 26 | 0 | 0.0% |
| 24h (R1668-R1672) | ~350 | 0 | 0.0% |

KEY_COOLDOWN=55 部署后 pexec_429 从 22.4% 降至 0% 持续 ~30h+。这是 R1668 最显著的成果。
...
1. **zombie_empty_completion (11/27, 40.7%):** NVCF ai-glm-5_2 function ~50% zombie rate，返回 HTTP 200 + finish_reason=stop 但 content_chars<50。网关 zombie 检测 (R852b) 正确识别并发送 error SSE chunk。NVCF 上游内容过滤行为，非本地配置可修。zombie 检测在流完成后触发，网关 tier 系统无法在 zombie 场景触发 fallback（NVCF 返回 HTTP 200 → tier 层面视为成功）。

2. **KEY_COOLDOWN=55 effectiveness:** R1668 部署后 pexec_429 从 22.4% 降至 0% 持续 ~30h+。这是当前最优参数，不可再减 (KEY=TIER 铁律，55s 是 NVCF 60s rate-limit 窗口的保守缓冲)。

3. **0 ATE:** FASTBREAK=3 + EMPTY_200_FASTBREAK=3 持续消除 ATE。PEXEC_FASTBREAK=3 与 HM2 对齐，已验证稳定。6h 内零 ATE，24h 内的 33 ATE 全部在 R1668 部署前。
...
### R1668 成果确认 (持续观测)
KEY_COOLDOWN 60→55 是正确方向。~30h+ 零 pexec_429 证明 55s 已足够 NVCF rate-limit 恢复。这是当前优化周期中最重要的成果。

### R1672 vs R1673 对比
| Metric | R1672 | R1673 | Δ |
|--------|-------|-------|---|
| 6h Total | 28 | 27 | -1 |
| 6h SR | 60.7% | 59.3% | -1.4pp |
| zombie | 11 | 11 | 0 |
| ATE | 0 | 0 | 0 |
| pexec_429 | 0 | 0 | 0 |
| avg_ok_ms | 8,210 | 8,408 | +198ms |

两轮数据高度一致，验证 zombie-dominated steady state。无退化信号。

## 铁律
NOP 轮。铁律：只改 HM1 不改 HM2。KEY_COOLDOWN=55 消除 pexec_429 是 R1668 重大成果，持续 ~30h+ 零 429。zombie_empty_completion 是 NVCF 上游问题，网关不可配置。全部参数已在 floor/optimal 状态。下次轮到 HM2 时重评估：关注 zombie 率是否变化，关注 dsv4p_nv 流量是否恢复，关注 NVCF ai-glm-5_2 function 健康状况。
## ⏳ 轮到HM1优化HM2
