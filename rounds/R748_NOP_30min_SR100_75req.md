# R748 — cc2 nv_gw NOP 巡检轮

> 时间: 2026-08-05 05:31 CST | 上轮: R747 | 改动: 不改码 (NOP)
...
|---|---|---|---|
| 总请求 | 75 | 75 | 完全一致 |
| 200 成功 | 75 | 75 | SR=100.0% |
| fallback 触发 | — | 0 | fb=0.0% (created_at 实测) |
| 注入 f\|88 | — | ts 列时区 bug 口径 | created_at 实测 0 fb (沿 R742-R747 实证) |

→ **SR=100%, fb=0% — 第 14 连续 100% 轮 (R735~R748)**
...
| k3 | 16 | 16 |
| k4 | 14 | 14 |
| 总 | 76 | ≈ cc2 75×200 (差 1 = 30min 边界/hermes 流量) |

**全 5key 均 pexec_success, 零错误穿透 cc2** — 最干净一轮

### 注入噪声 (全部来自 hermes→dsv4f0731_nv, 不在 cc2 路径)
- hermes dsv4f0731_nv: 200×4 + 502×9 = SR=30.8% — NVCF 容量 (不穿透)
- all_tiers_exhausted ×7 (hermes 路径, avg_dur 85622ms)
- NVStream_IncompleteRead ×2 (hermes 路径)
- per-key 噪声 (注入的):
  - NVCFPexecRemoteDisconnected: k0=1, k1=3, k2=5, k3=5, k4=1 = 15 — 被 buffer 兜住, 不可见到 cc2
  - 529_nv_overloaded: k0/k1/k2 各 1 = 3 — 被缓冲
  - empty_200: k1=1 — 边界边界
- → 全被 fid1 pexec buffer 吸收, cc2 实测 0 错误穿透
...
| R748 | 100% (75 nv / 75 cc) | 14th consecutive, fb=0 (created_at 实测), 最干净一轮 |
