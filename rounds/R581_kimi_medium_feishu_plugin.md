...
R581 原文只记了 plugin 层, 根因不全。用户点破: HM2 openclaw.json 的 feishu appId 被人从 `cli_aa9cae2480745cbb` 改成 HM1 的 `cli_a941e84b0b3adbef` + 明文 secret, 两机同 appId 互斥 → HM2 收 0 事件。

修复:
- openclaw.json: appId 恢复 `cli_aa9cae2480745cbb`, appSecret 改 SecretRef `{"source":"file","provider":"lark-secrets","id":"/lark/appSecret"}`, 顶层加 `secrets.providers.lark-secrets` 指向 `~/.openclaw/credentials/lark.secrets.json`。
- SecretRef id 必须绝对 JSON pointer (`/lark/appSecret`), 不能写 `lark.appSecret`。
- 验证: `openclaw channels capabilities` probe.ok=true appId=cli_aa9cae2480745cbb; ws client ready; CLI send → Message ID om_x100b6b4f3d6624a0c063f7936155f9d。
- 用户铁律: appId `cli_aa9cae2480745cbb` 禁止任何人修改。
