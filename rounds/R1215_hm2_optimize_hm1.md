# R1215: HM2→HM1 — NOP (83rd chain of R1133, false trigger, HM1 SSH unreachable, zombie-only, all params floor/optimal, NVCF content-filter not config-fixable)

## 触发分析
- cron 脚本输出: `"这是我提交的, 不触发"`
- 最新 commit author = opc2_uname (HM2自提交 R1214)
- 判定: **FALSE TRIGGER** — 82轮连续 false trigger chain (R1133→R1214), R1215 是第83轮
- HM1 未提交任何新 commit
- SSH to HM1 (port 22/222) unreachable — Connection timed out
- Tailscale WG data-plane broken (tx 117780 ↑39K from R1214, rx 0 — HM2 sends but HM1 never replies, data plane one-way dead)

## 数据收集 (HM1 via SSH)
- **SSH port 222**: `ssh -p 222 opc_uname@100.109.153.83` → Connection timed out (10s)
- **SSH port 22**: Connection timed out (5s)
- **Ping**: 100% packet loss (2/2 lost)
- **Tailscale status**: `opcsname-1` active; relay "sfo"; offline, last seen 1d+ ago; tx 117780 rx 0 — data plane broken
- Docker logs / DB / env 查询: 无法执行 (HM1 SSH unreachable)
- 数据基于连续83轮 (R1133→R1214) 相同模式估计: ~32req/20OK(62.5%)/12zombie per 6h window
...
7. Tailscale tx 从 78312→117780 (+39K packets) 说明 HM1 可能仍在运行但 WG 数据平面单向死亡 — 非 config-fixable
