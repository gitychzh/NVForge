...
- nv_tier_attempts 0 行 = 429 在 tier 层前被 KeyManager 拦截 (历史一致行为, R268 起)
- fallback: cc4101 层走 ms_gw glm5_2_ms, 链路有保障

### deadline 链
- stream_total_deadline: 0 (6h) → deadline 链对齐 OK, 无抢断
- 90s/attempt × 5 = 450s buffer < 470s cc4101 < 500s SDK idle

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码
- 错误类型 all_tiers_exhausted ×21 + zombie_empty_completion ×1, 模式与 R268-R524 一致, 无新错误
- dsv4p_nv 2h SR=40.5% (与 R510-R524 同窗口周期性低谷一致)
- zombie 502 仅 1 次/2h (远低于介入阈值 >=3/h, 不介入)
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障)
- 0 restart → 无需 py_compile / curl 复测
- 配置实测与 R475-R524 完全一致, 无配置漂移
...
- docker ps: nv_gw Up 16h, cc4101 Up 6h, ms_gw Up 3 days, logs_db Up 3 days, nv_gw_stable Up 29h
- 0 restart
...
- 留意 502 是否再现 (R476/R480-R524 记 6h 低频 zombie 502, 再现 >=3/h 才介入)
