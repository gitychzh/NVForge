# R537 — NOP 巡检轮 (2026-08-03 07:31 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R536 间歇空闲窗口).
...
- dsv4p_nv 30min: 16 req SR=62.5% (10×200 + 4×429 + 2×502), 全 `hermes` caller (1× openclaw 200).
  SR 与 R536 的 62.5% 持平, 仍在 NVCF 侧配额波动区间 (40-65% band, R530-R536).
- 唯一错误类型 `all_tiers_exhausted` × 6 (avg 19.6s), 无新错误类型.
- 2× 502 = peer-fb-skip 已知路径 (23:02 + 23:17 UTC), R530-R536 同源, 非新错误.
- NV-GLOBAL-COOLDOWN tier=dsv4p_nv 周期性 429 仍在 (23:05/23:10/23:20/23:26 UTC),
  与 R268-R536 完全一致, NVCF 侧配额波动.
...
- 配置实测与 R475-R536 完全一致, 无漂移.

## 本轮改动
- 无 (NOP). 铁律1 cc2 视角不满足 (cc2 0 流量无评估样本) → 不动码.
- dsv4p_nv 错误模式与 R268-R536 完全一致 (周期性 all_tiers_exhausted + NVCF 侧 429 配额波动 +
...
- 全局冷却模式: 每 5min 1 次 NV-GLOBAL-COOLDOWN tier=dsv4p_nv all keys 429,
  与 R268-R536 完全一致, 是 NVCF 侧配额波动, 非 nv_gw 故障
