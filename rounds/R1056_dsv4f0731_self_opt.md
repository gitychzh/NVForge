# R1056: RemoteDisconnected 风暴延续 (第35轮) — 模型特异性第35次复现, NOP (无参数修改)

> 时间: 2026-08-06 10:10 BJT (02:10 UTC)
> 容器: dsvf0731_nv40666 (端口 40666, HM2, DeepSeek V4 Pro via NVCF)
> 状态: **NOP (无参数修改)** — NVCFPexecRemoteDisconnected 远程瞬断风暴在 attempt 层
>   持续主导, 第 35 次复现**模型特异性** (同容器同 key 同出口 glm5_2_nv 6h SR=95.5%
>   且 0 tier 错误, dsv4f0731_nv 6h SR=48.9% + attempt 层 RemoteDisconnected/529 主导) →
>   NVCF deepseek function 级劣化, 无本容器可解参数
> 信号: hm4104 fallback 持续 (PRIMARY-BREAKER-SKIP-STREAM 直走 fallback)

## 1. 背景 (改前必有数据)

R1021-R1055 连续记录同一 RemoteDisconnected 风暴并判定模型特异性 NOP。本轮间隔 ~10min
为同一风暴第 35 轮延续。

### 30min 窗口 (pre-run 注入) — nv_requests (tier_model=dsv4f0731_nv)
- 总量 27, 200=11, 失败=16, fallback=3, **SR=40.7%**
- Avg 139675ms, p50 75755ms, p95 511201ms, max 635327ms
- 429: 0 计数
- upstream_type: nvcf_pexec 23 (200=11, SR=47.8%), ms_fallback 3 (200=0), 空 1 (200=0)
- finish_reason: tool_calls=9, stop=2 (正常业务完成)

### 错误分类 (30min, nv_requests)
| error_type | count | avg_ms |
|---|---|---|
| all_tiers_exhausted | 11 | 196865 |
| buffer_exhausted | 3 | 254877 |
...
### per-key 错误 (30min)
- k0: all_tiers_exhausted=9, buffer_exhausted=3
...
- **glm5_2_nv 6h**: 179 请求, 171 200, **SR=95.5%**, avg=54874ms
- **dsv4f0731_nv 6h**: 399 请求, 195 200, **SR=48.9%**, avg=103020ms
- **结论**: 同容器同 key 同出口, glm5_2_nv 95.5% vs dsv4f0731_nv 48.9% → 模型特异性
  第 35 次成立。故障仅存在于 dsv4f0731_nv 具体 function 的远端执行层。

### 6h/24h 趋势
- 6h 请求层: 394 请求, 193 200, SR=49.0%, fallback=55
- 3h 逐小时: 02:00=50.0%(4/8), 01:00=33.8%(22/65), 00:00=37.2%(29/78), 23:00=46.7%(28/60)
- 24h all_tiers_exhausted: 224 次

### hm4104 fallback 日志 (最近 5min)
- **PRIMARY-FAIL-STREAM**: nv_gw 流式 server_5xx status=502 after 70197ms / 92258ms, 切 fallback
...
**远程 function 级**瞬断/劣化, 与容器 env 无关 (第 35 次确认):
...
本轮继续 NOP, 等待 NVCF 侧恢复 (第 35 轮)。模型特异性铁证 (6h glm5_2_nv 95.5% +
0 tier 错误 vs dsv4f0731_nv 48.9% + 错误均匀分散) 第 35 次成立。hm4104 断路器
...
NVU_PEER_FALLBACK_ENABLED=0. 无重启。

## 4. 验证

- /health: status=ok, nv_num_keys=5, nvcf_pexec_models 含 dsv4f0731_nv, port=40666 ✓
- 容器: dsvf0731_nv40666 Up 29 hours, env 读取正常 (与 R1055 一致, 无漂移) ✓
- 无 env 变更, 无需重启
- 本次 pre-run 注入的 fallback 日志 PRIMARY-FAIL-STREAM/PRIMARY-BREAKER-SKIP-STREAM
  为本容器自身轮次触发的正常 fallback 信号, 非本轮修改引起

## 5. 下一步建议

继续等待 NVCF deepseek function 侧恢复。本轮模型特异性铁证第 35 次成立 (6h glm5_2_nv
95.5%/0 tier 错误 vs dsv4f0731_nv 48.9%/错误均匀分散)。风暴延续 ≥36h 无衰减。
