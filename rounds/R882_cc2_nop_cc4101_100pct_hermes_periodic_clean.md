...
> 近窗 cc4101-primary SR=100% (104×200) 零错误, 残留 all_tiers_exhausted×6 全为 caller=hermes
> 外部 cron (502×6, avg ~179.5s), fallback 0%, buffer 全 attempt1 一次成交, 不改码。
> 2026-08-07 ~14:12 CST (DB UTC, 06:12 UTC)。
...
| **per-key nv_tier_attempts** | dsv4f0731_nv 5key pexec_success 20~22/key, NVCFPexecRemoteDisconnected + Timeout + 529_nv_overloaded + empty_200 瞬态全跨 key 吸收 | ✅ |
| **三容器 health** | nv_gw / cc4101 / dsv4p 均 ok, primary=dsv4f0731_nv, 5 keys | ✅ |

## 关键判断: cc2 路径全净, hermes 周期 all_tiers_exhausted 非本链路问题

30min 窗口链路总览: cc4101-primary **104×200 零错误**; 非 200 (502×6) **全部 caller=hermes**
(外部客户端, 非 cc4101): all_tiers_exhausted×6, avg 179555ms ≈ buffer deadline 全额耗尽签名 —
属 hermes 外部 cron 周期性请求 (沿用 R853-R881 判定), 而非本链路退化。
...
nv_gw(40006): pexec_us_rr 单模式, KEY_FID_BIND 全 bind b1b22d03, BUFFER 5×90s=450s, WAIT max 120s,
              ...
              FORCE_STREAM_UPGRADE=0/NVU_FORCE_STREAM_UPGRADE_TIMEOUT=150, NVU_DISABLE_MS_FALLBACK=0,
