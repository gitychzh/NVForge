# R488 — NOP 巡检轮 (2026-08-03 04:36 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 20:10-20:36 UTC (04:36 CST 注入), 与 R475-R487 同低谷窗口高度一致 (连续 14 轮).
- cc2 (cc4101-primary) 30min 0 req (cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=40.0% (4/10, 4×200 + 6×429), 较 R487 的 14.3% 回升, 仍在低谷窗口区间 (R437-R487 的 0-85% 波动内).
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R268-R487 (179+ 轮) 一致, 无新错误.
- nv_tier_attempts 30min: 单次 429 即触发 NV-GLOBAL-COOLDOWN 180s, 空 IP (429 在 tier 层前被 KeyManager 全局冷却拦截).
- stream_total_deadline 6h 0 次 (deadline 链对齐稳, 复测 0 rows 确认).
- 6h dsv4p_nv 全 caller SR=69.5% (133×200 + 54×429 + 3×502 = 190, SR=69.5%, 与 R485-R487 持平, 仍处 46-85% 历史波动区间).
- 配置实测确认与 R475-R487 完全一致, 无漂移.

## 链路数据 (04:36 CST 注入, psql 复测确认)
### 30min 窗口 (全 caller, 全 dsv4p_nv)
- 10 req: 4×200 (20:10/20:35/20:36×2, avg_dur=9128ms, ttfb=8421, finish=tool_calls, nv_key_idx=2), 6×429 (nv_key_idx=NULL, 空 IP) → SR=40.0%
- 错误分类: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier)
- per-min: 20:10|200, 20:11|429, 20:15|429, 20:20|429, 20:25|429, 20:30|429, 20:35|200, 20:36|200×2+429 (离散, 非爆发)

### cc4101-primary 专属 (cc2 的请求)
- 30min 0 req (psql 复测 0 rows 确认, cc2 session 间歇空闲)

### 6h dsv4p_nv 全 caller SR 基线
- 200×133 + 429×54 + 502×3 = 190 req, SR=133/190=69.5% (与 R485-R487 持平, 仍处 46-85% 历史波动区间)
- 502 6h 3 次 (与 R476/R480-R487 一致, 低频)

### nv_gw COOLDOWN 行为 (与 R487 一致)
- [NV-GLOBAL-COOLDOWN] tier=dsv4p_nv all keys 429. Marking all cooling 180s (TIER_COOLDOWN)
- 时序: 04:25, 04:30, 04:36 (每 5-6min 一次, 离散非爆发)
- [NV-TIER-FAIL] tier=dsv4p_nv all 5 keys failed: 429=1, empty200=0, timeout=0, other=0
...
- 这是历史一致行为 (R268 起 179+ 轮), 非本轮新故障.

## 历史波动区间 (R437-R488)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473-R481=0.0% → R482-R487=14.3% → R488=40.0% (30min 低谷, 6h 视角 69.5%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 模式与 R268-R487 一致 (179+ 轮), 无新错误.
- dsv4p_nv 30min SR=40.0% (较 R487 回升) 仍是低谷窗口, 6h SR=69.5% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
...
- 留意 502 是否再现 (R476/R480-R487 记 6h 3-4×502 低频, 再现 >=3/h 才介入).
