# R489 — NOP 巡检轮 (2026-08-03 05:30 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 20:10-20:36 UTC (04:39 CST 注入, 05:30 psql 复测确认), 与 R475-R488 同低谷窗口高度一致 (连续 15 轮).
- cc2 (cc4101-primary) 30min 0 req (psql 05:30 复测 0 rows, cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=40.0% (4/10, 4×200 + 6×429), 与 R488 完全持平, 仍在低谷窗口区间 (R437-R488 的 0-85% 波动内).
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R268-R488 (190+ 轮) 一致, 无新错误.
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截, 空 IP, 与 R470-R488 一致).
- stream_total_deadline 6h 0 次 (deadline 链对齐稳, 复测 0 rows 确认).
- 6h dsv4p_nv 全 caller SR=69.6% (133×200 + 54×429 + 3×502 = 190, SR=69.6%, 与 R485-R488 持平, 仍处 46-85% 历史波动区间).
- 配置实测确认与 R475-R488 完全一致, 无漂移.

## 链路数据 (04:39 CST 注入, 05:30 psql 复测确认)
...
- per-egress-IP: 203.10.96.139|4×200=100% (有 IP 时全成功); 空 IP|6×429 (429 全是 KeyManager 全局冷却拦截, 未进 tier)

### cc4101-primary 专属 (cc2 的请求)
- 30min 0 req (psql 05:30 复测 0 rows 确认, cc2 session 间歇空闲)

### 6h dsv4p_nv 全 caller SR 基线
- 200×133 + 429×54 + 502×3 = 190 req, SR=133/190=69.6% (与 R485-R488 持平, 仍处 46-85% 历史波动区间)
- 502 6h 3 次 (与 R476/R480-R488 一致, 低频)

### nv_gw COOLDOWN 行为 (与 R488 一致, 30min 日志)
- [NV-GLOBAL-COOLDOWN] tier=dsv4p_nv all keys 429. Marking all cooling 180s (TIER_COOLDOWN)
- 时序: 04:11, 04:15, 04:20, 04:25, 04:30, 04:36 (每 5min 一次, 离散非爆发)
...
## 历史波动区间 (R437-R489)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473-R481=0.0% → R482-R484=14.3% → R485-R486=14.3% → R487=14.3% → R488-R489=40.0% (30min 低谷, 6h 视角 69.6%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 模式与 R268-R488 一致 (190+ 轮), 无新错误.
- dsv4p_nv 30min SR=40.0% (与 R488 持平) 仍是低谷窗口, 6h SR=69.6% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
...
- 留意 502 是否再现 (R476/R480-R488 记 6h 3-4×502 低频, 再现 >=3/h 才介入).
