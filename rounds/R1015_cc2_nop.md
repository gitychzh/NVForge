# R1015 — cc2 NOP inspection round

**日期**: 2026-08-07 16:22 CST
...
| nv_requests 总 bad (非 200) | 5 条 (all_tiers_exhausted ×4, NVStream_IncompleteRead ×1), 经 DB `SELECT caller` 铁证全属 hermes | ✅(主链) |
| 30min cc_requests | 2049 request, fallback 0 次 | ✅ |
...
### 依据 (live 复核 2026-08-07 16:22 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **127/127 全 200 = 100% SR, 0 bad** (live `SELECT status,count(*) ... caller='cc4101-primary'`)。
- 主链专属错误 (caller=cc4101-primary, status!=200) = **0 rows** (所有 5 条 bad 经 `SELECT caller,status,error_type` 确认全属 hermes)。
- 30min 错误分类: `all_tiers_exhausted` ×4 + `NVStream_IncompleteRead` ×1, 全 502, 全 caller=hermes — 与主链 host 分离保持。
- fallback (cc_requests 30min 2049 request) = **0 次** 无 fallback 触发。
- nv_tier_attempts: per-key pexec_success (k0 25 / k1 26 / k2 25 / k3 24 / k4 27) + 瞬态 RemoteDisconnected(17)/Timeout(4)/empty_200(1), 全被 buffer 一次 attempt 吸收, 未穿透到 caller。
...
- health: 40006/4101 全 200; 容器 nv_gw/cc4101/dsv4p_nv40066 皆 Up。

## 关键判断

cc2 主链路连续第 **123** 轮 (R893-R1015) 100% SR 干净, 主链专属错误 0 rows。
本轮 5 条 bad (all_tiers_exhausted ×4 + NVStream_IncompleteRead ×1) 归属全属 hermes 越界宿主 (fid 52e1ddb6 泄漏路径), 与主链 host 分离保持, 主链 127/127 全 200。fallback 0 次, 无新 cc2 主链错误类, 无持久 key 疲劳。nv_tier_attempts 里瞬态 RemoteDisconnected/Timeout/empty_200 全被 buffer (attempt=1) 吸收, 未穿透。multi-key round-robin + func_health + buffer 已达稳态。
**不改码**: ①主链 SR 100% + 专属错误 0 行, 无优化需求; ②本轮 bad 全属 hermes 越界, 主链无根因可查; ③参数处于稳态, 无参数可调。
...
## 容器健康
- nv_gw Up 13h, cc4101 Up 12h, dsv4p_nv40066 Up 2d; /health 40006/4101 全 200。
