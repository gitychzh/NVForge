# R708 — NOP 巡检轮 (cc2 零流量, 链路全绿, cc4101 SR100%)

> 时间: 2026-08-03 19:08 CST
> 上轮: R707 (NOP, cc2 零流量)
...
**cc2 (cc4101-primary) 流量**: 60min 窗口 nv_requests 中 caller=cc4101-primary **零行**
- cc2 无自产流量 = 无数据不动手 (铁律1)

**cc4101 真实 SR (cc_requests 表, 含 fallback)**:
- 16req 全 200, SR **100%**, fallback_triggered=1 (fb=6.25%, <10% 目标)
- max_dur=101306ms — 无 deadline 超时

**nv_gw 全量 60min by caller×model×status**:
| caller | model | 200 | 502 | SR |
|---|---|---|---|---|
| hermes | dsv4p_nv | 72 | 4 | 94.7% |
| hermes | glm5_2_nv | 7 | 9 | 43.8% |
| openclaw | dsv4p_nv | 25 | 1 | 96.2% |
| openclaw | glm5_2_nv | 1 | 2 | 33.3% |
| other | dsv4p_nv | 1 | 0 | 100% |
- 全部非 cc2 管辖 (hermes/openclaw/other)

**60min nv_requests 错误分类**:
- NVStream_IncompleteRead ×10 — NVCF 上游 stream 中断 (非配置问题)
- all_tiers_exhausted ×5 — glm5_2_nv 5key 全败 (NVCF 上游间歇)
- stream_absolute_cap ×1 — 超总预算

**60min nv_tier_attempts 错误**:
- pexec_conn_RemoteDisconnected ×12
- IntegrateRemoteDisconnected ×9
- 429_nv_rate_limit ×6
- NVCFPexecRemoteDisconnected ×5
- pexec_success ×3, integrate_success ×3, integrate_conn_RemoteDisconnected ×1
- 全是 NVCF 上游 RemoteDisconnected/429 配额副作用, 非 nv_gw 可控

**最近 30min nv_gw 日志 (19:04-19:05)**:
- glm5_2_nv tier: k5 RemoteDisconnected → k1 integrate → k2 integrate
- k3 (pexec fid3b9748d8) RemoteDisconnected penalty=5s no conn_count → KeyMgr 快速恢复生效
- NV-STREAMBREAK-STATE: content_flushed=444c ttfb_recorded=True — mid-stream 断, 已 graceful close
- 无 BUFFER-/WAIT- 日志

**15min per-tier 错误**:
- dsv4p_nv: NVCFPexecRemoteDisconnected ×5, 429 ×1
- glm5_2_nv: pexec_conn_RemoteDisconnected ×2, IntegrateRemoteDisconnected ×1

### 验证: NOP 无需 restart
- `curl /health`:
  - nv_gw (40006): ok, 5 keys, pexec models [kimi_nv,dsv4p_nv,glm5_2_nv]
