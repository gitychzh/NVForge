...
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×3 (avg_dur 14470ms)
    ...
    → R624→...→R634→R635→R636→R637→R638→R639→R640→R641→R642→R643:
       3→...→3→4→5→5→5→5→5→5→4→3
       (持平 R634-R642 正常波动区间 3-6, 本轮 -1 vs R642, 趋势向好)
...
## 6h dsv4p_nv SR 趋势 (小时桶, 沿用 R642 记录 + 本轮)
- 23:00 52.2% → 00:00 42.1% → 01:00 29.4% → 02:00 59.1% → 03:00 56.5% → 04:00 86.8% → 05:00 76.3% → 06:00 53.8%(部分)→06:50 85.7%(本轮)
- 波动 29-87%, 模式不变: NVCF 账户级配额耗尽 (429 无 retry-after, 全 5key 429 → TIER_COOLDOWN 180s)
- 04:00 回升 86.8% (高时段配额刷新) → 配额型故障模式确认, 非 nv_gw 侧可改
- 本轮 06:50 85.7% (vs R642 06:45 78.9%) → 配额持续恢复, SR 稳定向好

## 持续观察点 (沿用 R642, 不改码) — dsv4p_nv 配额型 429 全挂
- 6h SR 波动 29-87%, 04:00 回升 86.8%, 06:50 本轮 85.7% → 配额型故障模式确认, 非 nv_gw 侧可改
- 01:00 单小时 SR=29.4% 触及 R621 升级阈值 (SR<55%), 但:
  - cc2 视角 0 样本 (铁律1 不满足)
  - 非持续 (04:00 回升至 86.8%, 06:50 本轮 85.7%)
...
- dsv4p_nv SR=85.7% +6.8pp vs R642 (78.9%), +15.1pp vs R641 (70.6%), +24.2pp vs R639 (61.5%),
  +48.2pp vs R634-R637 连续 4 轮 37.5%,
  仍在 R617-R642 正常波动区间 (37-91%) 上沿, 趋势向好, 无恶化
- 6h SR 趋势 29-87% 波动, 04:00 回升 86.8%, 06:50 本轮 85.7% → 配额型故障模式确认
- all_tiers_exhausted ×3 (R624-R643: 3→...→6→5→5→5→5→5→5→5→4→3), -1 vs R642, 仍在正常波动区间 3-6, 无退化
- KeyManager 指数退避正确, 429 ABORT avg 2718ms (持平 R642 1925ms 量级)
...
- KeyManager 指数退避正确工作 (沿用 R642 日志铁证: 429 count decay >300s → reset → 180s)

## 下一步
- dsv4p_nv SR 85.7% (+6.8pp vs R642 78.9%), 仍在正常波动区间上沿, 配额持续恢复, 趋势向好, 无恶化
- **升级标注解除** (R621 设定 SR<55% 或 exhausted>=8 → 切 PRIMARY 回 glm5_2_nv):
  - 01:00 单小时触及 SR<55% (29.4%), 但非持续 (04:00 回升 86.8%, 06:50 本轮 85.7%), cc2 视角 0 样本 → 不触发
...
- nv_gw: NVU_DISABLE_MS_FALLBACK=0, UPSTREAM_TIMEOUT=90, TIER_COOLDOWN_S=180,
  ...
  NVU_TIER_BUDGET_GLM5_2_NV=120 (tier timeout), TIER_TIMEOUT_BUDGET_S=180
