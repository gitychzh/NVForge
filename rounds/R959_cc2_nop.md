# R959 cc2 NOP 巡检轮 — cc2 主链路连续第 67 轮 100% 干净

- 时间: 2026-08-07 ~12:14 CST
- 类辨: NOP (SR 100%, 专属错误 0, 无新错误类, 不改码)
- 容器: nv_gw Up 9h, cc4101 Up 8h, dsv4p_nv40066 Up 2d

## 判定依据 (live DB 30min 实拉 ~12:13 CST)

- **cc4101-primary (cc2 自己的请求, 主 nv_gw:40006)** = **122/122 = 100% SR, 0 bad**,
...
- **fallback (cc_requests 30min)** = **0 次** (122 req, fb=0, SR=100%)。
...
- **health**: nv_gw/cc4101/40066 全 ok; `nv_model_tiers`=[kimi_nv,dsv4p_nv,dsv4f_nv,dsv4f0731_nv,glm5_2_nv], `nv_default_model`=glm5_2_nv; cc4101 primary=dsv4f0731_nv。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **122/122 = 100% SR, 0 bad** (live 实拉) | ✅ |
| cc4101-primary 专属错误 | **0 rows** | ✅ |
| bad caller 归属 | 全 caller=hermes (502 all_tiers_exhausted ×2 + 502 zombie_empty_completion ×1) | ⚠️ 越界 |
| cc2 泄漏 | 0 条 (122 req 全 200) | ✅ |
| fallback (cc_requests) | 0 次 (122 req 全 200, SR=100%) | ✅ |
