...
- 脚本正确检测到自提交并标记 "不触发"
- cron 仍被派遣 — 误触发 (double-dispatch, R1189→R1190)
- R1190 为 R1133 僵尸链的第 58th NOP
...
## 3. 6h 窗口数据 (2026-07-11 ~10:00–16:10 UTC)
...
|------|-------|-----|------|--------|-----|--------|-----|
| glm5_2_nv | 24 | 12 | 12 | 12 | 0 | 6,685ms | 38,540ms |
| dsv4p_nv | 0 | — | — | — | — | — | — |
| kimi_nv | 0 | — | — | — | — | — | — |
| minimax_m3_nv | 0 | — | — | — | — | — | — |
...
- Gateway detection + error-chunk 正确: `[NV-ZOMBIE-ERROR-CHUNK] sent finish_reason=content_filter error SSE chunk`
- 每 30min 一次 zombie (规律性), input_chars 持续增长 (169K→174K)
- 日志: 无 ERROR/WARN (除 zombie detection 外)

### 3.5 24h 扩展数据
| 模型 | 24h Total | 24h OK | 24h Fail | 24h Zombie | 24h ATE |
|------|-----------|--------|----------|------------|---------|
| glm5_2_nv | 184 | 139 | 45 | 43 | 0 |
| dsv4p_nv | 31 | 26 | 5 | 0 | 5 |
| kimi_nv | 7 | 7 | 0 | 0 | 0 |
| minimax_m3_nv | 9 | 9 | 0 | 0 | 0 |

- dsv4p_nv 最后流量: 21h前 (2026-07-10 19:05 UTC), 之后 HM2 hermes 使用 hm4104→dsv4p_ms fallback
- dsv4p_nv 5 ATE: 4 个~61s (NVCFPexecTimeout pattern), 1 个 1.3s (early abort), 全在 24h 窗口早期
- 0 nv_tier_attempts (dsv4p_nv ATE 无 tier-level 记录, 符合 only-fail-attempts 写入规则)
...
| TIER_TIMEOUT_BUDGET_S | 198 | 覆盖 ms_gw fallback 180s + UPSTREAM 66, 充足 | ❌ 无需改 |
| NVU_TIER_BUDGET_DSV4P_NV | 72 | dsv4p_nv pexec ATE ~61s, 72 足够覆盖 UPSTREAM=66+margin | ❌ 无需改 |
| NVU_EMPTY_200_FASTBREAK | 2 | 0 empty_200 6h, dsv4p 0 traffic | ❌ 无需改 |
| TIER_COOLDOWN_S | 15 | 0 tier_attempts, 已足够 | ❌ 无需改 |
| NVU_PEER_FB_SKIP_MODELS | glm5_2_nv | 当前 zombie 为 glm5_2_nv integrate (NVCF content-filter, 非 function 不可用), 放开 peer-fb 无意义 | ❌ 无需改 |
| 所有其他参数 | floor/optimal | 已验证 20h+ 稳定 | ❌ 无需改 |

## 5. 决策

**NOP** — false trigger, 58th chain of R1133.
...
- 0 tier_attempts, 0 ATE 6h, 0 ms_gw traffic, 0 429
- Gateway detection + error-chunk 正确, zombie 机制工作正常
- dsv4p_nv/kimi_nv/minimax_m3_nv 零流量 6h (hermes 使用 hm4104→dsv4p_ms 回退链路)
- 24h dsv4p_nv 5 ATE 全在窗口早期 (20h+ ago), 近期 zero
