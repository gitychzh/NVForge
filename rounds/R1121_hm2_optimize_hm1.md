# HM2 Optimize HM1 — Round R1121

> **铁律**: 只改HM1不改HM2

## 1. 触发分析

R1120 为 NOP (double-dispatch of R1119, same commit 807a2b6)。脚本检测到 HM1 有新 commit (R1120 的 git push)，触发 HM2 执行本回合。HM1 本地 git log 仍停留在 R821 (~300 rounds behind) — 正常。
...
- 后重启窗口: ~9h
- Tier chain: `['dsv4p_nv'] (no fallback, 3model)` — FALLBACK_GRAPH={} 设计
...
### 2.3 Post-Restart (19:03 UTC→04:00 UTC, ~9h)
...
- dsv4p_nv: 4/4 (3 nvcf_pexec + 1 nv_integrate), avg dur ~9.5s
- glm5_2_nv: 4/4 (4 nv_integrate), avg dur ~8.5s
- 0 nv_tier_attempts
- 0 fallback_occurred
- 0 key cycling (all first-attempt successes)
...
- **zombie_empty_completion (9×)**: 代码级 zombie 检测 (R1107), 在 3-15s 内快速 abort 替代旧版 96s NVStream_TimeoutError hang。代码级特性, 不可配置修复。
- **all_tiers_exhausted (3×)**: dsv4p_nv single-tier ATEs (~61s), tiers_tried_count=1, fallback_actually_attempted=false。FALLBACK_GRAPH={} 设计下的正常行为。ms_gw BrokenPipeError (stream sync defect, 代码级)。
- **NVStream_TimeoutError (2×)**: glm5_2_nv integrate ~96s 超时, 远超 NVU_STREAM_FIRST_BYTE_DEADLINE_S=20 和 NVU_STREAM_TOTAL_DEADLINE_S=42 — deadline 未强制执行 (代码级缺陷)。
...
5. ms_gw 同样处于 optimal: EMPTY_200_FASTBREAK_THRESHOLD=3 (floor), 86% 6h SR
6. 无新错误类型, 无性能退化, 无绑定约束
7. 与 R1120 数据完全一致 (仅���增 2 个 glm5_2_nv integrate success, 04:03 UTC)
