# R942 cc2 NOP 巡检轮 (HM2 nv_gw 主链路)

> 轮次: R942 | 日期: 2026-08-07 CST | 类型: NOP (不改码)
> 结论: cc2 主链路连续 **50** 轮 100% 干净 (R892→R942);
...
- 30min 所有 bad (非 200) = `caller=hermes` ×3: `all_tiers_exhausted ×1` + `stream_absolute_cap ×1` + `zombie_empty_completion ×1`。
- **caller 列实拉铁证 (带 request_id)**: 3 bad 全 caller=hermes (81fdfa9d, 105fb55f, e4f8ea10), 0 个属于 cc2 主链 (host-separated)。
- zombie_empty_completion 保持 **1** (R941 首现 1, R942 未扩散 → 稳定, 越界 hermes 不构成 cc2 风险)。
- fallback (cc_requests 30min) = **0 次** (115 req, fb=0, SR=100.0)。
- buffer 日志: cc4101-primary 全 attempt=1 成功 (verdict=success_tool_call), 无 WAIT-/KEY- 错误, 450s 预算内。
- 容器 health: 4101/40006/40066 全 ok (200)。
- 容器 UP: nv_gw 8h (与 R941 持平, 平稳, 非重启回归), cc4101 7h (固定节奏)。
...
| hermes 线 bad (非 200) | all_tiers_exhausted ×1 + stream_absolute_cap ×1 + zombie_empty_completion ×1 | ⚠️ 越界 |
| bad caller 归属 | 3 req 全 caller=hermes; cc2 primary 0 条 | ✅ 隔离 |
| fallback (cc_requests) | 0 次 (SR=100.0%) | ✅ |
| zombie_empty_completion | 1 (R941 首现 → R942 未扩散) | ⚠️ 观察 |
...
- 30min 所有 bad 分组 (caller 列铁证带 request_id): 3 条全 hermes (81fdfa9d, 105fb55f, e4f8ea10), cc2 主链 0 bad。
- cc_requests fallback = 0 次。
- docker logs nv_gw buffer 段: cc4101-primary 全 attempt=1 success, 无错误噪音。

### 关键判断
cc2 主链路连续第 **50** 轮 (R892-R942) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
bad 请求 100% 属 hermes (caller 列实拉铁证带 request_id, 未进 cc2 主链), fallback 0 次, 无新 cc2 主链错误类。
zombie_empty_completion 未扩散 (保持 1, R941→R942), 且属 hermes 越界。本次 hermes bad 由 R941 的 4 回落至 3。
**不改码**: ①主链 SR 100% + 专属错误 0 行, 无优化需求; ②坏请求全属 hermes 越 cc2 范围;
③多 tier round-robin + func_health 健康选择已达稳态, buffer 全 1-attempt 成功。

> ⚠️ 观察项 (R942): nv_gw Up = 8h (与 R941 持平, 平稳无重启回归);
> 同机有 `nv_gw_stable Up 5d` 并存。zombie_empty_completion 保持单例未扩散,
