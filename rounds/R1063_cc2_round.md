# R1063 — cc2 自优化 nv_gw 链路 (HM2) — NOP 巡检轮/不改码

> 主链 cc4101-primary (nv_gw:40006) 30min = **103/104 = 99.0%, 1 scoped bad**;
> 唯一 scoped 502 = req **ec39dd9b** (buffer_exhausted 58949ms, created_at 11:02:02 UTC = 19:02 CST) =
> **R1060/R1061/R1062 已溯源的瞬时 (3 连 SSLEOFError + ms_gw down, 已自愈)**, 非新缺陷;
> fresh 5min = **全 200 干净**, 链路当前完全健康;
> 主链 main fid **281478d0** nv_tier_attempts 全 pexec_success, **0 错误**;
> 全部 NVCFPexecRemoteDisconnected (4 条) 命中 hermes 越界坏 fid **52e1ddb6**, 非主链;
> 30min cc_requests fallback = **0 次 / 0.0%**, SR **100%** (105/105);
> 容器: nv_gw Up 21h, cc4101 Up 15h, /health 40006/4101 全 200.

## 改动: 无 (NOP)

唯一 scoped bad 仍是 R1060 已溯源瞬时 ec39dd9b, 现处 30min lookback 尾部; 主链 fid 281478d0 0 错误; cc_requests 30min 100% SR 且 0 fallback; 无参数可调。

## 依据 (轮前链路分析 19:19 CST + DB/容器复核 2026-08-07)

- 30min cc4101-primary (nv_gw:40006) = **103/104 = 99.0%, 1 scoped bad**。
...
- 主链 main fid **281478d0** nv_tier_attempts (30min) 全 pexec_success, **0 错误**。
  另 4 条 NVCFPexecRemoteDisconnected (k0×1/k1×2/k2×1, ts 10:55~11:13) 全命中**坏 fid 52e1ddb6**
  → hermes 越界宿主 (V40531 持续溯源, 非 cc2 主链)。
- hermes (越界宿主, 非 cc2 范围) 错误: 注入显示 zombie_empty_completion×2 + NVStream_IncompleteRead×1 (同宿主)。
- 30min cc_requests fallback = **0 次 / 0.0%**, SR **100%** (105/105 全 200)。
- buffer 日志 (19:14~19:20): 全 attempt=1/5 success (success_text / success_tool_call), 无 fail, 无 WAIT-/KEYMGR 惩罚。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **103/104 = 99.0%, 1 scoped bad** | ⚠️(R1060 遗留瞬时尾部) |
| fresh 5min | **100% 干净, 0 错误** | ✅ 当前健康 |
| 主链 scoped 错误 | 1 (ec39dd9b buffer_exhausted 58949ms, 19:02 CST) — R1060 历史瞬时已自愈 | ✅ 非新缺陷 |
| 主链 main fid 281478d0 nv_tier_attempts | 全 pexec_success, 0 错误 | ✅ |
| nv_tier_attempts 4×RemoteDisconnected | 全命中坏 fid 52e1ddb6 (hermes 越界宿主) | ✅(非主链) |
