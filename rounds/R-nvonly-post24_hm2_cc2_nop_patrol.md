# R-nvonly-post24 — hm2_cc2 NOP 巡检轮

- 轮次: R-nvonly-post24
- 主机: HM2 (cc2)
- 时间: 2026-08-02 02:27 CST
- 类型: NOP 巡检轮 (0 改动, 0 重启)
- 上轮: R-nvonly-post23 (7 连庄)

## 本轮数据 (30min 窗口, caller=cc4101-primary 即 cc2)

| request_model | status | fallback_occurred | fallback_to | count | avg_ms |
|---------------|--------|-------------------|-------------|-------|--------|
| glm5_2_nv     | 200    | f                 |             | 1     | 4895   |
| glm5_2_nv     | 200    | t                 | ms_gw       | 1     | 166464 |

cc2 30min 2/2 = SR 100%.
- 1 次 glm5_2_nv 直接成功 (4895ms, 健康快路径).
- 1 次 nv 侧失败 → fallback 到 ms_gw 兜底成功 (166464ms 总耗时, 含 nv 失败 + ms 重试).
- ms_gw fallback 符合"fallback 已恢复"指令, 非 nv_gw 故障.

## 其他 caller (非 cc2 流量, 仅记录不优化)

| caller | model | SR |
|--------|-------|-----|
| hermes+openclaw+other | dsv4p_nv | 25/34 = 73.5% (9× all_tiers_exhausted + 4×429 + 3×502) |
| other | glm5_2_nv | 1/1 = 100% |

dsv4p_nv 73.5% 是 NVCF 侧限流 (key2 仅 1×429_nv_rate_limit, 主要为 all_tiers_exhausted 5key 全挂 avg 7326ms).
与 cc2 无关 (cc2 已切 glm5_2_nv 主链).
...
1 次 fallback �� ms_gw (glm5_2_ms), 符合指令.
...
| docker ps | cc4101 Up 23m, nv_gw Up 23m, nv_gw_stable Up 27m, ms_gw Up 2d, logs_db Up 2d ✓ |
