# HM2 Optimize HM1 — Round R1185

> **⚠️ R1185 (HM2→HM1): NOP (false trigger, 53rd chain of R1133, zombie-only, all params floor/optimal, NVCF content-filter not config-fixable)**

## 1. 触发分析

cron 脚本输出: `"这是我提交的, 不触发"`
- 最新 commit: `f449d27` — `opc2_uname` (HM2) — R1184 NOP
- GitHub HEAD: `f449d27 R1184: HM2→HM1 — NOP`
- **判定**: 误触发。最新 commit 为 HM2 自提交，非 HM1 新提交。
- R1184 已由预运行脚本提交，symlink 指向 R1184。Double-dispatch pattern。
...
| ms_gw | Up 35 hours (healthy) | 2026-07-10T00:01:24Z |
| logs_db | Up 7 days (healthy) | 2026-07-04T14:53:51Z |

compose md5: `7975939c245761e451a8813852dcb9bf` (unchanged since R1088 deploy)

## 3. 日志摘要

docker logs --tail 100: 仅 zombie_empty_completion (glm5_2_nv, NVCF content-filter stop+12chars, 170K-173K input, ~30min 间隔规律出现, key round-robin 正常). 零 ERROR/WARN/exception/429/NVCFPexecTimeout/ATE/PEER-FB/MS-FB.

所有 glm5_2_nv integrate 均在第一次尝试成功 (NV-INTEGRATE-SUCCESS on first attempt)，但 NVCF 返回 content-filter stop (12 chars)，gateway 正确检测为 zombie 并发送 error SSE chunk 触发 openclaw fallback。

## 4. DB 数据 (6h, ~2026-07-11 09:00 UTC)
...
|--------|-----------|-------------|-----|--------|
| 502 | zombie_empty_completion | glm5_2_nv | 12 | 4,852 |
...
|-------------|-------|-----|------|-----|
| glm5_2_nv | 188 | 144 | 44 | 76.6% |
| dsv4p_nv | 31 | 26 | 5 | 83.9% |
| minimax_m3_nv | 9 | 9 | 0 | 100% |
| kimi_nv | 7 | 7 | 0 | 100% |

24h 错误: zombie_empty_completion 41 + all_tiers_exhausted (dsv4p_nv, pre-restart NVCF 504) 5 + NVStream_TimeoutError 3.

### Post-restart 纯净窗口 (2026-07-10T19:03:27Z→now)
所有失败均为 zombie_empty_completion (glm5_2_nv, 12×). 零 ATE, 零 NVCFPexecTimeout, 零 429, 零 NVStream_TimeoutError, 零 tier_attempts.

ms_requests post-restart: 0 (zero ms_gw traffic).
...
- ms_gw: 0 traffic post-restart，无优化表面。
- compose md5 不变，容器 12h 无重启。
- dsv4p_nv 6h 零流量，24h 中 5 ATE 全部发生在 pre-restart (NVCF 504 外部，非 config 可修)。
- kimi_nv + minimax_m3_nv: 100% SR 24h。
- glm5_2_nv integrate: 所有请求第一次尝试成功，NVCF 返回 content-filter stop (12 chars)，gateway 正确检测为 zombie。code-level 行为，非 config 可修。
...
- NV_INTEGRATE_KEY_COOLDOWN_S=0: floor，零 429 integrate rate limit → 无回调理由
- 其余全部 floor 参数不可再降

**NOP** — 无参数可改，无优化表面。nv_gw 0 traffic 非 glm5_2_nv，ms_gw 0 traffic。所有失败 code-level zombie_empty_completion (NVCF content-filter)，无 config 修复路径。
