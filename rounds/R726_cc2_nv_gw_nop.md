# R726 (cc2/HM2): NOP 巡检轮

> 2026-08-03 20:45 CST | 容器: nv_gw Up 5h, cc4101 Up 5h, dsv4p_nv40066 Up 5h
...
- **cc2 (cc4101) 30min**: 16 req, 16×200 = 可见 SR **100.0%**
  ...
  - 唯一 fallback: primary glm5_2_nv 失败 → dsv4p 200 兜底
- **nv_gw 全量 30min**: dsv4p_nv 62×200 = SR **100.0%**, glm5_2_nv 0 req
- **错误分类 30min**: 0 错误 (无新错误类型)
- **tier attempts 30min**: 0 行 (glm5_2_nv 本窗口无 tier 流量, 全被 primary 失败后 fallback)
- **buffer/wait/keymanager 日志**: docker logs --since 30m = 0 行 (无触发)

### 6h 窗口 (更大小本)
- **glm5_2_nv**: 29×200/19×502 = SR **60.4%** (48 req 低流量)
  - tier: pexec_conn_RD×19, pexec_success×17, IntegrateRD×15, integrate_conn_RD×7, integrate_success×7, SSLEOF×4, pexec_500×1
  - 全 NVCF 上游连接断开/配额副作用, 非 nv_gw 可控
- **dsv4p_nv**: 405×200/18×429/17×502 = SR **92.0%** (440 req 高流量, 主力稳定)
- **cc4101 6h**: 16 req, 100% SR, fb 6.3%
- **错误无新类型**: pexec_conn_RD/IntegrateRD/SSLEOF/429/pexec_500 全已知 NVCF 上游副作用
