# R1943 (HM2→HM1): NOP — false trigger, 0 new data, 0 config-fixable
...
All 9 zombies: glm5_2_nv big_input (131K-147K chars), NVCF function-level empty200 degradation.

### 延迟 (成功请求)
| mapped_model | total | avg_ms | min_ms | max_ms |
|--------------|-------|--------|--------|--------|
| glm5_2_nv | 24 | 10937 | 2333 | 27809 |
| dsv4p_nv | 2 | 30487 | 17893 | 43081 |

### Breaker 状态
- BIG_INPUT_FAIL_N=1, COOLDOWN=21600 (6h)
- Container up ~1h (restarted ~22:30 UTC) → breaker reset → 1 leak zombie at 23:03 before re-OPEN
- Post-breaker: all subsequent big_input → peer-fb → 100% success (6 ATE status=200 in 2h)
- Live state: breaker OPEN, peer-fallback handling all big_input

### Peer-Fallback 性能
| 时间 | dur_s | 结果 |
|------|-------|------|
| 23:33:42 | 10.6 | 200 OK |
| 23:33:31 | 11.3 | 200 OK |
| 23:04:11 | 17.8 | 200 OK |
| 23:03:53 | 15.8 | 200 OK |
| 15:33:42 | 10.6 | 200 OK |
| 15:33:31 | 10.4 | 200 OK |
| ... | ... | 100% success |
...
| 2. SR断崖 | ❌ | 74.3% SR 是 NVCF 侧 glm5_2 大输入退化, breaker 已正确拦截 |
...
breaker 机制 (FAIL_N=1, COOLDOWN=21600) 正确运行: 第1个 zombie 触发 breaker OPEN → 后续 big_input 请求直走 peer-fallback → 全成功 (6/6, 100%)。容器重启导致 breaker 重置 → 1个 zombie 泄漏是不可避免的 (in-memory state)。所有参数已在 floor, 无进一步紧缩空间。NVCF glm5_2 大输入退化是服务端问题, 非 HM1 配置可修复。

R1940→R1941→R1942→R1943 连续四轮 NOP: glm5_2 big_input zombie 模式稳定, breaker+peer-fb 全路径100%成功。等待 NVCF 恢复后 breaker 自然 CLOSED。
