# R908 — cc2 NOP 巡检轮 (main 100% clean, sub round 17)

> 2026-08-07 | HM2 cc2 → HM2 nv_gw:40006 (primary=dsv4f0731_nv)
> **判定: NOP, 不改码.** cc2 主链路连续第 17 轮 100% SR 干净 (R892-R908).
> 坏请求 3 条 100% 属 hermes caller (JOIN 铁证, 越 cc2 范围); bad fid 52e1ddb6 0 泄漏进 cc2 主链.

## 改动: 无 (NOP)

cc4101-primary 主链 30min 实拉 = **132/132 = 100% SR, 0 bad**。
bad fid 52e1ddb6 在候选池 ×1-5 (全 0 success RemoteDisconnected/empty_200/529/Timeout, 未被选中) → 0 泄漏进 cc2 主链。

## 依据 (live DB 实拉 2026-08-07 ~09:1x CST)

- **30min cc4101-primary** (主 nv_gw:40006): `WHERE caller='cc4101-primary' AND status!=200` → **0 条**, 132/132 全 200 (100% SR)。
- **30min 所有 bad** (JOIN 铁证): `stream_absolute_cap ×2` + `all_tiers_exhausted ×1` = 3 条, **全 caller=hermes** (502), 非 cc2。
- **per-key (nv_tier_attempts)**: 5 key 全走健康 fid **281478d0** ×26-27 = 100% pexec_success, 0 错误。
  bad fid 52e1ddb6 在候选池 key0 ×1-4 (0 success, 0 选中) → 0 泄漏 (func_health 正确规避)。
- **buffer (cc4101-primary)**: 全 attempt=1/5 成交 (上轮 1-13s 复盘 success), 0 重试 / 0 429 / 0 cooldown。
- **fallback (cc2 线)**: 0 次。
- **容器 health**: 4101 (primary=dsv4f0731_nv) / 40006 (5 keys) / 40066 — 全 ok (200)。nv_gw Up 6h, cc4101 Up 5h。
...
| bad fid 52e1ddb6 泄漏 | cc2 primary JOIN 0 条 (候选池 key0 ×1-4 未被选中) | ✅ 隔离 |
...
- per-key 主链全 281478d0 健康 0 error; 52e1ddb6 ×1-5 候选池全失败 0 选中。

## 关键判断
cc2 主链路连续第 17 轮 (R892-R908) 100% SR 干净。bad 请求 100% 属 hermes caller 活动,
