# R800 — cc2 NOP 巡检 (R799 后置续净, tier 连续 3 轮零错历史最长, 2026-08-05)
...
- ok=1015 / total=1024, fb=9 (0.88%), SR=99.1% (含 499), **排 499 = 100%**
- 9 err **全 499 client_gone_mid_stream** (用户中途断开, real_err=0, 非 nv 链路问题)
- 9 fb 全 200 < 10% 目标 ✅

### 2. glm5_2_nv tier 连续 3 轮零错 (历史最长净窗口)
- 30min 68 attempts **全 pexec_success**, error_type 列只 `pexec_success` 一项
- per-key 均布: k0:18 k1:13 k2:14 k3:12 k4:11
- 对比: R798=77 attempts 0 错, R799=72 attempts 0 错, **R800=68 attempts 0 错**
- 注入数据里 16 RemoteDisc 是 09:39 注入窗口早期抖动, 后续净窗口稀释至当前 0

### 3. buffer 全 attempt=1 success
- 注入摘要: 无 buffer/wait/keymanager 日志 (5key 全活, buffer 一次过)

### 4. 1× nv_gw 502 死请求 (历史延续, 非新错误)
- f15fe5ef dur=400589ms buffer_exhausted = R796 wait_queue 180 验证案例本身
- 已记 R797/R798/R799 (4 轮延续), cc_requests 表无对应 request_id = 未穿透用户视角
- 当前实时 nv_requests 30min caller=cc4101-primary 仅 200 (无 502)

### 5. 容器健康
- nv_gw: status=ok, Up 29min (R796 09:10 up -d 基线续跑)
- cc4101: status=ok, Up 8 hours

## 判稳结论

- cc4101 SR 排 499 = 100% ≥ 99% → NOP 巡检轮 ✅
