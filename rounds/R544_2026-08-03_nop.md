# R544 — NOP 巡检轮 (2026-08-03 08:00 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R543 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本, 铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 8 req SR=37.5% (3×200 + 5×429), 全 `hermes` caller.
  SR 较 R543 的 44.4% 回落 7pp, 但样本仅 8 req, 仍落在 R530-R543 标注的 NVCF 配额波动
  区间 (40-65% band) 低端, 非 nv_gw 侧新故障.
- 唯一错误类型 `all_tiers_exhausted` × 5 (avg 1304ms ≈ 1.3s, 较 R543 的 2132ms 更快 —
  KeyManager 全局冷却在 tier 层前拦截, tier_attempts 0 行, 历史一致).
- NV-GLOBAL-COOLDOWN tier=dsv4p_nv 周期性 429 仍在 (23:35/23:40/23:45/23:50/23:55 UTC),
  与 R268-R543 完全一致, NVCF 侧配额波动.
...
- dsv4p_nv 8 req: 3×200 (k2=3) + 5×429 (每 5min GLOBAL-COOLDOWN 180s)
- 唯一错误类型: `all_tiers_exhausted` × 5, avg_dur=1304ms (全 NVCF 配额型, 非 nv_gw 故障)
- per-key (dsv4p): k2=3×200 (9950ms avg dur), 其余 key 全 429
- per-egress-IP: 203.10.96.139=3×100% SR, 其余 0 success
- 200 finish_reason: tool_calls × 2, stop × 1 (无 zombie)
- fallback_occurred=f × 8 (nv_gw 层 dsv4p_nv 不 fallback, cc4101 层兜底)
- 全局冷却模式: 每 5min 1 次 NV-GLOBAL-COOLDOWN tier=dsv4p_nv all keys 429, 与 R268-R543 一致
