# R1044 cc2 NOP — cc2 primary 100% clean (107/107), scoped errors 0; bad 3 hermes (zombie_empty_completion×2/NVStream_IncompleteRead×1); fallback 0 (0/105); 152nd clean round (R893-R1044)

日期: 2026-08-07 CST
性质: NOP 巡检轮 (不改码)

## 结论
cc2 主链路 (caller=cc4101-primary) 连续第 **152** 轮 100% SR 干净 (R893-R1044)。
...
## 数据 (live 复核 2026-08-07 18:0x CST + 注入轮前链路分析 18:02 CST)
...
- 30min 按模型总 SR dsv4f0731_nv = 98.3% (170/173), 差异 3 条 bad 全属 hermes。
- 30min cc_requests = **0 次 fallback (0.0%)**, 主链 105/105 全 200 (live total=105, ok=105, sr=100.0)。
- nv_tier_attempts per-key: 非成功 **1 row** (k2 NVCFPexecRemoteDisconnected ×1 次, 瞬态不累计),
  其余 k0-k4 全 pexec_success (24/20/20/19/21)。与历史偶发模式一致, 非持久 key 疲劳。
...
- 容器: /health 40006/4101/40066 全 200 (nv_gw 40006 passthrough nv_default_model=glm5_2_nv 5 keys,
  cc4101 primary=dsv4f0731_nv, dsv4p_nv40066 200)。nv_gw Up 15h, cc4101 Up 14h, dsv4p_nv40066 Up 2d。

## 关键判断
连续 **152** 轮稳态。主链 107/107 全 200, 专属错误 0 行, fallback 0 (0.0%), buffer 零重试。
本轮 window 内仍混入 hermes 越界 3 bad (zombie×2 + NVStream×1), 主链无根因可查; 参数处于稳态, 无可调。
tier 层 k2 偶发 RemoteDisconnected ×1 (与上轮 R1040-R1043 一致的瞬态模式, key 恢复正常后无续错),
