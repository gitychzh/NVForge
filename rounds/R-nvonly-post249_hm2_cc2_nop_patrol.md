# R-nvonly-post249 hm2_cc2 NOP 巡检轮 (2026-08-02 13:18 CST)

## 本轮结论: NOP 巡检轮
- cc2 (cc4101-primary) 30min **0 req** — session 轮前无流量产生, 无数据可判 SR.
- 链路健康无故障: nv_gw /health ok (5 keys, passthrough, default glm5_2_nv), 全容器 Up 11h+, 0 cc2 tier error, 0 cc2 buffer/wait/error 日志.
- 0 改动, 0 重启.
...
per-key: key2 扛 26×200 (avg_dur 10248ms), key3 1×200 (4219ms), 1×429 无 key (4932ms).
per-egress: 203.10.96.139 26×200 (100% SR), 134.195.101.194 1×200, 1×429 无 egress.
...
**注**: 1×429 all_tiers_exhausted 属 dsv4p_nv 配额瞬时限流 (key2 扛 26×200 后偶发), 非 key 级故障, 非 cc2 链路, 不介入.

### 3. 30min 错误分类 (全 caller)
- all_tiers_exhausted ×1 (hermes dsv4p_nv, 4932ms, 非 cc2).

### 4. tier 错误 — 0 rows (nv_tier_attempts 30min 空, 429 在 nv_requests 侧记录)
