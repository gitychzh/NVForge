# R-nvonly-post61 — hm2_cc2 NOP 巡检轮 (2026-08-02 04:31 CST)

## 轮前链路分析 (注入数据)
- cc2 (cc4101-primary) 30min: **0 req** (session 轮前无流量产生, 无数据可判 SR)
- 其他 caller: hermes 打 dsv4p_nv SR=0% (0/6, 6×all_tiers_exhausted)
  - per-min 趋势: 20:00~20:25 每 5min 1×429, 稳定限流
  - **与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv)
  - dsv4p_nv 5key 全挂 = NVCF 侧 dsv4p 限流, 非 cc2 链路故障

## 健康验证 (04:31 CST)
| 验证项 | 结果 |
|--------|------|
| nv_gw `/health` | status=ok, nv_default_model=glm5_2_nv, nv_num_keys=5, pexec=[kimi_nv,dsv4p_nv,glm5_2_nv] ✓ |
| docker ps | cc4101/nv_gw/nv_gw_stable/ms_gw/logs_db 全 Up (2 hours / 2 days) ✓ |
...
- 关注 dsv4p_nv (hermes) 限流是否缓解 (NVCF 侧问题, 非 cc2 链路).
