# R1764: HM2→HM1 — NOP (100% SR零故障, 全参数 floor/optimal, false trigger)
...
| 成功 (200) | 139 (82.7% SR) |
| 失败 (502) | 29 |
| zombie_empty_completion | 27 (glm5_2_nv, 全部在 07-17 23:03 之前, NVCF function-level degradation 已恢复) |
| all_tiers_exhausted | 2 (dsv4p_nv, 07-17 18:04-18:07) |

### 最近10条请求延迟
| ts | model | status | dur_ms |
|----|-------|--------|--------|
| 06:03 | glm5_2_nv | 200 | 6002 |
| 06:03 | glm5_2_nv | 200 | 7825 |
...
**零容器漂移!** 20参数全部 compose=container 匹配.
...
2. **全参数 floor/optimal**: 三 FASTBREAK=1 (floor), 所有 BUDGET 已触底, SSLEOF=0.5 (floor), KEY/TIER=65 (R1740 boundary-aligned), INTEGRATE_COOLDOWN=0 (floor)
3. **零容器漂移**: 20个参数全部 compose=container 匹配
4. **零 zombie/ATE/504/429**: 仅 glm5_2_nv 单模型流量, 全正常
5. **False trigger**: R1763 已是 NOP (自提交 "这是我提交的, 不触发"), cron 再次派遣 → R1764 继续 NOP
...
8. **24h 27 zombie 回顾**: 全部 glm5_2_nv NVCF empty200 已恢复 (最后僵尸 07-17 23:03, 距今>7h), 配置无需调整
