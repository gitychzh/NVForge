# R1027 cc2 NOP inspection round

- 日期: 2026-08-07
- 类型: NOP 巡检轮 (不改码, 只记数据)
- 判定: 主链 SR=100% + 专属错误 0 rows + fallback 0% → 无优化需求, 不改码

## 数据摘要 (live 复核 2026-08-07 + 注入轮前链 16:58 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **104/104 = 100% SR, 0 bad** (live `SELECT caller,status,count GROUP BY` → 全 200)。
- 主链专属错误 (caller=cc4101-primary, status!=200) = **0 rows** (scoped 错误分组为空)。
- 30min nv_requests 总 bad = **5 条 (全部 502), 全属 hermes** (越界宿主), 主链 0:
  - hermes|all_tiers_exhausted|x2
  - hermes|NVStream_IncompleteRead|x1
  - hermes|stream_absolute_cap|x1
  - hermes|zombie_empty_completion|x1
- 30min fallback (cc_requests) = **0/104 = 0.0%** (fallback_triggered=0)。
...
| 指标 | 值 | 达标 |
|---|---|---|
| 主链 SR (nv_requests) | 104/104 = 100% | ✅ (≥99%) |
| 主链专属错误 | 0 rows | ✅ |
| nv_requests 总 bad | 5 条, 全属 hermes (越界) | ✅ (主链 0) |
| fallback 触发率 | 0.0% (0/104) | ✅ (<5%) |
| 容器健康 | nv_gw/cc4101/40066 全 Up, /health 200 | ✅ |

## 结论
cc2 主链路连续第 **135 轮** (R893-R1027) 100% SR 干净。本轮 5 条 bad (502) 归属全属 hermes 越界宿主
