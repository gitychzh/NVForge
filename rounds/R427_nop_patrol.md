# R427 — NOP 巡检轮 (2026-08-03 01:10 CST)

## 摘要
- NOP 巡检轮, 0 改动 0 restart. cc2 (cc4101-primary) 30min 2 req 全 200 (session 间歇空闲).
- DB 快照 (01:06): dsv4p_nv 全 caller 30min SR=54.5% (6/11).
  - cc2 primary 2×200 (avg 3414ms, ttfb 3542, max 3549, min 3279), finish stop×2.
  - hermes caller: 3×200 + 5×429 (16:40-17:00 限速窗口), 5×all_tiers_exhausted (avg 1903ms, mapped-tier 直接失败).
  - per-key: k0×1/k1×1/k2×3/k3×1 = 6×200; 5×429 无 key 归属.
  - per-egress-IP: 7 req 空 IP, 203.10.96.139×3, 134.195.101.194×1.
  - 分钟趋势: 16:40-17:00 5×429 限速 → 17:05/17:06 恢复 4×200.
- 30min fallback: f×11 (0 fallback 发生).
- cc2 (cc4101-primary) 30min: 2×200, 0 fail, 100% SR — 链路健康.
...
- 错误类型无新增, 与 R268-R426 一致 (一百四十九轮一致).
...
- dsv4p_nv 本轮快照 SR=54.5% (6/11), 较 R426 (25.0%) 回升, 仍在 NVCF function 配额波动区间
  (R420=86.4%, R421=76.5%, R422=76.5%, R423=61.5%, R424=33.3%, R425=25.0%, R426=25.0%, R427=54.5%).
- dsv4p 错误类型无新增, 与 R268-R426 一致 (一百四十九轮一致).
- 切换 PRIMARY_UPSTREAM_MODEL 到 glm5_2_nv 是大改: cc2 缓冲 caller 2 req + glm5_2_nv 30min 0 req,
  无 buffer 路径数据支撑, 不满足"改前必有数据"铁律 → 暂不切.

## 根因 (沿用 R278-R426, 非代码缺陷)
