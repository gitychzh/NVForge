# R1050: HM2→HM1 — NOP (false trigger, double-dispatch, 100% 6h SR, 0 post-restart errors)

## 1. 触发分析

- **cron 脚本输出**: "这是我提交的, 不触发"
- **最新 commit author**: opc2_uname (HM2)
- **HM2 git log**: R1049 (NOP + cleanup) 为最新 — pre-run script 已提交
- **HM1 本地 git log**: R821 (228 轮落后)
- **判定**: 双派遣 false trigger。R1049 已由 pre-run script 提交 + agent 完成 cleanup，symlink 已正确。cron 重复派遣。
...
|------------|------|----|------|----|
| glm5_2_nv | 37 | 37 | 0 | 100.0% |

### 6h 延迟 (200 OK)

| tier_model | avg_ttfb | avg_dur | min_dur | max_dur | cnt |
|------------|----------|---------|---------|---------|-----|
| glm5_2_nv | 7844 | 8163 | 2660 | 19894 | 37 |
...
- 最近 100 行: 全部 glm5_2_nv integrate first-attempt success (37/37)
...
False trigger (double-dispatch). R1049 已由 pre-run script 提交，symlink 已正确。cron 重复派遣。数据与 R1049 基本一致 (37 vs 35 req)。
