# R1180: HM2→HM1 — NOP (false trigger, 48th chain of R1133, zombie-only, all params floor/optimal, NVCF content-filter not config-fixable)
...
- 脚本正确检测到自提交并标记 "不触发"
- cron 仍被派遣 — 误触发 (48th chain of R1133)
- R1179 已是最新回合
...
- compose md5: 7975939c245761e451a8813852dcb9bf (unchanged from R1179/R1178/R1177)
- health: OK

### 2.2 6h 总体
- 26 req / 11 OK (42.3%) / 15 zombie
- All upstream: nv_integrate (glm5_2_nv only)
- dsv4p_nv: 0 traffic 6h
- All errors: zombie_empty_completion (15)
...
### 2.5 Tier Attempts
- 0 rows (no key-level failures in 6h window)

### 2.6 ms_gw
- 0 nv-initiated traffic (no ms_gw fallback triggered)
- 0 requests in ms_requests table

### 2.7 僵尸日志分析
- All: NVCF content-filter stop+12chars, 169K-172K input, no tool_calls → gateway sends error SSE chunk → openclaw falls back to next model
...
- Pattern perfectly identical to R1179
...
### 3.1 故障模式
- NVCF glm5.2 function 对 openclaw 的超长上下文请求（169K-172K input chars）返回 content_filter stop + 12 chars
- 这完全在 NVCF 侧，配置不可修复
...
### 3.3 决策
NOP — 无参数可优化。所有参数在 floor/optimal，zombie 是 NVCF content-filter（不可配置），dsv4p_nv 无流量，ms_gw 无 nv 回退流量。数据与 R1179 完全相同。
