# R418 — NOP 巡检轮 (2026-08-03 00:40 CST)

## 数据快照 (注入, 30min 窗口 ~16:05-16:30)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 连续第 10 轮).
- dsv4p_nv 全 caller 30min SR=95.3% (41/43) — 与 R417 持平.
  - 41×200 全 key2 + egress 203.10.96.139, avg 12569ms (ttfb 12011, max 33184, min 5153),
    finish tool_calls×35 + stop×6, 无 IP 归属 fail.
  - 2×all_tiers_exhausted (avg 18605ms, mapped-tier 直接失败, 无 key/IP 归属).
  - 1×429 (16:30, 限速模式) + 1×502 (16:09, 恢复期偶发).
  - 30min fallback: f×43 (0 fallback 发生).
  - 分钟趋势: 16:05-16:26 连续 22min 出 41×200, 中间 16:09 1×502, 16:30 1×429.
...
- dsv4p_nv SR=95.3% (41/43), 较 R415 持续回升, 仍在 NVCF function 配额波动区间, 非代码缺陷.
- 错误类型无新增, 与 R268-R417 一致 (**一百四十二轮一致**).
- 0 fallback, 0 deadline 链问题.
- /health ok, nv_gw/cc4101/logs_db 容器健康.

## 根因 (沿用 R278-R417 分析, 非代码缺陷)
...
## dsv4p_nv SR 趋势 (近 8 轮 30min 快照, 全非缓冲 caller, cc2 0 req)
- R409 25.0% → R410 37.5% → R411 68.4% → R412 82.8% → R413 88.2% → R414 91.4% → R416 95.3% → R417 95.3% → **R418 95.3% (41/43)** — 持平高位, 逼近 99% 目标.
