# HM2 Optimize HM1 — Round R1053

## 触发类型

HM1 提交了 R1052 (NOP commit)，脚本检测到新 commit 后触发 HM2→HM1 优化轮次。

## 数据收集 (改前必有数据)

### 6h DB
- 39req/39OK(100.0%)/0fail
- All requests: nv_integrate (glm5_2_nv only)
- avg TTFB: 9161ms, avg duration: 9463ms, max: 39617ms
...
### nv_gw 日志 (tail 200)
- 37 integrate successes on first attempt
- 2 SSLEOFErrors on k2 (proxy 7895)
  - Both recovered by SSL-CYCLE to k3 in ~4s
  - SSLEOFError at 5002ms and 5002ms — consistent k2 mihomo proxy issue
- 零 NV-TIER-FAIL, 零 NV-EMPTY-FASTBREAK, 零 NV-TIMEOUT-FASTBREAK
- 零 NV-MS-FB, 零 NV-PEER-FB, 零 ATE
- 100% first-attempt integrate success rate (37/37 first-attempt, 2/2 SSL-CYCLE rescue)
...
理由:
1. 6h 100% SR (39/39), 零错误, 零 ATE — 系统已完美稳定
2. nv_tier_attempts 0 rows — 无任何 key 级失败
3. 全部参数已在 optimal/floor 状态:
   - FASTBREAK 全部=1 (floor), NVU_EMPTY_200_FASTBREAK=2
