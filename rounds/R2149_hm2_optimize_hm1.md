...
## Analysis
- 14th consecutive round with 0 ATE — cooldown compression is safe
- R2145 (KEY 58→56), R2146 (TIER 40→38), R2148 (KEY 56→54) → R2149 hits TIER turn
- KEY+TIER=54+36=90 < TIER_TIMEOUT_BUDGET=153 (63s headroom) ✓
...
- 36 > 24s UPSTREAM safe margin ✓
- Low traffic (6.8 req/h) with 5 keys → inter-request gap >> TIER_COOLDOWN_S
...
| Param | Old | New | Δ |
|-------|-----|-----|---|
| TIER_COOLDOWN_S | 38 | 36 | -2s |

## Verification
- `docker exec nv_gw env | grep TIER_COOLDOWN_S` → `TIER_COOLDOWN_S=36` ✓
