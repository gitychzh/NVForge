# R634: HM2 nv_gw 自优化回合

## 数据收集 (HM2 nv_gw 40006, 06:00-06:15 UTC 窗口)

### 30min cc2 (cc4101-primary)
- 0 req (session 间歇空闲, 同 R626-R633, 无 cc2 评估样本)

### 30min hermes/dsv4p_nv (live 实测)
- 8 req: 3×200 + 5×429, SR=37.5%
- vs R633 64.7% / R632 73.7% / R631 76.2% / R630 80.8% / R629 86.7%
  → **本轮续降, 仍在 R617-R633 正常波动区间下沿 (37-91%), 小样本 8req 波动剧烈, 趋势未恶化**
- per-key: k2=3×200 全 first-attempt 命中 (k2 主力不变)
- 429 nv_key_idx=NULL → KeyManager 层 ABORT (hermes caller 不走 buffer, 同 R626-R633)
...
- `all_tiers_exhausted` ×5 (avg_dur 1339ms = 429 快速 ABORT)
  - R624→...→R633→R634: 3→3→3→3→3→4→5→5→5→6→5 (持平正常区间 3-6, 本轮 -1)
...
| 05:00 | 30 | 39 | 76.9% |
| 06:00 | 3 | 6 | 50.0% |
→ 波动 37-87%, 模式不变: NVCF 账户级配额耗尽 (429 无 retry-after 头, 全 5key 429 → TIER_COOLDOWN 180s), 非 nv_gw 侧可改

### KeyManager 日志摘要 (30min)
- 全 5key 429 → NV-GLOBAL-COOLDOWN tier=dsv4p_nv 180s (TIER_COOLDOWN)
- count decayed (>300s) 后 count=1 重置, 累积到 count=3 cooldown=480s (指数退避正确)
- 429 resp headers: ratelimit/retry=(none) — NVCF 不提供 retry-after, 配额型 429 确认
- egress 单 IP 203.10.96.139 (mihomo-7902) 主力不变

### deadline 链
- cc_requests stream_total_deadline 6h=0 (deadline 链健康, 90s×5=450s buffer < 470s cc4101 < 500s SDK)

### 容器 + 配置
- nv_gw Up 24h (/health ok, nv_num_keys=5, nv_default_model=glm5_2_nv)
- cc4101 Up 14h, ms_gw Up 4d, logs_db Up 4d, nv_gw_stable Up 36h
- 配置无漂移 (env 全项匹配 R633 快照)

## 持续观察点 (沿用 R633, 不改码)
- **dsv4p_nv SR 6h 波动 37-87%**: NVCF 账户级配额型故障, 非 nv_gw 侧可改
- 01:00 单小时 SR=36.8% 触及 R621 升级阈值 (SR<55%), 但:
  - cc2 视角 0 样本 (铁律1 不满足)
  - 非持续 (04:00 回升至 86.8%)
  - → 不触发回切 PRIMARY glm5_2_nv
- KeyManager 指数退避正确 (429 count=1→180s, =2→180s, =3→480s)
- 全 5key 429 → TIER_COOLDOWN 180s 全挂, hermes caller 不走 buffer, 被 cc4101 fallback ms_gw 兜住
...
- 容器状态: nv_gw Up 24h (/health ok, 5 keys), cc4101 Up 14h, ms_gw/logs_db/nv_gw_stable 长稳
- 配置无漂移 (env 全项匹配 R633 快照)
