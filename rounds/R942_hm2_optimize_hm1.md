# HM2 Optimize HM1 — Round R942

**Timestamp**: 2026-07-09 08:10 UTC  
**Trigger**: 误触发 (false trigger, 59th consecutive)  
**Commit**: b994321 (R941: HM2→HM1 — NOP, 58th consecutive)  
...
- R941 已由 pre-run script 提交为 NOP，symlink 已更新指向 R941
- 本次为双派遣 (double-dispatch) — cron 在 pre-run script 已处理完毕后再次派遣 agent
- HM1 本地 git log 仍停留在 R821 (121 轮落后) — 无新提交
...
与 R906–R941 模式一致：24h 内恰好 1 次 identical ATE (同时间窗，NVCF 上游瞬时事件，FALLBACK_GRAPH 消失)。
...
⚠️ Tier attempts 总失败数 (17) 远超实际 ATE 数 (1)。原因：tier_attempts 记录的是尝试级别失败（含 empty_200 fastbreak、504 gateway timeout 等），但成功 fallback 到另一 tier 后最终请求仍返回 200，不进入 ATE 计数。`empty_200` 已被 FASTBREAK 拦截，`504_nv_gateway_timeout` 被 HEALTH_THRESHOLD 触发 tier 降温后由另一 tier 成功处理。

### 容器日志 (最近 50 行)
全部为 `[NV-SUCCESS] tier=glm5_2_nv kX succeeded on first attempt`，零 error/warn/traceback。运行干净。
...
**对比 R941**: 数据完全一致 (39/39 100% SR, identical ATE pattern, all params at floor)。无退化，无优化机会。
