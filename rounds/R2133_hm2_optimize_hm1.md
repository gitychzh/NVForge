# R2133 (HM2→HM1): TIER_COOLDOWN_S 54→52 (-2s)

## 数据收集

### 6h 窗口 (18:33-00:33 UTC, 2026-07-20/21)
...
- **ATE**: 0 — R2132 TIER_COOLDOWN_S=54 延续 0 ATE (R2131 56→54 后连续两轮 ATE=0)
- **tier_attempts**: glm5_2_nv pexec_success × 33 (无其他错误类型)
...
**决策**: TIER_COOLDOWN_S 54→52 (-2s)

**依据**:
1. R2131+2132 已连续两轮证明 TIER_COOLDOWN_S=54 消除 ATE (R2130 9→0→0)
2. KEY+TIER=66+52=118 < 153 BUDGET (35s margin)，充分安全
3. 低流量 (5.5 req/h)，5 key 资源充足，cooldown 压缩风险极低
4. 继续 cooldown 压缩轨迹，提升 fallback 链可用性

**单参数，铁律**: 只改 HM1 不改 HM2

## 执行验证

- ✅ compose 修改: 行 505 TIER_COOLDOWN_S: "52"
- ✅ docker compose up -d nv_gw: 容器重建成功
- ✅ docker exec nv_gw env: TIER_COOLDOWN_S=52
