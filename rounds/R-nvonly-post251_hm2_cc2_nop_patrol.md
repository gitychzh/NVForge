# R-nvonly-post251 hm2_cc2 NOP patrol

**时间**: 2026-08-02 13:21 CST
**轮次**: R-nvonly-post251 (hm2_cc2)
**类型**: NOP 巡检轮
**改动**: 0  **重启**: 0

## 判稳依据

cc2 (cc4101-primary) 30min 窗口 0 req — session 轮前无 cc2 流量产生, 无数据可判 SR.
链路健康无故障: nv_gw /health ok (passthrough, 5 keys, default glm5_2_nv), 全容器 Up 11h+.
0 cc2 tier error, 0 cc2 buffer/wait/error 日志. **无数据不动手 (铁律 1)**.

## 30min 数据

### cc4101-primary (cc2) — 0 req
无流量, 链路健康.

### 其他 caller (hermes/openclaw, 非 cc2 链路)
| caller | model | 200 | 429 | SR |
|--------|--------|-----|-----|-----|
| hermes | dsv4p_nv | 23 | 2 | 92.0% (25req) |
| openclaw | dsv4p_nv | 1 | 0 | 100% (1req) |

dsv4p_nv 整体 SR=92.3% (26req): 24×200 + 2×429 (all_tiers_exhausted, avg 3494ms).
per-key: key2 23×200 (avg 10478ms), key3 1×200 (4219ms), 2×429 无 key.
per-egress: 203.10.96.139 23×200 (100%), 134.195.101.194 1×200.
finish_reason: tool_calls×19, stop×5 (无 zombie).
fallback 发生率: 0/26 (无 fallback).
按分钟: 04:55-05:11 稳定 200, 05:15 / 05:20 各 1×429.

**429 根因 (nv_gw 日志 13:20:29)**:
```
[NV-KEYMGR] 429 tier=dsv4p_nv k3 count=4 cooldown=180s
[NV-KEYMGR] 429 tier=dsv4p_nv k4 count=2 cooldown=180s
[NV-KEYMGR] 429 tier=dsv4p_nv k5 count=2 cooldown=180s
[NV-GLOBAL-COOLDOWN] tier=dsv4p_nv all keys 429. Marking all cooling 180s
[NV-ALL-TIERS-FAIL] All 1 tiers failed (ring tiers tried: ['dsv4p_nv']), elapsed=2054ms, ABORT-NO-FALLBACK
```
→ dsv4p_nv 配额瞬时限流 (k3/k4/k5 同时 429), 触发 TIER_COOLDOWN 180s + 全 tier fail abort.
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv). 不介入.

### 错误分类 (全 caller 30min)
- all_tiers_exhausted ×2 (hermes dsv4p_nv, avg 3494ms, 非 cc2).

### tier 错误 / buffer/wait 日志 (cc2) — 空

## 健康验证 (13:21 CST)
| 项 | 结果 |
|----|------|
| nv_gw /health | ok, passthrough, 5 keys, default glm5_2_nv ✓ |
| docker ps | nv_gw/cc4101/nv_gw_stable Up 11h, ms_gw/logs_db Up 3d ✓ |
| cc2 (cc4101-primary) 30min SR | 0 rows (无流量, 链路健康) ✓ |
| 30min tier error (全 caller) | all_tiers_exhausted ×2 (hermes dsv4p_nv, 非 cc2) ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复) ✓ |

## 参数快照 (无变化, 同 post249)
- nv_gw: NVU_DISABLE_MS_FALLBACK=0, BUFFER_MAX_RETRIES=5, BUFFER_TIMEOUT_STAIRS=90×5, BUFFER_TOTAL_DEADLINE=450s, TIER_TIMEOUT_BUDGET=180s, TIER_COOLDOWN_S=180s, UPSTREAM_TIMEOUT=90s, KEY_COOLDOWN_S=30, NV_INTEGRATE_KEY_COOLDOWN_S=90, MIN_OUTBOUND_INTERVAL_S=10, NVU_FORCE_STREAM_UPGRADE_TIMEOUT=150, NVU_FORCE_STREAM_UPGRADE=0, NVU_BUFFER_CALLERS=cc4101-primary,openclaw2, NVU_PEER_FB_SKIP_MODELS=glm5_2_nv,dsv4p_nv, NVU_CALLER_KEY_MAP=hermes:2;openclaw:3;opencode:4
- cc4101: CC4101_STREAM_TOTAL_DEADLINE_S=470, PRIMARY_HEADER_TIMEOUT=400, UPSTREAM_TIMEOUT=130, UPSTREAM_IDLE_TIMEOUT=150, CC4101_PRIMARY_SKIP_S=30, CC4101_PRIMARY_FAIL_THRESHOLD=3, FALLBACK_UPSTREAM_URL=ms_gw:40007, PRIMARY_UPSTREAM_URL=nv_gw:40006, PRIMARY_UPSTREAM_MODEL=glm5_2_nv, FALLBACK_UPSTREAM_MODEL=glm5_2_ms

## 下一步
继续 NOP 巡检. 等 cc2 流量产生后再判 SR. dsv4p_nv 配额限流属 hermes/openclaw 链路瞬态, 若扩散到 glm5_2_nv 再介入.
