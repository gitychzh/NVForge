...
**日期**: 2026-07-22 (HM2)
**轮号**: R2112_hm2_oc2 (上一轮 R2111_hm2_oc2)
**动作**: 0 改动 0 restart. 连续第 60 轮 NOP 冻结.
**STATE 滞后修正**: 第 19 次 (STATE.md 停 R2105, 主仓 openclaw2 上轮 R2111_hm2_oc2 已在主仓但 STATE.md 未覆写, 本轮 cat+git log 双确认 R2111→R2112 对齐).
...
| 指标 | R2111 | R2112 | Δ |
|------|-------|-------|---|
| 6h SR | 98.49% (782/794) | **98.46%** (763/775) | -0.03pp 持平 golden |
| 6h 错误 | 9z+2IR+1cap | 9z+2IR+1cap | 同构全良性背景波 |
| 6h ATE | 0 | **0** | 保持干净 |
| 6h 499 | 0 | **0** | 持续健康 |
| 6h 真中断 | 1 (other cap) | **1** (other cap) | 持平 非旋钮 |
| dsv4p 6h SR | 90.12% (374/415) | **90.89%** (389/428) | +0.77pp 续回升 |
...
- 6h 真中断 1 = other 域 stream_absolute_cap (nv+ms 都挂 → 上游 NVCF 瞬时非旋钮能修, 与 R2111 同模式)

### glm5_2_nv 30min (caller 维度)

```
cc4101-primary 21×200                            — R2145/R2149 修复路径稳定
other          28×200                            — R2145/R2149 修复路径稳定
unknown         1×200 + 1×502(zombie)            — 良性背景波
```
- 30min glm5_2_nv: 51 总, 50 成功 = 98.0%, **0 ATE**
- caller cc4101-primary + other 全 glm5_2_nv (R2145/R2149 修复零退化, 全 200)
- 30min 全错 3 = dsv4p 2 ATE (unknown caller default=dsv4p 路径) + glm5_2 1 zombie (unknown 良性背景波)

### fallback + 499

- fallback 30min: cc4101 5 次 (FALLBACK-OK 救回, 0 真中断: 上游瞬时), opclaw4103 1 次
- 6h 499 = 0 (cc2 R2199 全局 settings env 改后 openclaw2 域健康持续, R2149 锁定 model=glm5_2_nv 后零退化)

### dsv4p_nv (非本域, 参考)

- 6h 389/428 = 90.89% (R2105 67.86% → R2106 82.0% → R2107 83.3% → R2109 88.3% → R2110 89.95% → R2111 90.12% → 本轮 90.89% 持续回升)
...
StartedAt=2026-07-21T23:56:40Z  RestartCount=0  (连续第 30 轮 RC=0)
```
env 无漂移 (与 R2111 逐行一致). StartedAt 23:56:40Z = compose up -d 重建 (非 restart, 非 RC, R2108 已记录).

## 主仓对齐检查

- 主仓最新: R2230 (HM1 域 KEY_COOLDOWN_S 30→28, 非本域 铁律只改 HM2)
- openclaw2 上轮: R2111_hm2_oc2 (主仓已提交, STATE.md 停 R2105 滞后)
- cc2 最新: R2228_hm2_cc2 (R2192 task1 落地补记 + 巡检, 非 nv_gw 旋钮改动 不撞本域旋钮)
- HM1 peer 最新: R2230 (KEY 30→28, 持续交替 KEY→TIER→KEY, 非 openclaw2 域)
- 本轮 R2112_hm2_oc2, 上一轮 = R2111

## 归因结论

**冻结继续 — openclaw2 不该动.** 五重佐证:

1. **glm5_2_nv 6h 98.46%** (763/775) 持平 R2111 98.49% / R2110 98.27% / R2109 98.40% / R2108 98.49% golden 区多轮.
2. **glm5_2_nv 30min 0 ATE** (50/51 全 200 除 1 zombie 背景波) — 自愈保持, 稳定.
3. **6h 0 ATE** (9z+2IR+1cap 全良性背景波, 无 all_tiers_exhausted) — 路径干净.
4. **R2145/R2149 修复零退化**: caller cc4101-primary 21 + other 28 30min 全 glm5_2_nv 全 200.
5. **fallback 30min 6 全救回 0 真中断** (cc4101 5 + opclaw4103 1, 0 真中断); 6h 499=0; env 无漂移 StartedAt 23:56:40Z 连续第 30 轮 RC=0.

真中断 1 (6h other 域 stream_absolute_cap, nv+ms 都挂 → 上游 NVCF 瞬时非旋钮能修).
fallback 30min 6 全救回 0 真中断: 上游瞬时.
6h 499=0: cc2 R2199 全局 settings env 改后 openclaw2 域健康持续 (R2149 锁定 model=glm5_2_nv 后零退化).
dsv4p_nv 6h 90.89% 是 NVCF 端 function 74f02205 坏 (自愈中), 非 nv_gw 旋钮能修, 不影响 glm5_2_nv 路径. 等 NVCF 自愈.

## 改动

无. 0 env 改, 0 源码改, 0 restart. 连续第 60 轮 NOP.
...
2. **拉 30min + 6h + caller 维度**: 重点检验:
   - glm5_2_nv 6h SR 是否 > 97% 持续 (本轮 98.46% golden)?
...
连续 60 NOP, glm5_2_nv 6h 98.46% golden 持平, 0 ATE 0 499, fallback 全救回 0 真中断, env 无漂移. openclaw2 冗余巡检无 cc2 未覆盖可改点, 不动. HM2 only.
