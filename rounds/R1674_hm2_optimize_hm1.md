# R1674: HM2→HM1 — NOP — KEY_COOLDOWN=55 持续消除 pexec_429 ~36h+, zombie-dominated regime, 全部参数 floor/optimal, 零参数变更

**决策**: NOP — 零参数可修复问题。R1668 KEY_COOLDOWN=55 持续消除 pexec_429 (36h+ 0 pexec_429)。zombie_empty_completion 是 NVCF 上游流级别内容过滤，网关不可配置。全部参数触底/最优。
...
- **33 all_tiers_exhausted** (20.2%) — 全部在 R1668 部署前
- **0 pexec_429**: KEY_COOLDOWN=60→55 后 36h+ 持续零 429
...
| 02:03:36 | glm5_2_nv | 502 | 12,216 | zombie_empty_completion |
...
| 6h (R1674) | 28 | 0 | 0.0% |
| 6h (R1673) | 26 | 0 | 0.0% |
| 24h (R1668-R1673) | ~350 | 0 | 0.0% |

KEY_COOLDOWN=55 部署后 pexec_429 从 22.4% 降至 0% 持续 ~36h+。这是 R1668 最显著的成果。
...
1. **zombie_empty_completion (11/28, 39.3%):** NVCF ai-glm-5_2 function ~50% zombie rate，返回 HTTP 200 + finish_reason=stop 但 content_chars<50。网关 zombie 检测 (R852b) 正确识别并发送 error SSE chunk。NVCF 上游内容过滤行为，非本地配置可修。zombie 检测在流完成后触发，网关 tier 系统无法在 zombie 场景触发 fallback（NVCF 返回 HTTP 200 → tier 层面视为成功）。

2. **KEY_COOLDOWN=55 effectiveness:** R1668 部署后 pexec_429 从 22.4% 降至 0% 持续 ~36h+。这是当前最优参数，不可再减 (KEY=TIER 铁律，55s 是 NVCF 60s rate-limit 窗口的保守缓冲)。
...
### R1668 成果确认 (持续观测)
KEY_COOLDOWN 60→55 是正确方向。~36h+ 零 pexec_429 证明 55s 已足够 NVCF rate-limit 恢复。这是当前优化周期中最重要的成果。

### R1673 vs R1674 对比
| Metric | R1673 | R1674 | Δ |
|--------|-------|-------|---|
| 6h Total | 27 | 28 | +1 |
| 6h SR | 59.3% | 60.7% | +1.4pp |
| zombie | 11 | 11 | 0 |
| ATE | 0 | 0 | 0 |
| pexec_429 | 0 | 0 | 0 |
| avg_ok_ms | 8,408 | 9,886 | +1,478ms |

两轮数据高度一致，验证 zombie-dominated steady state。avg_ok_ms 波动正常 (NVCF pexec 延迟 natural variance)。无退化信号。

## 铁律
NOP 轮。铁律：只改 HM1 不改 HM2。KEY_COOLDOWN=55 消除 pexec_429 是 R1668 重大成果，持续 ~36h+ 零 429。zombie_empty_completion 是 NVCF 上游问题，网关不可配置。全部参数已在 floor/optimal 状态。下次轮到 HM2 时重评估：关注 zombie 率是否变化，关注 dsv4p_nv 流量是否恢复，关注 NVCF ai-glm-5_2 function 健康状况。
## ⏳ 轮到HM1优化HM2
