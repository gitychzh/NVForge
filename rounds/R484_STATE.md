# R484 — NOP 巡检轮 (2026-08-03 04:35 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 19:55-20:20 UTC (04:23 CST 注入, 04:35 复测确认), 与 R475-R483 同低谷窗口高度一致 (连续 10 轮锁定 14.3%).
- cc2 (cc4101-primary) 30min 0 req (psql 复测确认 0 rows, cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=14.3% (1/7, 1×200 + 6×429), 与 R483 完全持平, 仍处低谷窗口.
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R268-R483 (175+ 轮) 一致, 无新错误.
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截, 空 IP, 与 R470-R483 一致).
- fallback 7 次 (dsv4p 全挂走 ms_gw 兜底, 链路有保障).
- stream_total_deadline 6h 0 次 (deadline 链对齐稳).
- per-min 趋势: 每 5min 1 次离散 429 (03:55/04:00/04:05/04:11/04:15/04:20), 20:10 唯一一次 200 (dur=7760ms, tool_calls finish), 非集中爆发.
- nv_gw Up 14h, cc4101 Up 4h, nv_gw_stable Up 26h, ms_gw Up 3 days (本轮未重启).
- 配置实测确认与 R475-R483 完全一致, 无漂移.

## 链路数据 (04:23 CST 注入, 04:35 复测确认)
...
- nv_tier_attempts: 0 行 (429 未进入 tier 尝试, 空 IP, 与 R470-R483 一致)
- fallback: 7 次 (dsv4p 全挂 → cc4101 走 ms_gw glm5_2_ms, 兜底正常)
- per-min: 03:55|429, 04:00|429, 04:05|429, 04:10|200, 04:11|429, 04:15|429, 04:20|429 (离散, 非爆发)

### cc4101-primary 专属 (cc2 的请求)
- 30min 0 req (psql 04:35 复测 0 rows 确认, cc2 session 间歇空闲)

### 6h dsv4p_nv 全 caller SR (R483 基线 04:30)
- 200=130, 429=53, 502=4, 共 187 → SR=69.5% (R482=71.9% 略降, 仍处 46-85% 历史波动区间)
- 502 6h 4 次 (与 R476/R480/R481/R482/R483 一致, 低频)

### nv_gw COOLDOWN 日志 (04:35 复测, 30min 无 buffer/wait 日志)
- [NV-GLOBAL-COOLDOWN] tier=dsv4p_nv all keys 429. Marking all cooling 180s (TIER_COOLDOWN)
  时序: 03:55, 04:00, 04:05, 04:11, 04:15, 04:20 (每 5min 一次, 离散非爆发)
...
- 这是历史一致行为 (R268 起 175+ 轮), 非本轮新故障.

## 历史波动区间 (R437-R484)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473-R481=0.0% → R482=14.3% → R483=14.3% → R484=14.3% (30min 低谷, 6h 视角 69.5%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 模式与 R268-R483 一致 (175+ 轮), 无新错误.
- dsv4p_nv 30min SR=14.3% (与 R483 持平) 仍是低谷窗口, 6h SR=69.5% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
- fallback 7 次 (ms_gw 兜底正常, dsv4p_nv 自恢复不够时 cc4101 层有兜底, 链路有保障).
- stream_total_deadline 6h 0 次 (deadline 链对齐稳, 铁律4 满足).
- 0 restart → 无需 py_compile / curl 复测 (仍做 /health + docker ps 确认).
- 配置实测与 R475-R483 完全一致, 无配置漂移.

## 容器健康 (本轮实测 04:35)
- curl /health: status=ok, nv_num_keys=5, nvcf_pexec_models=[kimi_nv,dsv4p_nv,glm5_2_nv], nv_model_tiers=[kimi_nv,dsv4p_nv,glm5_2_nv], nv_default_model=glm5_2_nv, port=40006.
...
- 留意 502 是否再现 (R476/R480/R481/R482/R483 记 6h 4×502 低频, 再现 >=3/h 才介入).
