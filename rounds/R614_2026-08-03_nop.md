# R614 — NOP 巡检轮 (2026-08-03 13:04 CST)

## 基线 (R614 实测, 04:31-04:57 CST 窗口)
- cc2 (cc4101-primary) 30min: **0 req** (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv (hermes caller) 30min: 26 req, 23×200 + 3×502 → SR=88.5%
  - vs R613 91.3% / R612 85.0% / R611-R610 82.4% / R609 90.9% → **NVCF 配���波动区间内**, 非新低
  - per-key: k2 命中 23×200 (avg_dur 10612ms, max 30745ms, min 4166ms, ttfb 10209ms) + 1×502 IncompleteRead 36373ms; 空 key 2×502 all_tiers_exhausted
  - per-egress: 203.10.96.139 24 req (96ms avg route) 健康; 空 egress 2×502
  - finish_reason: tool_calls×20 + stop×3 (健康, 无 zombie)
  - fallback_occurred=f ×26 (cc4101 层 ms_gw 兜底, 预期)
- 错误分类 2 类 (均非新错误):
  - `all_tiers_exhausted` ×2 (avg_dur 19659ms, sub=all_tiers_failed_in_mapped_tier, 04:41:14 + 04:57:34)
    → 全 key cooling 时 ABORT-NO-FALLBACK 快速返回, dsv4p_nv 跳 peer fb (NVU_PEER_FB_SKIP_MODELS 含 dsv4p_nv)
...
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, NVU_BUFFER_CALLERS=cc4101-primary,openclaw2)
...
- dsv4p_nv 88.5% SR 在 R605-R613 历史波动区间 (82-91%), 非新低, 与 NVCF 配额波动模式一致
- KeyManager 行为正确: 全挂时 ABORT-NO-FALLBACK (avg_dur 19659ms) = dsv4p_nv 跳 peer fb 快速 abort
- NVStream_IncompleteRead ×1: 30min 内单次瞬态 (04:47:03), 与 R612/R613 同模式, 非模式性
- all_tiers_exhausted ×2 (19659ms avg): 比 R613 ×1 (5011ms) 多 1 次且 avg 偏高, 但 2 样本不足判模式升级
- 无新错误类型, 无参数漂移 → 无介入必要
...
## 下一步
- 继续 NOP 巡检, 等 cc2 流量恢复后观察 dsv4p_nv buffer 路径行为 (hermes caller 不走 buffer)
