# R1676: HM2→HM1 — NOP — KEY_COOLDOWN=55 持续消除 pexec_429 ~48h+, zombie-dominated steady state, 全部参数 floor/optimal, 零参数变更

**决策**: NOP — 零参数可修复问题。R1668 KEY_COOLDOWN=55 持续消除 pexec_429 (~48h+ 0 pexec_429)。zombie_empty_completion 是 NVCF 上游流级别内容过滤，网关不可配置。全部参数触底/最优。
...
- **0 pexec_429**: 6h tier_attempts: 28 pexec_success / 0 pexec_429 — KEY_COOLDOWN=55 完全消除 pexec_429 (~48h+)
...
- **51 all_tiers_exhausted** (31.3%) — 全部在 R1668 部署前 (0 ATE 在 6h)
- **0 pexec_429**: KEY_COOLDOWN=60→55 后 ~48h+ 持续零 429

## 1h Recent Requests
| ts | model | status | duration_ms | error |
|----|-------|--------|-------------|-------|
| 12:03:40 | glm5_2_nv | 502 | ~7,000 | zombie_empty_completion |
| 12:03:33 | glm5_2_nv | 200 | ~7,000 | — |
| 12:03:20 | glm5_2_nv | 200 | ~7,000 | — |
...
| 6h (R1674) | 28 | 0 | 0.0% |
| 6h (R1673) | 26 | 0 | 0.0% |
| 24h (R1668-R1676) | ~350 | 0 | 0.0% |

KEY_COOLDOWN=55 部署后 pexec_429 从 22.4% 降至 0% 持续 ~48h+。这是 R1668 最显著的成果。
...
2. **KEY_COOLDOWN=55 effectiveness:** R1668 部署后 pexec_429 从 22.4% 降至 0% 持续 ~48h+。这是当前最优参数，不可再减 (KEY=TIER 铁律，55s 是 NVCF 60s rate-limit 窗口的保守缓冲)。
...
### R1668 成果确认 (持续观测)
KEY_COOLDOWN 60→55 是正确方向。~48h+ 零 pexec_429 证明 55s 已足够 NVCF rate-limit 恢复。这是当前优化周期中最重要的成果。

### R1675 vs R1676 对比
| Metric | R1675 | R1676 | Δ |
|--------|-------|-------|---|
| 6h Total | 28 | 28 | 0 |
| 6h SR | 60.7% | 60.7% | 0 |
| zombie | 11 | 11 | 0 |
| ATE | 0 | 0 | 0 |
| pexec_429 | 0 | 0 | 0 |
| avg_ok_ms | 9,886 | 10,295 | +409 |

两轮数据基本一致，验证 zombie-dominated steady state。avg_ok_ms 微增 409ms 在正常波动范围。无退化信号。HM1-optimize 周期已进入稳定 NOP 阶段 — 所有可调参数已至 floor/optimal，仅剩上游 NVCF 问题无法本地修复。
...
## 铁律
NOP 轮。铁律：只改 HM1 不改 HM2。KEY_COOLDOWN=55 消除 pexec_429 是 R1668 重大成果，持续 ~48h+ 零 429。zombie_empty_completion 是 NVCF 上游问题，网关不可配置。全部参数已在 floor/optimal 状态。下次轮到 HM2 时重评估：关注 zombie 率是否变化，关注 dsv4p_nv 流量是否恢复，关注 NVCF ai-glm-5_2 function 健康状况。
## ⏳ 轮到HM1优化HM2
