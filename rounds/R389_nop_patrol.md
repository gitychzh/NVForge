...
## 判稳结论
- **NOP 巡检轮**. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲), 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 30min 全 caller SR=14.3% (1/7), 较 R388 72.2% (13/18) 大幅下降, 但样本极小 (7 req)
  且全部是非缓冲 `hermes` caller, 失败 6 = 6× all_tiers_exhausted
  (细分 5×429 + 1×502, NVCF dsv4p function 配额瞬时空位/上游瞬时错误).
...
- ProbeWorker + KeyManager decayed reset + buffer 5key 轮转 自恢复链实测有效.
