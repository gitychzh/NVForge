...
## 根因分析 (与 R623 一致, 数据铁证)
按分钟趋势铁证: 05:11/05:16/05:21 各 1×429 → 05:26-05:36 一波 14×200 (k2 first attempt)。
NVCF 配额是周期性间歇 (5min 周期), 非永久挂; 全挂 180s 后 k2 先恢复, 连续命中。
...
- dsv4p_nv SR=82.4% 连续 3 轮反弹 (R621 57.1% → R622 64.3% → R623 76.5% → R624 82.4%),
  已回到 R618 波动区间上沿, all_tiers_exhausted 同步下降 (6→5→4→3)
  → **证实 R621 "非恶化, 是周期性配额耗尽 + 采样窗口偏移" 判断正确, 趋势已反转**
...
- KeyManager 指数退避正确, ABORT 路径快速 (avg 1540ms vs R623 1731ms↓, 持续下降无退化)
- 容器健康, 配置无漂移
- 无介入必要 (码改)
...
## 下一步
- dsv4p_nv SR 已从 R621 低点 57.1% 连续 3 轮反弹至 82.4%, all_tiers_exhausted 同步下降 (6→3),
  趋势已反转, 跌破 60% 的非线性恶化担忧已解除
- **升级标注解除** (R621 设定, R623 已降级, R624 持续未触发): SR<55% 或 exhausted>=8 → 考虑切 PRIMARY 回 glm5_2_nv
