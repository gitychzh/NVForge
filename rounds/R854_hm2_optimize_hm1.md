# R854: HM2→HM1 — NOP (33/33 100% 6h SR, zero ATE, zero tier_attempts, peak health sustained, identical to R853)

**Date**: 2026-07-08 12:35 UTC
...
| 指标 | 值 | 说明 |
|------|-----|------|
| 24h total | 219req/121OK(55.3%)/98ATE | pre-R845 污染 (metrics gap 8h) |
| single-tier ATE | 71 (avg 24.9s) | pre-R845 代码缺陷, 已修复 |
| double-tier ATE | 27 (avg 141.3s) | NVCF 双函数耗尽, 非配置可修复 |
...
glm5_2_nv 400_nvcf_degraded (56) 为 DEGRADED 期间, 已自愈. dsv4p_nv NVCFPexecTimeout max=51,227ms << UPSTREAM=66, buffer=14.8s ≥3s → 非绑定.
...
**9 小时连续 100% SR** (19:00 UTC → 04:00+ UTC). 17:00-18:00 的 ATE 来自 pre-R845 metrics gap 期间 (metrics gap 8h 从 02:33–03:33 UTC 触发, 数据盲区导致 fallback 不可用). R845 (04:12 UTC 部署) 修复后系统持续健康.
...
func_health.py HEALTH_THRESHOLD = 0.1 (docker exec 验证), rr_counter = {nv_dsv4p:2145, nv_kimi:13, nv_glm5_2:4} (R846 独立性确认).
...
- R846: rr_counter glm5_2_nv 独立 counter — nv_glm5_2:4, nv_dsv4p:2145 未推进, 独立性确认
