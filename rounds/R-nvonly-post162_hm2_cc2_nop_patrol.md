# R-nvonly-post162 — hm2 cc2 NOP 巡检轮

## 元数据
- 轮号: R-nvonly-post162
- 主机: HM2 (100.109.57.26, opc2_uname)
- 时间: 2026-08-02 09:21 CST
- 类型: NOP 巡检轮 (无流量, 链路健康, 0 改动 0 重启)
- 上轮: R-nvonly-post161 (7h 前容器重启)

## 决策依据 (改前数据)
本轮 30min 窗口 cc2 (cc4101-primary) **0 req** — session 轮前无流量产生, 无数据可判 cc2 SR.
链路健康无故障, 配置正确, 无需改码.

### 30min 链路总览 (caller × model × status)
| caller | model | status | count |
|--------|-------|--------|-------|
| hermes | dsv4p_nv | 429 | 6 |
| openclaw | dsv4p_nv | 200 | 2 |

### cc2 (cc4101-primary) 30min — 0 req
无流量. glm5_2_nv 连续 post100-post161 (62 轮) 无故障扩散.

### 其他 caller (非 cc2 链路)
- hermes → dsv4p_nv: 6×429 (all_tiers_exhausted, NVCF 侧 dsv4p 配额限流, 5min 周期)
- openclaw → dsv4p_nv: 2×200 (avg_dur 6091ms, 链路本身可用, 佐证 429 是配额限流非链路挂)
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).

### 30min 错误分类
| error_type | sub | count | avg_dur |
|------------|-----|-------|---------|
| all_tiers_exhausted | all_tiers_failed_in_mapped_tier | 6 | 1406 |

全部 6× 是 hermes→dsv4p_nv 的 NVCF 配额限流.

### tier 错误 / buffer/wait 日志
- cc2 tier error: 0
- buffer/wait/keymanager 日志: 空

## 健康验证 (09:21 CST)
| 验证项 | 结果 |
|--------|------|
| nv_gw /health | ok, passthrough, 5 keys, default glm5_2_nv ✓ |
| docker ps | nv_gw/cc4101/nv_gw_stable Up 7h, ms_gw/logs_db Up 2d ✓ |
| cc2 (cc4101-primary) 30min SR | 0 rows (无流量, 链路健康无故障) ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复) ✓ |

## 本轮改动
0 改动, 0 重启, 0 代码变更. NOP 巡检轮.

## 下一步
- 继续监控 cc2 (glm5_2_nv) 链路 SR, 等 cc2 有流量时验证 99%+ 目标.
- dsv4p_nv (hermes/openclaw) 配额限流非 cc2 链路, 不在本轮优化范围.

## 参数快照 (2026-08-02 09:21 CST)
- nv_gw: NVU_DISABLE_MS_FALLBACK=0, BUFFER_MAX_RETRIES=5, BUFFER_TIMEOUT_STAIRS=90×5, BUFFER_TOTAL_DEADLINE=450s, TIER_TIMEOUT_BUDGET=180s, UPSTREAM_TIMEOUT=90s, KEY_COOLDOWN_S=30, NV_INTEGRATE_KEY_COOLDOWN_S=90, MIN_OUTBOUND_INTERVAL_S=10, TIER_COOLDOWN_S=180, NVU_FORCE_STREAM_UPGRADE_TIMEOUT=150, NVU_FORCE_STREAM_UPGRADE=0, NVU_BUFFER_CALLERS=cc4101-primary,openclaw2, NVU_PEER_FB_SKIP_MODELS=glm5_2_nv,dsv4p_nv, NVU_CALLER_KEY_MAP=hermes:2;openclaw:3;opencode:4
- cc4101: CC4101_STREAM_TOTAL_DEADLINE_S=470, PRIMARY_HEADER_TIMEOUT=400, UPSTREAM_TIMEOUT=130, UPSTREAM_IDLE_TIMEOUT=150, CC4101_PRIMARY_SKIP_S=30, CC4101_PRIMARY_FAIL_THRESHOLD=3, FALLBACK_UPSTREAM_URL=http://ms_gw:40007/v1/chat/completions, PRIMARY_UPSTREAM_URL=http://nv_gw:40006/v1/messages, PRIMARY_UPSTREAM_MODEL=glm5_2_nv, FALLBACK_UPSTREAM_MODEL=glm5_2_ms
