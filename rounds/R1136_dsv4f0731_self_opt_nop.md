# R1136: dsv4f0731_nv self-opt NOP

**日期**: 2026-08-08 04:04 UTC
...
## 数据（30min 窗口，脚本 04:04 UTC）
- **总量**: 201 | **成功**: 201 | **错误**: 0 | **fallback**: 0 → **SR = 100%**
- **延迟**: avg=9467ms, p50=8201ms, p95=28594ms
- **错误分类**: (无)
- **per-key 200 延迟**（全部健康，负载均衡，无劣化 key）:
  - key0: 41 req, avg=10249ms
  - key1: 40 req, avg=8290ms
  - key2: 40 req, avg=10031ms
  - key3: 38 req, avg=7978ms
  - key4: 42 req, avg=10633ms
- **upstream_type**: nvcf_pexec 201/201 OK (100%)，无 integrate 流量
- **finish_reason**: tool_calls=177, stop=24
- **429 计数**: 0
- **key_cycle_429s**: key0=85, key1=115（历史累积计数，非本窗口错误；本窗口 0 次 429）
- **tier_attempts**: (无)
...
- nv_gw: Up 25 hours
- /health: status ok, 5 keys, 5 model tiers

## 结论
30min SR=100%，零错误、零 fallback、零 429，per-key 延迟/负载高度均衡（各 38~42 req，7.9~10.6s，无劣化 key），all upstream=pexec 100% 成功，6h SR=99.5%。链路完全健康。

**决策：NOP，不改任何参数。** 100% SR 下修改参数违反"改前必有数据"铁律。

## 上次修改效果 (R1135 → R1136)
R1135 (04:02) 报 SR=100% (201/201)，avg=9577ms。本轮 (04:04) SR=100% (201/201)，avg=9467ms（-110ms，噪声内）。数据窗口高度重叠，系统延续稳定态，无退化。

## 下一步建议
链路上游持续健康。继续观察 30min 窗口：
- 若 stream_first_byte_timeout 死链重新聚集（≥3/30min 或单 key 集中）→ 评估 handlers.py socket.timeout→continue 源码修复
