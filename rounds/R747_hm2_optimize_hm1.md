# R747: HM2→HM1 — ⏸️ NOP. All 12 params at optimal/floor; 102 ATE NVCF dual-function exhaustion + dead glm5_2_nv primary (health=0.0). R719+R744 confirmed. Zero-change.

## 6h Data (2026-07-05 ~13:45–19:45 UTC)

### Overall
- 337 req / 236 OK / 101 ATE → **70.0% SR**

### Per-Model
| model | total | ok | ate | SR |
|-------|-------|-----|-----|-----|
| dsv4p_nv | 231 | 138 | 93 | **59.7%** |
| glm5_2_nv | 104 | 97 | 7 | **93.3%** |
| kimi_nv | 2 | 1 | 1 | 50.0% |

### ATE Breakdown
- tiers_tried_count=1: **23** (22.8%) — avg 64,417ms, all `fallback_actually_attempted=f`. 22 via dsv4p_nv (avg 67,229ms), 1 via start_tier_idx=0 (2,551ms). All blocked by glm5_2_nv health=0.0 < FALLBACK_HEALTH_THRESHOLD=0.10.
- tiers_tried_count=2: **78** (77.2%) — avg 100,505ms, both tiers genuinely exhausted (NVCF dual-function failure).

### NVCFPexecTimeout
| tier | cnt | avg_ms | max_ms |
|------|-----|--------|--------|
| dsv4p_nv | 46 | 40,555 | **59,596** |
| glm5_2_nv | 59 | 47,460 | 57,797 |

dsv4p_nv max=59,596ms → UPSTREAM=64 (+4.4s overhead, NOT binding). Uniform across 5 keys: [8,8,12,9,9] → function-level timeout, not key-specific. glm5_2_nv: [8,12,11,15,13] → also uniform, function-level.

### Upstream Path
- nvcf_pexec: 236 req, 236 OK (100%) — all successful requests via pexec
- NULL (ATE): 101 req, 0 OK — all ATE with no upstream_type

### Success Duration Buckets (dsv4p_nv, no fallback)
| bucket | cnt |
|--------|-----|
| <5s | 4 |
| 5-10s | 8 |
...
- glm5_2_nv→dsv4p_nv fallback: **working** — confirmed by logs showing glm5_2_nv→NV-FALLBACK→dsv4p_nv→NV-FALLBACK-SUCCESS
- `[NV-PEER-FB] peer-originated request (hop=1)` — peer fallback only for hop=1, not local ATEs (R744 confirmed)
- All 101 ATE `fallback_actually_attempted=false` — no fallback rescue attempted

### Container & Env Verification
- Container: nv_gw, started 10:09 UTC (~9.6h), MIN_SAMPLES expired
...
All params compose↔env verified consistent. No drift. R746→R747: SR 69.6%→70.0% (within noise, +0.4pp).
...
| NVU_SSLEOF_RETRY_DELAY_S | 1.0 | No SSLEOF errors. | **REJECT** |

## Decision: ⏸️ NOP (ZERO-CHANGE)

**Root cause summary:** The 101 ATEs (30.0%) are caused by two NVCF upstream issues, neither config-fixable:

1. **dsv4p_nv→glm5_2_nv fallback killed (23 single-tier ATEs, 22.8%)**: glm5_2_nv primary function `3b9748d8` is dead (health=0.0). FALLBACK_HEALTH_THRESHOLD=0.10 correctly excludes it. The auto-switch function `f966661c` (health=1.0) works for glm5_2_nv's own tier_chain (93.3% SR), but the auto-switch is NOT propagated to fallback target health checks (R719 code-level defect). dsv4p_nv→glm5_2_nv fallback remains broken until either `3b9748d8` recovers or the code is patched.

2. **NVCF dual-function exhaustion (78 double-tier ATEs, 77.2%)**: Both dsv4p_nv and glm5_2_nv independently exhausted all 5 keys. NVCFPexecTimeout uniformly distributed across keys → function-level bottleneck, not key-specific. UPSTREAM_TIMEOUT not binding (+4.4s headroom). No config parameter can reduce NVCF function-level timeout rates.
...
- R746: Confirmed same pattern. SR 69.6%→70.0% (marginal improvement, within noise).

**Regime status:** The system is in a "NVCF upstream outage" regime where no config optimization can improve SR. All 78 double-tier ATEs and 23 single-tier ATEs are NVCF function-level failures. Waiting for NVCF recovery is the only path forward. The 93.3% SR on glm5_2_nv (auto-switched to healthy `f966661c`) demonstrates that the gateway config is optimal — when NVCF functions are healthy, performance is excellent.
