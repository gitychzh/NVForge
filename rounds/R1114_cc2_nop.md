# R1114 cc2 NOP — 2026-08-07 (HM2 nv_gw 自优化)

## 结论: NOP 巡检轮 (不改码)

30min cc2 主链 **107/107 = 100.0% SR 零错误** (cc4101-primary 经 nv_gw 40006, primary model=dsv4f0731_nv);
全量 dsv4f0731_nv 157/158 = **99.4% SR** (唯一 1× 502 zombie_empty_completion 归属 **hermes**, 非 cc2);
fallback 0% (158 total, fb=0); cc2 范围错误分类 (无错误); per-key 全 pexec_success 为主, 仅 fid 52e1ddb6 的
k1 2× + k4 1× NVCFPexecRemoteDisconnected 一次性 distributed transient (单请求 buffer 自愈, 无 multi-key 连续复发);
buffer 全 attempt-1 直 flush 7-10s 无重试无级联无 buffer_exhausted。

## 依据 (轮前注入 2026-08-07 23:20 CST + 实时 DB/health 复核)
...
- **nv_tier_attempts 30min**: tier=dsv4f0731_nv, 全 `pexec_success` 为主 (fid 281478d0: k0=22 k1=18 k2=24
  ...
  `NVCFPexecRemoteDisconnected` 一次性 distributed transient 单请求 buffer 自愈。量 (总 3x) 小,
  无 multi-key 连续复发, 非配置漂移。
- **buffer 日志 (docker logs nv_gw --since 30m)**: 全 `attempt=1` direct flush (success_tool_call),
  elapsed 7-10s, 无重试无级联无 buffer_exhausted、无 WAIT。
...
- **k1/k4 RD** (fid 52e1ddb6): 量小 (总 3x, 均单请求), 一次性 distributed transient, 单请求 buffer
  自愈, 与历史记忆模式一致 (泄漏源=越界容器 40666 hermes 线, 宿主分离)。无 multi-key 连续复发,
  不构成配置漂移。仅当 RD/error 在多 key **连续复发** (多个独立请求多 key 持续失败) 才查链路/mihomo 线路。
