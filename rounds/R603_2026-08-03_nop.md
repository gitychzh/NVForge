# R603 — NOP 巡检轮 (2026-08-03 12:12 CST)

## 基线 (R603 链路分析注入, 12:12 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本; 复测 `nv_requests` 确认 0 行)
- dsv4p_nv 30min: 26 req, 22×200 + 2×429 + 2×502 (SR=84.6%, hermes caller)
  - vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% → 波动区间内上升, 与 R545-R602 同一 NVCF 配额波动模式
  - per-key: key2 22×200 (命中可用 key 100% 200, avg_dur=11405ms, IP 203.10.96.139)
  - 空 key 2×429 + 2×502 (全挂时 NVCF 侧拒绝, 429 avg_dur=1266ms 快速返回, 502 avg_dur=17494ms 慢速 peer-fb-skip)
- 唯一错误 `all_tiers_exhausted` ×4 (avg_dur=9380ms, NVCF 配额型, 非 nv_gw tier 故障)
  - vs R602 avg_dur=7868ms → 变长 1.2x, 仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort)
- nv_tier_attempts 0 行 (复测确认) = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 22×200 (命中可用 key 时 100% 200)
- 无 buffer/wait 日志 (30min 无 buffer 触发, 复测确认), 无 stream_total_deadline, 无 zombie
...
- 按分钟趋势: 03:46-03:51 全挂 2×429 → 03:56-04:12 恢复 22×200 + 2×502 (NVCF 配额恢复)
...
- dsv4p_nv 26 req: 22×200+2×429+2×502 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1266ms 快速返回)
...
- all_tiers_exhausted avg_dur 9380ms (vs R602 7868ms) 变长但仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort), 非 nv_gw tier 故障
- 502 avg_dur 17494ms ×2 = peer-fb-skip 慢速返回 (dsv4p_nv 跳 peer fb), 全挂时 NVCF 侧慢拒绝
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% → 波动区间内, 与 R545-R602 同一 NVCF 配额波动模式
