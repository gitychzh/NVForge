# R568 — NOP 巡检轮 (2026-08-03 09:15 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R567 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本,
  铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 17 req, 12×200 + 5×429 (SR≈70.6%, 全 `hermes` caller).
  与 R567 (12×200+5×429) / R566 (12×200+5×429) 完全一致量级, 12×200 聚集在 00:51-01:06
  命中配额空隙, avg_dur=9175ms 稳定 (max=25392ms), 仍在 NVCF 配额波动区间, 非 nv_gw 侧新故障.
- 唯一错误类型 `all_tiers_exhausted` × 5 (avg_dur=3036ms, 与 R567 的 3036ms /
  R566 的 2916ms 同量级 — KeyManager 全局冷却在 tier 层前拦截,
  tier_attempts 0 行, 历史一致).
- 周期性 GLOBAL-COOLDOWN 180s 每 5min 1 次 (00:45/00:55/01:01/01:06/01:10),
  与 R268-R567 完全一致.
...
- 唯一错误类型: `all_tiers_exhausted` × 5, avg_dur=3036ms (全 NVCF 配额型, 非 nv_gw 故障)
- nv_tier_attempts 0 行 = 429 在 tier 层前被 KeyManager 全局冷却拦截, 历史一致
- per-key: key2=10×200, key3=2×200, 5×429 无 tier 命中 (全局冷却拦截, key 级未触发)
- per-egress-IP: 203.10.96.139=10×100, 134.195.101.194=2×100, 5×429 无 IP 标记
