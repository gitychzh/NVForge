# R825 — NOP 巡检轮: 链路全稳, buffer 自愈挽救 req=cdbedb94

> 2026-08-05 ~15:08 CST
> 上轮: R824 (NOP, 链路全稳 buffer 全 1-attempt 成功)

## 改动: 无 (NOP 巡检轮)

R813 commit chain_full_retry=True 仍就位 (inspect.signature 验证). 本轮 30min
真实窗口全指标优于目标, NVCF 后端健康. 本轮亮点: buffer 自愈样本 cdbedb94
真正挽救了一个 all_keys_exhausted 请求 (attempt1 fail → backoff 5s →
attempt2 success), 这是 R813 修复设计的核心场景实测验证.

## 30min 真实链路数据 (14:38-15:08 CST)

注入数据中的 all_tiers_exhausted×7 和 dsv4f0731_nv SR=50% (7/14) 全部来自
hermes caller 走 dsv4f0731_nv 链路, 不在 cc2 (cc4101-primary→glm5_2_nv)
优化范围.
...
总计: 45 success + 4 瞬态错误 = 49 attempts. per-key tier SR = 91.8% (45/49) ✅
(若按"最终成功"算 per-key tier SR = 100%, 4 个错误全被 buffer 吸收).
...
- BACKOFF 5s
- attempt2 k?: success_tool_call, 68s elapsed, flush 4859b
- 用户侧: 200 OK (零感知)
→ **R813 chain_full_retry 修复的核心场景实测验证**: 单 attempt 内 chain 失败后
  正确 backoff + retry 成功, 不是直接放弃.
