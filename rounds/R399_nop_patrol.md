# R399: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=93.3% 28/30, all_tiers_exhausted×2 avg 18021ms, 一百二十三轮一致)

## 当前轮基线 (2026-08-02 23:26 CST, R398 已完成, R399 巡检)
- 本仓 master: R398 已 commit. hermes 仓: R398 已 push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R399 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=93.3% (28/30), 失败 2 = all_tiers_exhausted×2
  (sub=`all_tiers_failed_in_mapped_tier`, avg 18021ms, NVCF dsv4p function 本窗口配额瞬时空位,
  全非缓冲 caller mapped-tier 直接失败无轮转保护).
  成功 28×200: key2 27× (hermes mapped, avg 10845ms, egress 203.10.96.139×27) + key3 1× (openclaw, 4445ms, egress 134.195.101.194×1).
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 mapped-tier 直接失败, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, 一百二十三轮一致).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **一百二十三轮一致 R268-R399**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~23:25 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- caller × model × status: 仅 hermes/dsv4p_nv (27×200+1×429+1×502) + openclaw/dsv4p_nv (1×200), 无 cc4101-primary 行.

### 2. dsv4p_nv 30min 全 caller SR=93.3% (28/30)
| caller | request_model | status | count | avg_dur |
|---|---|---|---|---|
| hermes | dsv4p_nv | 200 | 27 | 10845 |
| hermes | dsv4p_nv | 429 | 1 | 2411 |
...
per-key (nv_key_idx): 空 → 2×fail (429+502, mapped tier 直接失败时 nv_key_idx 字段为空, 设计行为);
  key2 → 27×200 (avg 10845ms, egress 203.10.96.139×27); key3 → 1×200 (openclaw, 4445ms, egress 134.195.101.194×1).
fallback_occurred=f×30 (全部 false, 0 fallback).
finish_reason 分布: tool_calls×23 + stop×5 (无 zombie/异常长流).

### 3. 30min 错误分类
| error_type | sub | count | avg_dur |
|---|---|---|---|
| all_tiers_exhausted | all_tiers_failed_in_mapped_tier | 2 | 18021 |

### 4. 30min 按分钟趋势 (14:55-15:21 CST)
- 14:55 429×1 → 15:00-15:21 稳定 200 流 (27×200), 15:06 502×1 间夹.
- 失败集中 14:55 (429) + 15:06 (502), 其余时段全 200, function 配额波动间歇性.
