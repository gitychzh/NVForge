# R536 — NOP 巡检轮 (2026-08-03 07:30 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R535 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本, 铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 16 req SR=62.5% (10×200 + 4×429 + 2×502), 全 `hermes` caller (1× openclaw 200).
  SR 较 R535 的 60.0% 微升 2.5pp, 仍在 NVCF 侧配额波动区间 (40-65% band, R530-R535).
- 唯一错误类型 `all_tiers_exhausted` × 6 (avg 19.6s), 无新错误类型.
- 2× 502 = peer-fb-skip 已知路径 (23:02 + 23:17 UTC), R530 同源, 非新错误.
- NV-GLOBAL-COOLDOWN tier=dsv4p_nv 周期性 429 仍在 (23:05/23:10/23:20/23:26 UTC),
  与 R268-R535 完全一致, NVCF 侧配额波动.
...
- 配置实测与 R475-R535 完全一致, 无漂移.

## 本轮改动
- 无 (NOP). 铁律1 cc2 视角不满足 (cc2 0 流量无评估样本) → 不动码.
- dsv4p_nv 错误模式与 R268-R535 完全一致 (周期性 all_tiers_exhausted + NVCF 侧 429 配额波动 +
  peer-fb-skip 502), 非 nv_gw 代码问题.

## 依据
- cc2 30min 0 req → 无 cc2 评估样本 (铁律1 cc2 视角不满足)
- dsv4p_nv 16 req: 10×200 (k2=9 + k3=1, 23:01/23:04/23:16/23:26 UTC 恢复窗口, 7.9s avg dur) +
  4×429 (每 5min 1 次 GLOBAL-COOLDOWN 180s) + 2×502 (peer-fb-skip, 23:02 + 23:17)
- 唯一错误类型: `all_tiers_exhausted` × 6, avg_dur=19.6s (全 NVCF 配额型, 非 nv_gw 故障)
- per-key (dsv4p): k2=9×200 (8568ms avg), k3=1×200 (2408ms), 其余 key 全 429 (无成功)
- per-egress-IP: 203.10.96.139=9×100% SR, 134.195.101.194=1×100% SR, 其余 IP 0 success (6×429/502)
- 200 finish_reason: tool_calls × 9, stop × 1 (无 zombie)
- fallback_occurred=f × 16 (nv_gw 层 dsv4p_nv 不 fallback, cc4101 层兜底)
- 全局冷却模式: 每 5min 1 次 NV-GLOBAL-COOLDOWN tier=dsv4p_nv all keys 429,
  与 R268-R535 完全一致, 是 NVCF 侧配额波动, 非 nv_gw 故障
