# R712 — NOP 巡检轮 (2026-08-03 19:27 CST)
...
- **dsv4p_nv 全量 SR**: 93.3% (42/45) — 全 hermes/openclaw 非 cc2 流量
  - per-key: k0 8×200, k1 8×200, k2 10×200, k3 8×200, k4 8×200+1×502 — 均衡健康
  - per-IP: 5 US IPv4 全 89~100% (203.10.96.139 100%, 134.195.101.120 89%, 其余 100%)
  - 200 延迟: avg 15.5s, max 65.2s, ttfb 15.0s, avg_in 9 tok, avg_out 31 tok
  - finish_reason: tool_calls×31, stop×7, length×4 — 无 zombie
- **glm5_2_nv 全量 SR**: 0% (0/6) — 全 hermes 非 cc2, NVCF 上游持续退化 (10:20 起未恢复)
- **错误分类**: all_tiers_exhausted×5 (5key 全挂, glm5_2_nv NVCF 上游 RemoteDisconnected), NVStream_IncompleteRead×3, stream_absolute_cap×1
- **tier 错误**: NVCFPexecRemoteDisconnected×3, IntegrateRemoteDisconnected×4, pexec_conn_RemoteDisconnected×3, 429×1 — 全 NVCF 上游配额副作用, 非 nv_gw 可控
- **fallback**: f×51 (全 hermes/openclaw 非 cc2, glm5_2_nv 退化→dsv4p 兜底)

## 根因
- glm5_2_nv NVCF 上游间歇退化 (10:20 起持续至本窗口), cc4101 fallback → dsv4p 兜底
- cc2 本窗口无请求 = 无优化素材, NOP 巡检

## 验证: NOP 无需 restart
- `curl /health`: nv_gw ok(5keys, passthrough) + cc4101 ok(primary glm5_2_nv) + dsv4p_nv40066 ok(5keys, passthrough)
