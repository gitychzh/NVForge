# HM2 Optimize HM1 — Round R1223

## 0. 触发判定
- cron脚本输出: `"这是我提交的, 不触发"`
- 最新 commit: ca1426e (R1222, author=opc2_uname aka HM2自提交)
- SSH timeout: HM1 (100.109.153.83:222) 不可达 — Tailscale WG data-plane broken (tx 248508 rx 0)
- 判定: **FALSE TRIGGER** — R1222是HM2自提交，非HM1新提交。91st chain of R1133 false trigger (R1133→R1223)。

## 1. 数据收集

### 1.1 直接收集（HM1 SSH unreachable）
不可行。Connection timed out。

### 1.2 Chain Estimate（R1209 pattern）
从 R1133→R1222 chain 估算（82+ rounds of identical zombie-only data）：
...
- ms_gw: 0 traffic
- nv_tier_attempts: 0
- compose md5: 7975939c245761e451a8813852dcb9bf (unchanged since R1133 22:03 UTC)

### 1.3 Container params（HM1 compose 已验证 floor/optimal 48h+）
unchanged since R1133→R1222 chain — zero config changes possible. All params at floor/optimal:
...
- **SR ceiling**: ~62.5% — dsv4p_nv 100% SR (0 traffic) + glm5_2_nv zombie-limited
...
## 4. Chain context
R1133 trigger at 2026-07-13 22:03 UTC → R1134–R1223 all NOP (91 rounds of false-trigger chain-dispatch).
...
- Tailscale WG data-plane: tx incrementing (~248K→260K next cycle), rx 0 persistently — WG broken.
