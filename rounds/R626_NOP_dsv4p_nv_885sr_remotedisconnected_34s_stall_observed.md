...
- dsv4p_nv 30min (hermes caller): 26 req, 23×200 + 2×429 + 1×502 (SR=88.5%)
  - vs R625 85.0% / R624 82.4% / R623 76.5% / R622 64.3% / R621 57.1%
    → **连续 5 轮反弹 (57.1→64.3→76.5→82.4→85.0→88.5), 趋势持续, 已回到 R617-R618 正常波动区间 (75-91%) 上沿**
...
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×3 (avg_dur: 429→1662ms 快速ABORT; 502→34646ms 长ABORT)
    → all_tiers_exhausted R621×6 → R622×5 → R623×4 → R624×3 → R625×3 → R626×3 持平
...
- 02:40-03:40: 波动 0-80% (NVCF 配额间歇, 4×429+2×502)
- 03:50-05:40: 回升 60-100% 为主, 04:00 后多数桶 80-100%
- 整体: 反弹确认, 非线性恶化

## 新发现 (本轮观察点, 不改码) — RemoteDisconnected 34s 长卡顿
- 本轮 13:42:45 出现 1×502, duration=34646ms, 根因链:
  - `[13:42:10.8] NV-KEY attempt 1/7: k3 → NVCF pexec via socks5h://172.18.0.1:7902` (peek-retry 路径, explicit start_key=k3)
  ...
  - `[13:42:45.5] NV-PEER-FB model=dsv4p_nv in peer-fb skip list → returning local 502 for agent ms_gw fallback`
- DB 历史查证 (24h, dsv4p_nv, all_tiers_exhausted + duration>10s):
  - 08-03 05:42 (34646ms), 04:56 (34306ms), 04:07 (34716ms)
  - 08-02 23:16 (54494ms), 23:01 (53171ms), 16:09 (35690ms), 15:06 (33630ms), 14:25 (10722ms), 11:27 (34101ms), 10:56 (72299ms)
  - **至少 10 次, 反复发生的低频模式, 不是单次瞬态**
- 根因分析:
  - peek-retry 路径 (`_try_tier_keys` + `NV-PEEK-RETRY explicit start_key`) 设计语义 = 只试 1 个 key 探测, k3 失败不轮转其他 key
  - NVCF 侧间歇性慢响应: 接受连接后 34-72s 静默, 然后主动 FIN (RemoteDisconnected), 非 socket 超时
  - per_attempt_timeout=min(UPSTREAM_TIMEOUT=90, remaining_budget-5)=90s, 但 NVCF 在 34.6s 主动断, 未触达 90s read timeout
...
- 影响评估:
  ...
  - 改 nvcf_conn.py read timeout 行为 = 高风险 (影响全 key 全 model 连接层), 非本轮该动
