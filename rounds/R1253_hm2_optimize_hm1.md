# HM2 Optimize HM1 — Round R1253

## 类型：NOP (zombie code-level, pre-restart ATE, IncompleteRead, 77.4% SR all code-level failures)

## 触发分析

- HM1 cron 脚本检测到 HM1 提交了新 commit → 触发 HM2 执行优化
- HM1 git HEAD: 4d1c4eb (R1252: HM2→HM1 — NOP)
- 实际触发: HM1 在 R1252 后未提交新内容，但脚本检测到 git fetch 后 HEAD 更新

## 6h 数据 (HM1, 2026-07-14 00:05 UTC)

### 总体
- 93req/72OK/21fail = 77.4% SR
- 按模型:
  - glm5_2_nv: 92req/71OK/21fail = 77.2% SR
  - dsv4p_nv: 1req/1OK = 100% SR

### 按路径
- nv_integrate: 86req/68OK/18fail, avg_ttfb=13512ms, avg_dur=15153ms, max_dur=45191ms
- nvcf_pexec: 4req/4OK/0fail, avg_ttfb=24938ms, avg_dur=24939ms, max_dur=45950ms
- (NULL upstream): 3req/0OK/3fail, avg_dur=5449ms

### 延迟
- 成功请求 duration 分布:
  - <5s: 2, 5-10s: 26, 10-15s: 19, 15-20s: 8, 20-25s: 6
  - 25-30s: 2, 30-35s: 3, 35-40s: 2, 40-50s: 4
- 成功请求全部在 50s 以内，无异常延迟

### 错误分类
- zombie_empty_completion: 17 (glm5_2_nv integrate, NVCF content-filter stop+12chars, avg 16098ms abort, 3-28s range)
...
### Fallback
- fallback_occurred=false: 93/93
- fallback_actually_attempted=false: 93/93
- 0 fallback 触发 — FALLBACK_GRAPH={} (R832 预期)
- ms_gw fallback 未触发: zombie 路径在 stream abort 处理，在 ATE 之前
- ms_gw 日志: MS-STREAM-DONE 正常，ZHIPUAI/GLM-5.2 后端响应正常 (23-49KB)

### ms_gw
- ms_requests 6h: 5req/0OK (DB trap — ms_gw status 为 text 类型)
- ms_gw logs: MS-STREAM-DONE 正常，MS-OK-STREAM 正常

### Hourly SR
- 10:00 UTC: 35req/27OK/8fail = 77.1%
...
- nv_gw compose md5: 6e23559de1376d2d638f98f34a544139
- 所有容器 healthy
...
### 分析
1. **全部 21 个失败均为 code-level**：
   - 17 zombie_empty_completion (81%): NVCF glm5_2_nv integrate 对大 context (109-177K chars) 返回 content-filter stop+12chars。网关正确检测 finish_reason=stop + content_chars < 50 + input_chars >= 5000 → 判定 zombie → 发送 content_filter error SSE chunk → openclaw 走 model-fallback。3-28s 快速中止优于旧 96s hang。**R1107 确认：code-level feature，不可 config 修复**。
   - 3 all_tiers_exhausted (14%): 全部 pre-restart (12:33-12:39 UTC)，upstream_type=NULL，0 tier_attempts，avg 5449ms。Post-restart 零 ATE。**R1241 模式的 404/not found NVCF 函数退化，不可 config 修复**。
   - 1 NVStream_IncompleteRead (5%): 24,019ms。TCP 层面流中断，不可 config 修复。
...
4. **ms_gw 健康**：MS-STREAM-DONE 正常，ZHIPUAI/GLM-5.2 后端响应正常。nv_gw 不主动触发 ms_gw fallback 因为 zombie 路径在 stream abort 阶段处理，不会进入 ATE→ms_gw 路径。
...
7. **连续 8 轮 NOP (R1246-1253)**，所有可调参数已达 floor/optimal，所有失败归因于 NVCF 上游/content-filter/网络瞬断，非 config 可调。
