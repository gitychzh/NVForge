# R713 — NOP 巡检轮 (2026-08-03 19:31 CST)

## 改动: 不改码 (NOP)

## 依据 (30min 窗口 ~19:01-19:31 CST)
- **cc2 (cc4101-primary) 流量**: 30min cc_requests 窗口空 — cc2 本窗口零请求, 无数据不动手
- **nv_gw 全量 30min** (非 cc2 流量, hermes/openclaw caller):
  - dsv4p_nv: 32×200 + 2×502 = **SR 94.1%** (32/34) — 接近目标, 兜底链路健康
  - glm5_2_nv: 6×502 = **SR 0%** (0/6) — NVCF 上游持续退化 (10:20 起 ~9h, 未恢复), 非 nv_gw 可控
- **错误分类 (30min)**:
...
- **tier 错误 (glm5_2_nv)**:
  - IntegrateRemoteDisconnected × 4 (k1×1, k2×2, k3×1)
  - pexec_conn_RemoteDisconnected × 3 (k2)
  - NVCFPexecRemoteDisconnected × 1 (k1)
  - 全 RemoteDisconnected — NVCF 上游间歇断连, 非 nv_gw 配置问题
- **dsv4p per-key**: k0 6×200, k1 6×200, k2 8×200, k3 7×200, k4 5×200+1×502 — 均衡健康
- **dsv4p per-IP**: 5 US IPv4 全 83~100% (203.10.96.139 8×100%, 134.195.101.194 7×100%, 其余 6×83~100%)
- **dsv4p 200 延迟**: avg 10.6s, max 65.2s, min 1.3s, ttfb 10.2s, avg_in 12 tok, avg_out 41 tok
- **dsv4p finish_reason**: tool_calls×21, stop×7, length×4 — 无 zombie (无 null/空)
- **fallback**: f×40 (全非 cc2 流量, hermes/openclaw)
- **根因**: glm5_2_nv NVCF 上游持续退化 (10:20 起 ~9h 未恢复), cc4101 fallback → dsv4p 兜底, cc2 本窗口零流量无影响

## 验证: NOP 无需 restart
- `curl /health`:
  - nv_gw: ok, 5keys, pexec_models=[kimi_nv, dsv4p_nv, glm5_2_nv]
