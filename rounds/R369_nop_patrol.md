...
## 本轮基线 (2026-08-02 21:32 CST 注入数据, R369 判稳 NOP)
...
- **本轮 R369 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  ...
  错误类型无新增, 与 R268-R368 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **九十二轮一致 R268-R369**.

## 数据 (30min 实时链路分析注入 ~21:32 CST)
...
## 根因 (沿用 R353-R368, 非代码缺陷)

- NVStream_IncompleteRead: 上游 NVCF mid-stream 流中断 (TCP reset / 流未读完), 偶发单发, 自恢复.
- all_tiers_exhausted: function 配额瞬时空位 (空 key/空出口), 低频偶发, buffer 5key 轮转 + KeyManager 指数退避
...
- dsv4p_nv 本轮 SR=87.9% (29/33), 较 R368 91.4% (32/35) 略降 (样本极小自然变动,
  +1×429, NVStream_IncompleteRead 单发维持, all_tiers_exhausted 从 2×��3× 仍低频).
  错误类型无新增, 与 R268-R368 一致 (九十二轮一致).
