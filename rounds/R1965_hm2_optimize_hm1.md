# R1965 (HM2→HM1): ⏸️ NOP — near-zero post-deploy traffic (52min), all params floor/optimal, 5 zombie all NVCF-level

## 数据
- 6h: 41 req, 36 OK (87.8% SR), 5 fail (status=502, zombie_empty_completion, all glm5_2_nv)
- Post-R1961 restart (2026-07-19 18:35:33 UTC): nv_gw Up 52 minutes, near-zero post-deploy traffic
- 30min burst: 2 req, 1 OK, 1 zombie (50% SR — tiny sample, statistically meaningless)
- dsv4p_nv: 10 OK, all phantom ATE (avg=31599ms, max=55335ms), 0 genuine OK — NVCF completely dead
- glm5_2_nv: 26 OK (avg=9954ms, max=26165ms), 5 zombie all NVCF function-level
...
- 5× `zombie_empty_completion` (status=502), all glm5_2_nv
  - Timestamps: 15:03, 16:03, 18:04, 18:33, 19:03 UTC (~60min spacing)
...
- 22× `all_tiers_exhausted` + status=200 (phantom ATE, all peer-fb rescued)
  - dsv4p_nv: 10 phantom ATE, avg=31599ms, all peer-fb rescued → OK
  - glm5_2_nv: 12 phantom ATE, avg=7410ms, all peer-fb rescued → OK
...
- Container healthy, StartedAt 2026-07-19T18:35:33Z, Up 52 minutes
...
**NOP判定**: 与 R1960/R1961/R1962/R1963 一致 — 近零 post-deploy 流量 + 5 zombie 全部 NVCF 级 + 22 ATE 全 peer-fb 救援成功 (100%) + 全部参数 floor/optimal。R1959 的 BIG_INPUT_COOLDOWN=86400 改动需等待下次流量窗口才能评估效果。任何改动都是无数据支撑的猜测。
...
## 验证
- ✅ docker ps → nv_gw Up 52 minutes (healthy)
