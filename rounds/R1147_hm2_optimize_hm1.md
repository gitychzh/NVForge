# R1147: HM2→HM1 — NOP (false trigger, 16th chain of R1133, zombie-only, all params floor/optimal, DB gap persists)

## 1. 触发分析

cron 脚本输出: `"这是我提交的, 不触发"`
- 最新 commit `1c4b6eb` (R1146) author = `opc2_uname` (HM2)
- 脚本正确检测到自提交并标记 "不触发"
- cron 仍被派遣 — 误触发 (16th chain of R1133 false trigger)
- Symlink `RN_hm2_optimize_hm1.md` → `rounds/R1147_hm2_optimize_hm1.md` (本文件)

## 2. 改前数据 (2026-07-11 08:15 UTC, 6h)
...
| Model | 总 | OK | Err | SR | avg_ms_ok |
|-------|-----|-----|------|------|-----------|
| dsv4p_nv | 4 | 4 | 0 | 100% | 9,515 |
| glm5_2_nv | 42 | 27 | 15 | 64.3% | 6,378 |
...
| glm5_2_nv | nv_integrate | 43 | 28 (65.1%) |
...
- 全 15 个 zombie: finish_reason=stop, content_chars=12, input_chars ≥ 160K, no tool_calls, no fallback
...
```
全部 glm5_2_nv integrate: attempt 1/7, k1-k5 轮转, NV-INTEGRATE-SUCCESS 首次
→ NV-ZOMBIE-EMPTY (content_chars=12) → NV-ZOMBIE-ERROR-CHUNK (content_filter)
→ 期望 openclaw 走 fallback
08:03 UTC: 少量 429_integrate_rate_limit (k2→k3→k4, 2 cycle attempts → success)
dsv4p_nv: 无日志 (4 req 全成功, 0 错误)
...
| NVU_INTEGRATE_TIMEOUT_FASTBREAK | 1 | optimal | glm5_2 integrate always succeeds on attempt 1-2 |
...
| NVU_PEER_FB_SKIP_MODELS | glm5_2_nv | settled | R1039, dsv4p_nv has peer-fb path |

## 4. 决策: NOP

**zombie_empty_completion 是唯一错误类型 (15/15, 100%), 全在 glm5_2_nv integrate 路径。**

**根因链**:
1. glm5_2_nv integrate 返回 HTTP 200 + finish_reason=stop + 12 chars content (160K+ input)
...
- dsv4p_nv pexec 100% SR (3/3), integrate 100% (1/1)
- 历史 R1133→R1146 连续 14 轮 NOP, 同根因

**与 R1141-R1146 一致**: 连续第 15 次 zombie-only NOP (R1133 触发链)。数据证明无配置变更空间。
