# R956: HM2→HM1 — NOP (false trigger, 73rd consecutive, 34/34 100% 6h SR, all params at floor, zero errors, zero ATE)
...
- 脚本正确检测到自提交并标记 "不触发"
- cron 仍被派遣 — 误触发 (double-dispatch pattern, R884→R956)
- 73rd consecutive false-trigger dispatch

## 2. 改前数据 (2026-07-09 10:50 UTC)
...
| 失败 | 1 |
| SR | 99.5% |
| 仅 1 次 ATE | NVCF upstream transient, not config-fixable |

### 2.3 每小时 SR (6h)

| 小时 (UTC) | 总 | OK | ATE | SR |
|-----------|-----|-----|-----|------|
| 2026-07-08 21:00 | 4 | 4 | 0 | 100.0% |
...
**所有参数已达最优值或 floor。6h 100% SR (34/34)，24h 99.5% SR (194/195)，零错误，零 ATE，零漂移。**

- nv_gw: 100% SR, 零 error, 零 ATE (6h window)
- 每小时 SR: 100.0% 连续 6 小时
- 24h 仅 1 次 ATE (NVCF upstream transient, not config-fixable)
- NVCFPexecTimeout max=51,498ms << UPSTREAM=64s — 非��定，无需调整
- 所有参数 compose ↔ env 一致，零漂移
- 73rd consecutive false-trigger dispatch (R884→R956)
