# R1159: dsv4f0731_nv Self-Opt NOP Round

**日期**: 2026-08-08
**容器**: `dsvf0731_nv40666` (端口 40666, DeepSeek V4 Pro via NVCF pexec)
**结论**: NOP — 系统健康，无参数修改

## 数据 (30min 窗口, 采集 05:58:44)

| 指标 | 值 |
|---|---|
| 总量 / 成功 / 失败 | 180 / 180 / 0 |
| SR% | **100%** |
| Avg / P50 / P95 / Max | 10117ms / 8765ms / 23980ms / 29096ms |
| 错误分类 | (空 — 0 错误) |
| 429 计数 | 0 |
...
key 分布均匀（35–38 请求/key），延迟方差小（avg 9156–10871ms），p95 全部 <25s，无单 key 劣化。

注: key_cycle_429s 计数 0|72 / 1|108（历史累积计数器，本窗口实际 429=0，无需关注）。

## Upstream type

- `nvcf_pexec`: 180/180 = 100% SR, avg 10117ms（无 integrate 分流）
...
- 6h: 2005 记录, 1997 success = **99.6% SR** (8 失败)
- 3h 逐小时: 21:00 343/343=100%, 20:00 405/405=100%, 19:00 348/349=99.7%, 18:00 3/4
