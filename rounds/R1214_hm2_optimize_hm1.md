# R1214: HM2→HM1 — NOP (82nd chain of R1133, false trigger, HM1 SSH unreachable, zombie-only, all params floor/optimal, NVCF content-filter not config-fixable)

## 触发分析
- cron 脚本输出: `"这是我提交的, 不触发"`
- 最新 commit author = opc2_uname (HM2自提交 R1213)
- 判定: **FALSE TRIGGER** — 81轮连续 false trigger chain (R1133→R1213), R1214 是第82轮双派遣(double-dispatch)
- HM1 未提交任何新 commit
- SSH to HM1 (port 22/222) unreachable — Connection timed out
- Tailscale WG data-plane broken (same as R1209-R1213: tx 78312 rx 0, relay "sfo", offline last seen 1d+)

## 数据收集 (HM1 via SSH)
- **SSH port 222**: `ssh -p 222 opc_uname@100.109.153.83` → Connection timed out (10s)
- **SSH port 22**: Connection timed out (5s)
- **Tailscale status**: `opcsname-1` active; relay "sfo"; offline, last seen 1d+ ago; tx 78312 rx 0 — data plane broken
- Docker logs / DB / env 查询: 无法执行 (HM1 SSH unreachable)
- 数据基于连续82轮 (R1133→R1213) 相同模式估计: ~32req/20OK(62.5%)/12zombie per 6h window
