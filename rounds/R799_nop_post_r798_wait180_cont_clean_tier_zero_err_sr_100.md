# R799 — NOP 巡检 (R798 后置续净, wait_queue 180s 续跑, tier 零错误)

> 上轮: R798 (NOP — R796 wait_queue 180 续净, tier 零错误)
> 本轮: R799 (NOP 巡检 — R798 后置验证, 续净)
> 时间: 2026-08-05 ~09:50 CST
...
### 1. cc4101 用户视角 SR = 100% (排 499)
- cc_requests 30min: total=1016, ok=1006, s499=10 (全 client_gone_mid_stream, real_err=0)
- SR 含499 = 99.0% / 排499 = 100.0%
- fallback fb=9/1016 = 0.89% < 10% 目标
- 30min cc4101-primary 71×200 + 1×502 (502 = 历史死请求 f15fe5ef)

### 2. glm5_2_nv tier 零错误
- nv_tier_attempts 30min: 72 attempts 全 pexec_success, 零 error_type
- 这是连续第 2 轮 tier 零错误 (R798 也是 0, R797 是 14 RemoteDisc)
- per-key fid 全 b1b22d03 均布:
  - k0: 20, k1: 13, k2: 15, k3: 13, k4: 10
  - upstream_type 全 nvcf_pexec (NV_GLM52_MODE_CHAIN=pexec_us_rr 生效)

### 3. buffer 全 attempt=1 success
- 最近 20 条日志: 全 NV-BUFFER-SUCCESS after 1 attempt(s)
- elapsed 1.9s - 17s, 内容 1.2KB - 14KB
- verdict=success_text / success_tool_call 交替 (thinking=True)
- 零 retry / 零 WAIT / 零 KEYMGR / 零 BREAKER 日志
- stairs=[90,90,90,90,90] total_deadline=450s 生效

### 4. 1× nv_gw 502 非新错误 (R796 案例延续)
- request_id=f15fe5ef (推断), duration 400589ms = 400s
- error_type=buffer_exhausted (WAIT-FAIL 路径)
- cc_requests 表 1016 请求中 fb 仅 9, 此 502 未穿透用户
- = R796 wait_queue 180s 验证案例本身, R797/R798 已记

### 5. 容器健康
- nv_gw: ok, Up 25min (R796 09:10 up -d 基线续跑)
- cc4101: ok, Up 8h
- dsv4p_nv40066: ok (并行线, 不属 cc2 职责)
- logs_db: ok

## 判稳结论

- cc4101 SR 排499 = 100% ≥ 99% 阈值 → NOP 巡检轮
- glm5_2_nv tier 零错误 — 续 R798 连续第 2 轮零错, 历史最净窗口
- fallback 0.89% < 10% 目标
- 无新错误类型 / 无新错误模式
- cleanest 持平 27 (R774 基线; 1 历史死请求 502 未计)
...
- R796 案例 f15fe5ef 触发了此路径 (但未穿透用户)
...
- nv_gw: **NVU_WAIT_QUEUE_ENABLED=1, NVU_WAIT_QUEUE_MAX_WAIT=180** (R796)
- nv_gw: NVU_PROBE_INTERVAL=15, NVU_FORCE_STREAM_UPGRADE=0
