# R1120 — cc2 NOP 巡检轮 (不改码)

> 日期: 2026-08-07 ~23:47 CST
> 状态: **cc2 主链 100% SR 零错误; 全量 dsv4f0731_nv 100% SR 零错误零 fallback**

## 结论

cc2 主链 (cc4101-primary 经 nv_gw:40006, primary model=dsv4f0731_nv) 30min = **102/102 = 100.0% SR, 0 bad**。
全量 dsv4f0731_nv = 139/139 = **100.0% SR**, 非-200 = 空。fallback 0% (全走 primary)。
cc2 范围无新错误签名 → **NOP, 不改码**。

## 轮前链路分析数据 (注入 2026-08-07 23:47)

- **30min nv_requests (cc4101-primary)**: status 仅 200 × **102** = 100.0% SR, 0 错误。
- **dsv4f0731_nv 全量**: 139/139 = **100.0% SR** (cc4101-primary 102 + hermes 37)。
...
- **nv_tier_attempts 30min**: tier=dsv4f0731_nv, 全 `pexec_success` 为主 (fid 281478d0);
  ...
  k1 1× `NVCFPexecRemoteDisconnected` + k1 1× `empty_200` + k4 1× `empty_200`
  一次性 distributed transient 单请求 tier 自愈, 未上浮为 surface 错误, 无 multi-key 连续复发。
- **buffer 日志 (docker logs --since 30m)**: 无 BUFFER-/WAIT-/keymanager 日志, 全 attempt-1 direct flush, 无重试无级联无 buffer_exhausted 无 WAIT。
- **容器 /health 2026-08-07 23:47 CST**: nv_gw http 200 (Up 20h, primary=dsv4f0731_nv, 5 key), cc4101 http 200 (Up 20h)。
...
| 全量 dsv4f0731_nv | 139/139 = **100.0% SR** 零错误 | ✅ |
| 30min 错误分类 | (无错误) — 非-200 空 | ✅ 零错误 |
| fallback 触发率 | 0% (139 total, 全走 primary) | ✅ |
| per-key tier 错误 | 全 pexec_success (fid 281478d0); 仅 fid 52e1ddb6 的 k1 1× RD + k1 1× empty_200 + k4 1× empty_200 一次性 transient 单请求自愈 | ✅ |
...
- per-key 3 个错误全部量小 (k1 1× RD, k1 1× empty_200, k4 1× empty_200), 单请求 distributed
  transient, 未上浮为 surface 错误 (nv_requests 零错误), 与历史记忆 bad-fid-52e1ddb6 泄漏模式
  一致 (泄漏源=越界容器 40666 hermes 线, 宿主分离)。无 multi-key 连续复发 → 非配置漂移, 不处置。
...
- **k1/k4 错误** (fid 52e1ddb6): 量小且单请求 self-heal。仅当 RD/error 在多请求多 key **连续
  复发** 才查链路/mihomo 线路。
