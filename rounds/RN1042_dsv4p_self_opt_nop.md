# RN1042: NOP — dsv4f0731_nv 链路 30min SR=100% (170/170), 零错误零fallback零429, 5 key 全健康, 不改参数

**日期**: 2026-08-08
**采集窗口**: 2026-08-08 ~05:54 UTC
...
env 实测确认与 RN1041 一致，无漂移。`/health` 返回 ok。容器 Up 4 hours。
...
| 总量 / 成功 / 失败 | 170 / 170 / 0 |
| SR% | **100%** |
| Avg / P50 / P95 / P99 | 10432ms / 8946ms / 24036ms / 31408ms |
| 错误分类 | (空 — 0 错误) |
| 429 计数 | 0 |
...
key 分布均匀（33–35 请求/key），延迟方差小（avg 9906–11337ms），p95 全部 <25s，无单 key 劣化。per-key 错误列为空。

注: key_cycle_429s 有 k0=67 / k1=103 的历史累积计数（本窗口实际 429=0，无聚集，均为上一采集周期的旧值，无需关注）。

## Upstream type

- `nvcf_pexec`: 170/170 = 100% SR, avg 10432ms（无 integrate 分流，纯 pexec 路径）
...
- 6h: 1999 记录, 1991 success = **99.6% SR** (8 失败)
- 3h 逐小时: 21:00 316/316=100%, 20:00 405/405=100%, 19:00 349/348=99.7%, 18:00 26/25=96.2%
...
继续监控。当前链路质量稳定（连续多轮 100% SR）。若后续出现 `all_tiers_exhausted` 24h 计数显著上升或某 key 延迟持续劣化，再针对性调参。建议后续轮次重点关注 pexec 延迟的 p95 尾部（本窗口 24036ms，若持续抬升到 >30s 考虑 UPSTREAM_TIMEOUT 微调）。