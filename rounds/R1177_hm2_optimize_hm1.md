# R1177: HM2→HM1 — NOP (false trigger, 45th chain of R1133, zombie-only, all params floor/optimal, NVCF content-filter not config-fixable)
...
- 脚本正确检测到自提交并标记 "不触发"
- cron 仍被派遣 — 误触发 (45th chain of R1133)
- R1176 已是最新回合，symlink 已指向 R1176

## 二、数据收集（改前必有数据 — 6h 窗口）

### 2.1 容器信息
- nv_gw running, compose md5: 7975939c245761e451a8813852dcb9bf (unchanged from R1176)
...
- 1× IntegrateRemoteDisconnected (20,284ms, stale)
- 1× IntegrateTimeout (90,566ms, stale)

### 2.6 ms_gw
- 0 nv-initiated traffic (no ms_gw fallback triggered)
- No ms_gw BrokenPipeError in logs

### 2.7 24h 全景
- Total: 256/205(80.1%), avg_ok_dur=16.2s, max_ok=125.9s
- glm5_2_nv: 207/163(78.7%), avg_ok_dur=17.1s, max_ok=125.9s
- dsv4p_nv: 33/26(78.8%), avg_ok_dur=14.5s, max_ok=48.0s
- minimax_m3_nv: 9/9(100%), kimi_nv: 7/7(100%)
- 24h errors: 38 zombie_empty_completion, 7 ATE (all stale >12h), 6 NVStream_TimeoutError (all stale >12h)
...
```
Input chars holding steady at 166K-170K (openclaw conversation context stable).
...
| 14 | KEY_COOLDOWN_S | 25 | ❌ | Stable, long history |
| 15 | NVU_TIER_BUDGET_DSV4P_NV | 72 | ❌ | R1116 verified, dsv4p_nv 0 traffic |
| 16 | NVU_TIER_BUDGET_GLM5_2_NV | 96 | ❌ | Stable |
| 17 | NVU_PEER_FB_SKIP_MODELS | glm5_2_nv | ❌ | R923 defensive, zombie not peer-fb-fixable |
| 18 | NVU_INTEGRATE_TIMEOUT_FASTBREAK | 1 | ❌ | Floor |

## 四、变更

**Zero param change; zero compose edit; zero restart.**

All failures are NVCF content-filter stop+12chars (zombie_empty_completion) — a code-level NVCF behavior, not config-fixable. Gateway detection is already optimal (abort in 3-8s, return 502 error-chunk to openclaw). dsv4p_nv has 0 traffic — openclaw using ms_gw dsv4p_ms directly. All floor params at minimum. 24h ATE (7) and timeout (6) errors are all >12h stale. Input chars stable at 166K-170K. Identical to R1176.

**No optimization space. NOP.**

## 五、铁律确认
- ✅ 改前必有数据 (6h DB + logs + env + 24h全景 collected)
- ✅ 聚焦 nv_gw
- ✅ 所有修改写入仓库 (NOP 回合记录)
- ✅ 只改 HM1 不改 HM2 (本轮无变更)
- ✅ 少改多轮 (本轮不改, 等数据积累)
