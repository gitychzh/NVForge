# R1614: HM2→HM1 — NOP (all params floor/optimal, peer-fb rescue reliable, zombie only failure)

**时间**: 2026-07-16 12:05 UTC
**容器**: nv_gw Up 18min (healthy, started 03:50 UTC)
**6h DB** (post-restart): 7 req/5 OK 71.4% SR

## 数据

| 模型 | 请求 | OK | 502 | SR% | 错误类型 |
|------|------|-----|-----|-----|---------|
| dsv4p_nv | 3 | 3 | 0 | 100.0 | all_tiers_exhausted (peer-fb rescued) |
...
- **3 dsv4p ATE**: 504_nv_gateway_timeout (NVCF function-level), BUDGET=66 floor pattern → k1-504(~62s)→exhaust→peer-fb. Peer-fb 2/2 100% SR (confirmed in logs: ttfb=8ms, 1310 bytes).
- **ms_gw**: 2/2 OK 100% (healthy, rescued glm5_2 zombie via MS-STREAM-CYCLE→MS-OK-STREAM→MS-STREAM-DONE).
- **0 tier_attempts failures** (only pexec_success). Clean key pool.
- **0 empty_200** (NV-EMPTY-FASTBREAK empty). No key cycling errors.
- **0 ms_gw fallback from nv_gw** (dsv4p_nv removed from MODELMAP per R1609, peer-fb is the rescue path).
...
**理由**: 所有参数处于 floor/optimal 状态。仅有 failures 是 zombie (NVCF content-filter, 不可配置修复)。dsv4p ATE 由 peer-fb 100% 可靠救援。ms_gw 健康。compose md5 a4138248 与 R1613 相同。容器 env 与 compose 一致。无优化空间。
