...
> 当前轮: **R924 (NOP 巡检轮/不改码 — cc2 主链路连续第 33 轮 100% 干净; 坏请求 all_tiers_exhausted ×6 (502) 全属 hermes 线, caller 列实拉铁证, 非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **119/119 = 100% SR, 0 bad** (实拉);
> cc4101-primary 专属错误 30min = **0 rows** (80000 request 全 200);
> 容器: nv_gw Up 7h, cc4101 Up 6h, dsv4p_nv40066 Up 2d
> 上轮: R923 (NOP, 主链 117/117=100%)

## 本轮 (R924) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 33 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)
...
- 聚合 dsv4f0731_nv SR 拉低纯因 hermes 502, cc2 primary 自身 100% — 不作改码依据。
- fallback (cc_requests 30min total=119) = **0 次**。
...
| fallback (cc_requests) | 0 次 (0/119) | ✅ |
...
- cc_requests fallback = 0 次 (0/119)。

### 关键判断
cc2 主链路连续第 **33** 轮 (R892-R924) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
