# R372: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=66.7% 16/24, all_tiers_exhausted×8 + stream_first_byte_timeout×1, 根因不变)

## 本轮基线 (2026-08-02 21:46 CST 注入数据, R372 判稳 NOP)
...
- **本轮 R372 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  ...
  + 1× stream_first_byte_timeout (glm5_2_nv, other caller, 502, 29952ms, 非 cc2 流量单发).
  错误类型无新增, 与 R268-R371 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **九十五轮一致 R268-R372**.

## 数据 (30min 实时链路分析注入 ~21:46 CST)
...
### 3. 错误分类 (DB 实测 30min)
- 8 dsv4p 错误: 8× all_tiers_exhausted (all_tiers_failed_in_mapped_tier, avg_dur 964ms).
  细分: 5×429 (1541ms, NVCF dsv4p function 配额瞬时空位, 空 key/空出口) + 3×502 (1ms, NV-TIER-SKIP 全 5 key cooling 时直接 continue 跳过 tier, 0 attempt 0ms→1ms).
- NV-TIER-SKIP (upstream.py:1676-1693): 全 5 key cooling 时直接 continue 跳过 tier, 0 attempt 1ms 502.
  发生在 agent_type=_nv 非缓冲 caller. cc2 缓冲 caller (NVU_BUFFER_CALLERS) 不受影响, 走 buffer 5key 轮转 + WaitQueue.
- 1 glm5_2_nv 错误: stream_first_byte_timeout (key0, 502, 29952ms, other caller, 非 cc2 流量, 单发).
- 错误类型集合与 R268-R371 一致 (all_tiers_exhausted + stream_first_byte_timeout, 无新增).
...
## 根因 (沿用 R353-R371, 非代码缺陷)

- NV-TIER-SKIP: 全 5 key cooling 时对非缓冲 caller 的快速失败路径 (不等 NVCF 恢复, 快速返 5xx 让上层决策).
  设计如此, cc2 缓冲 caller 不受影响 (走 buffer 5key 轮转 + WaitQueue).
- all_tiers_exhausted (429): NVCF dsv4p function 配额瞬时空位, 低频偶发,
  buffer 5key 轮转 + KeyManager 指数退避 + ProbeWorker 15s 探测 + decayed reset 自恢复 (R268-R371 验证).
- stream_first_byte_timeout: glm5_2_nv other caller 单发, 非 cc2 流量, 自恢复.
- cc2 流量极低, 偶发错误罕见且自恢复, 不达介入阈值.
...
## 参数快照 (未变)
- cc4101: FALLBACK_UPSTREAM_URL=ms_gw:40007, CC4101_STREAM_TOTAL_DEADLINE_S=470, PRIMARY_HEADER_TIMEOUT=400, PRIMARY_UPSTREAM_MODEL=dsv4p_nv, UPSTREAM_TIMEOUT=130, UPSTREAM_IDLE_TIMEOUT=150, CC4101_PRIMARY_SKIP_S=30, CC4101_PRIMARY_FAIL_THRESHOLD=3, FALLBACK_UPSTREAM_MODEL=glm5_2_ms.
