# R1053: RemoteDisconnected 风暴延续 (第32轮) — 模型特异性第32次复现, NOP (无参数修改)

> 时间: 2026-08-06 09:45 BJT (01:45 UTC)
> 容器: dsvf0731_nv40666 (端口 40666, HM2, DeepSeek V4 Pro via NVCF)
> 状态: **NOP (无参数修改)** — NVCFPexecRemoteDisconnected 远程瞬断风暴在 attempt 层
>   持续主导, 第 32 次复现**模型特异性** (同容器同 key 同出口 glm5_2_nv 6h SR=96.6%
...
R1021-R1052 连续记录同一 RemoteDisconnected 风暴并判定模型特异性 NOP。本轮间隔 ~10min
为同一风暴第 32 轮延续（与 R1052 数据基本一致）。

### 30min 窗口 (pre-run 注入) — nv_requests (tier_model=dsv4f0731_nv)
- 总量 33, 200=12, 失败=21, fallback=8, **SR=36.4%**
- Avg 110234ms, p50 95745ms, p95 241242ms, max 258055ms
- 429: 0 计数
- upstream_type: nvcf_pexec 25 (200=12, SR=48.0%), ms_fallback 7 (200=0), nv_integrate 1 (200=0)
- finish_reason: tool_calls=10, stop=2 (正常业务完成)

### 错误分类 (30min, nv_requests)
| error_type | count | avg_ms |
|---|---|---|
| all_tiers_exhausted | 10 | 107026 |
| buffer_exhausted | 8 | 226085 |
| zombie_empty_completion | 3 | 16273 |
...
### per-key 错误 (30min)
- k0: all_tiers_exhausted=10, buffer_exhausted=7
- k1: zombie_empty_completion=1
- k2: buffer_exhausted=1, zombie_empty_completion=1
- k3: zombie_empty_completion=1
- → 错误跨全 5 key 分散, 非单 key/单 SOCKS5 代理问题

### 模型特异性重验证 (6h, 同容器同 key 同出口同时窗)
- **glm5_2_nv 6h**: 同链路 SR=96.6%（R1052 数据）
- **dsv4f0731_nv 6h**: 383 请求, 200=193, **SR=50.4%**, fallback=53
- **结论**: 同容器同 key 同出口, glm5_2_nv 96.6% vs dsv4f0731_nv 50.4% → 模型特异性
  第 32 次成立。故障仅存在于 dsv4f0731_nv 具体 function 的远端执行层。

### 6h/24h 趋势
- 6h 请求层: 383 请求, 200=193, SR=50.4%, fallback=53
- 3h 逐小时 SR: 01:00=34.1%(15/44), 00:00=37.2%(29/78), 23:00=43.3%(29/67), 22:00=55.6%(30/54)
- 24h all_tiers_exhausted: 212 次
...
- **PRIMARY-FAIL-STREAM**: nv_gw 流式 server_5xx status=502 after 147826ms, 切 fallback
...
**远程 function 级**瞬断/劣化, 与容器 env 无关 (第 32 次确认):
...
本轮继续 NOP, 等待 NVCF 侧恢复 (第 32 轮)。模型特异性铁证 (6h glm5_2_nv 96.6% +
0 tier 错误 vs dsv4f0731_nv 50.4% + 错误均匀分散) 第 32 次成立。hm4104 断路器
...
- 容器: dsvf0731_nv40666 Up 28 hours, env 读取正常 (与 R1052 一致, 无漂移) ✓
- 无 env 变更, 无需重启
...
继续等待 NVCF deepseek function 侧恢复。本轮模型特异性铁证第 32 次成立 (6h glm5_2_nv
