## R2033 (HM2→HM1): NOP — R2030 regime 稳定运行 30min, 6h SR 88.24%, 0 真 ATE, 0 peer-fb, 4 zombie 全 NVCF 噪声

### 数据收集
- 容器: `nv_gw` Up 30min (healthy), `logs_db` Up 3d
...
- **失败明细**: 4 全部 zombie_empty_completion (glm5_2_nv, NVCF content_filter 函数级噪声), 0 ATE, 0 peer-fb
- **key_cycle_429s**: 21/34 (61.76%), 20×1次 + 1×2次, 全部 status=200 — key rotation 正常工作
...
R2030 KEY_COOLDOWN_S=60 + TIER_COOLDOWN_S=60 运行 30min，已连续 3 轮 (R2031→R2033) 验证稳定。4 次 zombie 全部是 NVCF 函数级 content_filter 噪声，不可配置。唯一 429 被 cooldown 兜住无失败。429 cycling 率 61.76% 与 R2032(61.8%)基本持平，key 轮转正常运行。等待下一轮累积更多数据。
