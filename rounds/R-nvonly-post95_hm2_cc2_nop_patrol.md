# R-nvonly-post95 — hm2 cc2 NOP 巡检轮

- 时���: 2026-08-02 06:10 CST
- 主仓 git HEAD: dbc7f2c (上轮 post94 已 push NVForge main)
- 本轮 git HEAD: 见 commit (post95 push 后)

## 本轮判定: NOP 巡检轮 (不改码, 不重启)

### 判定依据
1. **cc2 (cc4101-primary) 30min 窗口: 0 req** — session 轮前无流量产生, 无数据可判 cc2 SR.
2. **链路健康无故障**:
   - 容器全 Up: nv_gw/cc4101/nv_gw_stable Up 4h, ms_gw/logs_db Up 2d.
   - `/health` ok: nv_default_model=glm5_2_nv, nv_num_keys=5, pexec=[kimi_nv,dsv4p_nv,glm5_2_nv].
3. **0 cc2 tier error / 0 buffer/wait/error 日志** (cc2 0 req, 无活动).
4. **无新错误类型**.
5. ms_gw fallback 已恢复 (`NVU_DISABLE_MS_FALLBACK=0`, `FALLBACK_UPSTREAM=ms_gw:40007`), 不主动禁用.
...
30min fallback 发生率: f=8 (dsv4p 全挂 fallback ms, fallback 正常工作).
...
### 4. dsv4p_nv per-key
- key 2: 3×200 (avg_dur 14456ms) — 唯一能跑通的 key.
- (空 key): 5×429 — mapped tier 全挂.

### 5. dsv4p_nv per-egress-IP
- 203.10.96.139: 3 成功 — 单 IP (hysteria2 节点) 能通.
- (空): 5 失败 — 多 IP 全挂时无 egress.
...
| git (hermes_improve_self) | dbc7f2c (post94), pulled ✓ |

## 改动
- 0 改动, 0 重启, 0 py_compile.
