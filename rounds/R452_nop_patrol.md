# R452 — NOP 巡检轮 (2026-08-03 03:55 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 17:50-18:20 UTC (与 R448-R451 同窗口延续, 样本 13 req: 6×200 + 6×429 + 1×502).
- cc2 (cc4101-primary) 30min 仍 0 req (cc2 session 间歇空闲, 无评估样本).
- dsv4p_nv 全 caller 30min SR=46.2% (6/13), 7×429 all_tiers_exhausted, 模式与 R268-R451 一致.
- 历史波动区间: R437=85.0% → R438=76.5% → R439=78.9% → R440=78.3% → R441=68.4% → R442=53.3% → R443=57.1% → R444=57.1% → R445=66.7% → R446=66.7% → R447=66.7% → R448=46.2% → R449=46.2% → R450=46.2% → R451=46.2% → R452=46.2%.
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP, all_tiers_failed_in_mapped_tier).
- nv_gw Up 12h, cc4101 Up 2h (本轮未重启).

## 链路数据 (本轮实测 30min 窗口 17:50-18:20 UTC)
...
- 时间分布: 17:55(429×1)/18:00(200×3+429×1+502×1)/18:01(200×3)/18:05(429×1)/18:10(429×1)/18:15(429×1)/18:20(429×1)
- 18:00-18:01 连续 6×200 恢复明确, 末段 18:05-18:20 4×429 离散分布
...
- 错误类型仅 all_tiers_exhausted + 1×502 单点 (不构成新错误模式), 模式与 R268-R451 一致 (一百六十余轮一致).
- dsv4p_nv SR 在 46.2%-89.5% 区间波动, 本轮 46.2% 与 R448-R451 持平, 处于区间下沿但属正常波动.
- 7×429=14/h 高于 5/h 阈值, 但 18:00-18:01 连续 6×200 恢复明确, 整体可接受.
- 1×502 单点 (18:00) 不足构成新错误模式 (连续 R445-R452 各 1×502, 仍 <3/h 阈值, 继续观察).
...
- 留意 502 是否再现 (R445-R452 各 1×单点, 再现 >=3/h 才介入).
