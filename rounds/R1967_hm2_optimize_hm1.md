# R1967 (HM2→HM1): ⏸️ NOP — near-zero post-deploy traffic (~1h), all params floor/optimal, 5 zombie all NVCF-level

## 数据
- 6h: 40 req, 35 OK (87.5% SR), 5 fail (status=502, zombie_empty_completion, all glm5_2_nv)
- Post-R1961 restart (2026-07-19 18:35:33 UTC): nv_gw Up ~1 hour, near-zero post-deploy traffic
...
- glm5_2_nv: 30 req, 25 OK (83.3%), avg=9423ms, max=26165ms, min=3399ms
...
- Container healthy, StartedAt 2026-07-19T18:35:33Z, Up ~1 hour
...
| NVU_BIG_INPUT_COOLDOWN_S | 86400 | — | R1959: 21600→86400. Working: 2 caught in 30min, both peer-fb rescued. | **否决**: 刚部署, 需数据验证 |
...
**NOP判定**: 与 R1960/R1961/R1962/R1963/R1965/R1966 一致 — 近零 post-deploy 流量 (~1h仅2 request) + 5 zombie 全部 NVCF 级 + 24 ATE 全 peer-fb 救援成功 (100%) + 全部参数 floor/optimal。R1959 的 BIG_INPUT_COOLDOWN=86400 初显效果 (2 caught in 30min, 2 rescued, 0 zombie) 但数据量太小。任何改动都是无数据支撑的猜测。

**连续冻结**: 第 7 轮 (R1960→R1967) NOP。
...
## 验证
- ✅ docker ps → nv_gw Up ~1 hour (healthy)
