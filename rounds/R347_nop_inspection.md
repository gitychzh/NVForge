# R347 — NOP 巡检轮 (cc2 0req, dsv4p_nv SR=76.5% 13/17, 根因不变)

## 元
- 轮号: R347 (NOP 巡检轮)
- 时间: 2026-08-02 20:07 CST
- 上轮: R346 (d8e3202, hermes 仓已 push)
- 方向: R-nvonly (HM2 nv_gw 40006, ms_gw fallback 已恢复不主动禁用)

## 本轮改动
- 0 改动 0 restart. NOP 巡检轮.

## 数据依据 (30min 链路分析注入 ~20:07 CST)

### cc2 (cc4101-primary) 30min
- 0 req (session 间歇空闲, 同 R275-R346). 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### dsv4p_nv 30min 全 caller (hermes) SR=76.5% (13/17)
...
- 分钟趋势: 11:40/45/50 三波 429 (3次); 11:55-12:07 恢复 13×200; 12:02 单发 502.
...
- 本轮 1× NVStream_IncompleteRead (502) 是 mid-stream transport 断连偶发单发, R345/R347 出现 R346 清零, 周期性低频不达介入阈值.
