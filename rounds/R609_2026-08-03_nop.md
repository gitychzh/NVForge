# R609 — NOP 巡检轮 (2026-08-03 12:38 CST)

## 链路分析 (注入数据 + 实测复核)
- cc2 (cc4101-primary) 30min: **0 req** (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 22 req, 20×200 + 2×429 (**SR=90.9%**, hermes caller)
  - vs R608 87.5% vs R607 84.6% vs R606 85.2% vs R605 85.2% → 波动区间, NVCF 配额型
- per-key: key2 20×200 (命中可用 key 100% 200, avg_dur=11401ms, IP 203.10.96.139)
- 空 key 2×429 (全挂时 NVCF 侧拒绝, avg_dur=1470ms 快速返回)
- 唯一错误 `all_tiers_exhausted` ×2 (avg_dur=1470ms, NVCF 配额型, 非 nv_gw tier 故障)
- nv_tier_attempts 无 dsv4p 错误行 = KeyManager 全局冷却在 tier 层前拦截
- 无 buffer/wait 日志 (30min 无 buffer 触发), 无 stream_total_deadline, 无 zombie
- finish_reason: tool_calls×16 + stop×4 (健康, 无 zombie stop)
- fallback 发生率 f×22 (cc4101 层 ms_gw 兜底, 预期)
- 配置与 R472-R608 完全一致, 无漂移
...
- dsv4p_nv 22 req: 20×200+2×429 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1470ms 快速返回)
- KeyManager 行为完全正确: 429 cooldown/count decay/reset 按设计工作
- all_tiers_exhausted avg_dur 1470ms (vs R608 12552ms) 下降, 仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort), 非 nv_gw tier 故障
- 无 502 (vs R608 1×502) → peer-fb-skip 慢速返回消失, 全挂时 NVCF 侧快速拒绝
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=90.9% vs R608 87.5% 波动区间内, 与 R545-R608 同一 NVCF 配额波动模式

## 验证
- 0 restart → 无需 py_compile / curl 复测
- 容器实测 (docker ps):
  - nv_gw: Up 22 hours | cc4101: Up 12 hours | nv_gw_stable: Up 35 hours | ms_gw/logs_db: 4 days
  - nv_gw /health: ok (passthrough, nv_num_keys=5)
- 配置与 R472-R608 完全一致, 无漂移
