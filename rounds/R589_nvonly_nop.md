# R589: NOP 巡检轮 (cc2 0 流量, dsv4p_nv 8req 3×200+5×429 SR=37.5% NVCF 配额型)

## TL;DR
继续 NOP 巡检。cc2 0 流量（cc4101-primary 30min 0 req），dsv4p_nv 8req 3×200+5×429 SR=37.5%，
vs R588/R587/R586/R585 均为 37.5% 完全一致，仍在 6h 波动区间 20-55% 内。
命中 key2 时 100% 200 (avg_dur 10675ms)，全挂时空 key 429 = NVCF 配额型波动，非 nv_gw tier 故障。
KeyManager 429 cooldown/decay/reset 按设计工作。无新错误类型，无参数漂移。铁律1 cc2 视角不满足 → 不动码。

## 一、轮前链路分析 (2026-08-03 10:58:32 CST 注入)

### 1.1 30min cc2 (cc4101-primary) — 0 req
session 间歇空闲，无 cc2 评估样本。铁律1 不满足。

### 1.2 30min dsv4p_nv — 8 req, SR=37.5%
| 维度 | 值 |
|------|----|
| 总量 | 8 req (3×200 + 5×429) |
| SR | 37.5% |
| 唯一错误 | all_tiers_exhausted ×5 (avg_dur 1463ms) |
| per-key | key2: 3×200 (avg_dur 10675ms) ; 空 key: 5×429 |
| per-egress-IP | 203.10.96.139: 3×200 (100%) ; 空 IP: 5×0 |
...
02:46|429|1
02:51|429|1
02:56|200|3   ← 命中可用 key 时连续 3×200
```
→ 5min 间隔单次 429 + 末段连续 200 = NVCF 配额型节流模式, 非 tier 故障。
...
### 2.2 KeyManager 行为
- 全挂时 `all_tiers_exhausted` (avg_dur 1463ms) = 5key 全冷却, NVCF 侧拒绝
- 命中 key2 时 100% 200 (avg_dur 10675ms) = key 可用即成功
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=37.5% vs R588 37.5% vs R587 37.5% vs R586 37.5% vs R585 37.5% vs R584 71.4% → 波动区间内, 与 R545-R588 同一 NVCF 配额波动模式
