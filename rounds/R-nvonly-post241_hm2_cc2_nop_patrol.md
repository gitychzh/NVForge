# R-nvonly-post241 hm2_cc2 — NOP 巡检轮 (2026-08-02 13:03 CST)

## 本轮结论: NOP 巡检轮, 0 改动 0 重启

cc2 (cc4101-primary) 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
链路健康无故障: nv_gw /health ok, 全容器 Up 11h+, 0 cc2 tier/buffer/wait/error 日志.
hermes 打 dsv4p_nv 18req (16×200/2×429, SR=88.9%, all_tiers_exhausted×2 NVCF 配额限流) — 与 cc2 无关.
...
- per-key: key2 扛 16×200 (avg_dur 9641ms), 2×429 来自无 key 映射 (empty key).
- per-egress: 203.10.96.139 扛 16×200 (100% SR), 2×429 来自无 egress.
- finish_reason: tool_calls×12, stop×4 (无 zombie).
- 按分钟趋势: 04:35/04:40 各 1×429 (限流), 04:45-05:01 16×200 (恢复).
- fallback: f×18 (无 fallback 触发).

### 3. 30min 错误分类
- all_tiers_exhausted (hermes→dsv4p_nv): 2× (avg_dur 1645ms), NVCF 配额限流, 非 cc2 链路.

### 4. tier 错误 (nv_tier_attempts 30min) — 0 rows
### 5. buffer/wait 日志 (cc2) — 空
