# R-nvonly-post20 — NOP 巡检轮 (hm2_cc2)

**日期**: 2026-08-02 02:09 CST (链路数据时间戳)
**主仓 HEAD**: bb38b41 (含 R40005 稳定版 40005 兜底容器, 本轮 commit 待 push)
**容器**: nv_gw Up 6m, cc4101 Up 6m, logs_db (healthy)
...
- tier 错误 (nv_tier_attempts 30min): 仅 1× `429_nv_rate_limit` 在 key2 (dsv4p_nv 流量)
- 三阈值全绿: cc2 SR=100%, 无新错误类型, 无 transport 异常
- post17→post18→post19→post20 连续满分 (glm5_2_nv 健康 tier)

### 非 cc2 流量 (hermes/openclaw caller 打 dsv4p_nv)
- dsv4p_nv SR=61.5% (16/26): 10× `all_tiers_exhausted` (5key 全挂) + 4×502
- per-key dsv4p: key3 8×200(11502ms), key2 6×200(7166ms), key0/key1 各 1×200; 但 6×429 + 4×502 无 key 归属 (全挂后 all_tiers_exhausted)
- per-egress-IP: 134.195.101.194 (8/100%), 203.10.96.139 (6/100%), 其余 1+1
