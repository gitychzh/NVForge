...
才评估 collect 中断触发 buffer 重试. R651-R654+R660 五轮复核, 150min 内始终只有 1 个
request_id (c1297569, 单次事件 07:20:50), 阈值未触发, 不改码.
...
- **现象**: `execute_request` 成功 (NVCF 连上 + 流开始) 但 `_collect_stream_to_anth` 读流途中
  因 `IncompleteRead`/`RemoteDisconnected`/`ConnectionResetError`/`OSError`/`timeout` 中断
  → 直接 502, 不触发 buffer 5key 重试.
- **为何不触发**: handlers.py:910 非流式 buffer retry 条件是 `not result.success`
  (execute_request 失败). collect 中断发生在 `execute_request` 成功**之后**,
  与 :910 条件互���, 落在盲区.
- **根因**: NVCF 侧流中途断 (98ac8c50 613bytes / bcc03ca7 类似), 非 nv_gw 逻辑缺陷,
  但单次 502 对 cc2 可用性有损 (cc2 9/10=90.0% → 若 collect 中断被 buffer 救回则 10/10=100%).

### 改动 (最小侵入, 镜像现有 NV-NONSTREAM-BUFFER-RETRY 模式)
**文件**: `/opt/cc-infra/proxy/nv-gw/gateway/handlers.py` (备份: `handlers.py.bak.R661`)

1. **签名加守护参数**: `_collect_stream_to_anth(..., _collect_buf_retried=False)`
   - 防递归 (虽然 buffer 内部 _synthesize_nonstream_json 不回调 collect, 双保险).

2. **插入 transport-interrupt buffer retry 块** (handlers.py:1853, 在 R1716 ms_gw fallback 块之前):
   - **触发条件** (窄, 仅传输中断类, 不误伤 zombie/empty/content_filter):
     - `_collect_err.startswith("NVAnthCollect_")` 且 `!= "NVAnthCollect_"`
     - `oai_body is not None`
     - `not _collect_buf_retried`
     - `caller in NVU_BUFFER_CALLERS` (cc4101-primary,openclaw2)
   - **动作**: 清本次 collect 残留 error 状态 → `BufferStreamSession(is_nonstream=True).run()`
     重新跑整个 NVCF 5key 请求 → 成功则 buffer 内部 `_synthesize_nonstream_json` + `_send_json` 给 CC.
   - **失败兜底**: buffer 全败 → 恢复 error_type, 落下方 R1716 ms_gw fallback 块 (ms disabled 不触发)
     → 最终 `_send_json(502)`.
   - **与 ms_gw fallback 关系**: buffer retry 在前, ms fallback 在后 (符合层级: NVCF keys 优先, ms 最后).
     当前 `NVU_DISABLE_MS_FALLBACK=1` / `NVU_MS_FALLBACK_ENABLED=0` → ms 块不触发, buffer 是唯一重试.
   - **buffer 内部**: `BufferStreamSession.run()` 自己处理 5key 轮转 + backoff + wait_queue +
     ms_gw gate (NVU_DISABLE_MS_FALLBACK=1 跳过 ms) + 失败发 SSE error. 非流式 `is_nonstream=True`
     走 `_synthesize_nonstream_json` → `_send_json(200, json)`.
...
- `docker logs nv_gw --since 1m` → **无 error/traceback/import 失败**, 启动正常 (ProbeWorker started)
- 配置无漂移: `NVU_DISABLE_MS_FALLBACK=1`, `NVU_BUFFER_CALLERS=cc4101-primary,openclaw2`
- **下窗口监控**: 等待 cc2 路径 IncompleteRead 再现 → 应触发 `NV-ANTH-COLLECT-BUFRETRY` 日志,
  buffer 接管后要么 200 (救回) 要么 `NV-ANTH-COLLECT-BUFRETRY-FAIL` (buffer 也败).
...
- **持续观察点**:
  1. **重点**: cc2 路径 IncompleteRead 再现时是否触发 `NV-ANTH-COLLECT-BUFRETRY` → 200 救回
     - 若 buffer 救回 → cc2 SR 从 90% 升至 100%, 改动生效
     - 若 buffer 也败 (`NV-ANTH-COLLECT-BUFRETRY-FAIL`) → 说明 NVCF 侧持续劣化, 需更深排查
  2. dsv4p_nv 配额型 429 全挂 (持续, NVCF 无 retry-after 头)
  3. deadline 链健康 (6h=0), 持续监控
  4. 150min 回查 IncompleteRead 是否回归单次事件 (改后应减少, 因 buffer 救回不再落 502)
...
1. **窄触发**: 仅 `NVAnthCollect_*` 传输中断类错误触发, 不误伤 zombie/empty/content_filter
   (那些是模型端问题, 重试同请求无收益, 留给 R1716 ms fallback 块或直接 502).
2. **双守护**: `_collect_buf_retried=True` 防递归; buffer 内部 `_synthesize_nonstream_json`
   不回调 `_collect_stream_to_anth` (无递归路径).
3. **层级正确**: buffer retry (NVCF keys) 在 ms_gw fallback 之前, 符合"NVCF 优先, ms 最后"设计.
4. **失败兜底**: buffer 全败 → 恢复 error_type → 落 R1716 块 (ms disabled 不触发) → `_send_json(502)`.
   即改动最坏情况 = 退化到改前行为 (502), 不会更差.
5. **幂等**: buffer 用原 `oai_body` 重发, NVCF 是无状态推理, 重发安全 (CC 侧 cc4101 非流式
   等单一 JSON 响应, buffer 成功即 `_send_json` 一次, 无重复响应).
