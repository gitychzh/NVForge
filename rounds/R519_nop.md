# R519 — NOP 巡检轮 (2026-08-03 06:18 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (全新 session, 低谷窗口延续, R503-R518 同窗口周期).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=37.5% (3/8: 3×200 + 5×429), 与 R510-R518 同窗口周期性行为 (实测 06:18 CST).
- 错误: all_tiers_exhausted ×5 (30min) / ×21 (2h), 历史 R268 起一致. zombie_empty_completion ×1 (2h, 远低于 ≥3/h 阈值).
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截).
- 配置实测确认与 R475-R518 完全一致, 无漂移.

## 链路数据 (06:18 CST 实测)
### 30min 窗口 (全 caller, dsv4p_nv)
- 8 req: 3×200 (avg_dur=9568ms, nv_key_idx=2, egress=203.10.96.139), 5×429 (空 idx, 空 IP, avg_dur=1674ms) → SR=37.5%
- per-caller: hermes|429×5, hermes|200×3 (仅 hermes caller, cc4101-primary 0 req)
...
- 30min 0 req, 2h 0 req (session 间歇空闲, 实测 0 rows)

### KeyManager 行为 (历史一致)
- 429 全在 dsv4p_nv tier, k1-k5 全挂, 单次 429 即触发全局冷却.
- tier=dsv4p_nv 只 1 tier 无 ring fallback → all_tiers_exhausted 直接 abort.
- 历史一致行为 (R268 起 200+ 轮), 非本轮新故障.

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型 all_tiers_exhausted ×5, 模式与 R268-R518 一致, 无新错误.
- zombie_empty_completion ×1/2h (远低于 ≥3/h 介入阈值, 不动).
- dsv4p_nv 30min SR=37.5% (与 R510-R518 同窗口周期性行为, 低谷窗口, 22:10 后 200 回升).
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R475-R518 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R476/R480-R518 记 6h 低频 zombie 502, 再现 >=3/h 才介入).
