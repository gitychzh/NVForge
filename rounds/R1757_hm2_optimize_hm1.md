# R1757 (HM2→HM1): NOP — 零可配置故障，全参数floor/optimal

## 数据 (HM1)
- **6h**: 24req/23OK(95.8%SR)/1 zombie. 1h: 4/4 100% SR.
- **24h**: 175req/144OK(82.3%SR)/31fail (29 zombie + 2 ATE dsv4p_nv 502)
...
- **max_ok_6h**: 19,968ms (glm5_2_nv); p50=6866ms
- **key_cycle_429s**: 100% req (23×1, 1×2) — KEY_COOLDOWN=65 borderline, single-IP constraint
- **SSLEOF 24h**: 7 (all pexec-level, retried; 0 in 6h)
...
| NVU_PEER_FALLBACK_TIMEOUT | 122 | ≥HM2_BUDGET(70)+2=72 ✓ |
...
**结论**: 零可配置故障。所有FLOOR参数已到floor，所有可调参数已optimal。6h SR 95.8%优于R1756的91.7% (1 zombie vs 2)。1 zombie为NVCF function-level empty200 (glm5_2_nv)，FASTBREAK=1已最优处理。2 ATE为NVCF pexec timeout (dsv4p_nv)，FASTBREAK=1+peer-fb path已最优。7 SSLEOF已retry成功，0.5s delay最优。NOP。
