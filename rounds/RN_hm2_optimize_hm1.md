# R793: HM2→HM1 — NOP — 90.8% SR, 双向fallback 100%, 全参数floor, 零单tier ATE

**时间**: 2026-07-06 21:57 UTC  
**分析窗口**: 6h (15:57–21:57 UTC)  
**决策**: NOP — 零参数变更，零容器重启
...
| 指标 | 值 | 判定 |
|---|---|---|
| **6h SR** | 260req/236OK (**90.8%**) | 与R792的90.7%一致，正常波动 |
| **ATE** | 24 (9.2%), 全部tiers_tried_count=2 | NVCF双tier真实耗尽 |
| **Fallback SR** | 52/52 **100%** | 双向完美 |
| **dsv4p_nv** | 115req/99OK (86.2%) | 健康 |
| **glm5_2_nv** | 143req/135OK (94.4%) | 健康 |
...
- 113/184 (61.4%)非fallback请求在≤30s内完成
- 28/52 fallback请求超过90s（双tier重试正常延迟）
...
### 日志关键事件（最后100行）
- 3次 dsv4p→glm5_2 fallback全部成功 (21:01, 21:04, 21:05 UTC)
- glm5_2→dsv4p fallback 双向活跃
- BrokenPipeError: 1次 (client断开，非gateway故障)
- 无任何 `(no fallback, 3model)` 标记

### 20:00 hour 瞬时ATE spike (8/15=53.3% SR)
- 3个记录捕获: 2个dsv4p_nv ATE (175s, 178s), 1个glm5_2_nv ATE (122s)
- 全部tiers_tried_count=2, 双tier真实耗尽
- NVCF上游瞬时拥塞，非配置问题
...
- 24 ATE全部归因于NVCF双tier上游耗尽
- UPSTREAM=66 buffer 14.4s — 充裕，既不需增(已非绑定)也不需减(降buffer无益)
- Fallback链路100%可靠，52次fallback全部成功
- 所有参数已在floor最优值，无进一步下调空间
- 90.8% vs R792's 90.7% — 正常6h窗口波动
- 20:00 hour瞬时ATE spike为NVCF上游拥塞，非配置可解

## NOP streak
R788 → R789 → R790 → R791 → R792 → **R793**: 连续6轮 NOP，系统进入自维持健康regime
