# R1677: HM2→HM1 — NOP — KEY_COOLDOWN=55消除pexec_429持续~18h+零429, zombie-dominated steady state, 全部参数floor/optimal, 零参数变更

**决策**: NOP — 零参数可修复问题。R1668 KEY_COOLDOWN=55 持续消除 pexec_429 (~18h+ 零 429)。zombie_empty_completion 是 NVCF 上游流级别内容过滤，网关不可配置。全部参数触底/最优。
...
- **Success latency**: avg 10,295ms (glm5_2_nv pexec)

## 24h Data
- **Total**: 354 req, 192 OK / 162 fail → **54.2% SR**
- **130 zombie_empty_completion** (80.2% of all failures)
- **32 all_tiers_exhausted** (19.8%) — 全部在 R1668 部署前/早期 (最后 ATE 在 2026-07-16 18:00 UTC, ~14h ago; 6h 零 ATE)
- **90 pexec_429**: 24h tier_attempts — 全部集中在 2026-07-16 10:00-12:00 UTC (~18h ago), 之后零 pexec_429
...
| 6h (R1674) | 28 | 0 | 0.0% |
| 6h (R1673) | 26 | 0 | 0.0% |
| 24h (R1668-R1677) | 297 | 90 | 23.3% (all pre-18h ago) |

KEY_COOLDOWN=55 部署后 pexec_429 从 22.4% 降至 0% 持续 ~18h+。24h 内 90 pexec_429 全部在 2026-07-16 10:00-12:00 UTC，之后零。
...
2. **KEY_COOLDOWN=55 effectiveness:** R1668 部署后 pexec_429 从 22.4% 降至 0% 持续 ~18h+。24h 内 90 pexec_429 全部在 2026-07-16 10:00-12:00 UTC (~18h ago)，之后持续零。这是当前优化周期中最显著的成果。

3. **0 ATE:** FASTBREAK=3 + EMPTY_200_FASTBREAK=3 持续消除 ATE。6h 内零 ATE，32 ATE 在 24h 全部在 ~14h 前。
...
### R1668 成果确认 (持续观测)
KEY_COOLDOWN 60→55 是正确方向。~18h+ 零 pexec_429 证明 55s 已足够 NVCF rate-limit 恢复。这是当前优化周期中最重要的成果。

### R1676 vs R1677 对比
| Metric | R1676 | R1677 | Δ |
|--------|-------|-------|---|
| 6h Total | 28 | 28 | 0 |
| 6h SR | 60.7% | 60.7% | 0 |
| zombie | 11 | 11 | 0 |
...
## 铁律
NOP 轮。铁律：只改 HM1 不改 HM2。KEY_COOLDOWN=55 消除 pexec_429 是 R1668 重大成果，持续 ~18h+ 零 429。zombie_empty_completion 是 NVCF 上游问题，网关不可配置。全部参数已在 floor/optimal 状态。下次轮到 HM2 时重评估：关注 zombie 率是否变化，关注 dsv4p_nv 流量是否恢复，关注 NVCF ai-glm-5_2 function 健康状况。
## ⏳ 轮到HM1优化HM2
