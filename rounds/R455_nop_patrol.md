# R455 — NOP 巡检轮 (2026-08-03 02:36 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 18:00-18:30 UTC (与 R454 延续, 9 req: 4×200 + 5×429).
- cc2 (cc4101-primary) 30min 仍 0 req (cc2 session 间歇空闲, 无评估样本).
- dsv4p_nv 全 caller 30min SR=44.4% (4/9), 与 R454 (57.1%) 基本持平量级, 仍处 46.2%-85% 历史波动区间下沿.
- 错误: all_tiers_exhausted ×5 (all_tiers_failed_in_mapped_tier, 5key 全挂, 空 IP), 模式与 R268-R454 一致.
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP).
- nv_gw Up 12h, cc4101 Up 2h (本轮未重启).

## 链路数据 (注入窗口 18:00-18:30 UTC, 实测复跑确认)
- caller×model×status: hermes|dsv4p_nv 4×200+5×429
- dsv4p_nv SR=44.4% (4/9)
- 错误分类: all_tiers_exhausted|all_tiers_failed_in_mapped_tier ×5 (avg_dur 1674ms)
- cc4101-primary 30min: 0 req
- 时间分布: 18:05(429×1)/18:10(429×1)/18:15(429×1)/18:20(429×1)/18:25(200×1)/18:26(200×3)/18:30(429×1)
- 18:25-18:26 连续 4×200 恢复明确, 其余 5×429 离散分布 (每 5 分钟 1 个)
- per-key: key2 4×200 avg 12870ms; 空 key 5×429
- per-egress-IP: 203.10.96.139 4×100%; 空 IP 5×失败
- 200 延迟: avg 12870ms, max 21640ms, min 4991ms, avg_ttfb 12409ms
- finish_reason: tool_calls×3, stop×1 (无 zombie, 全部正常结束)
...
## 实测复跑确认 (02:36 CST)
- nv_requests 30min: 4×200 + 5×429 (与注入 9 req 一致)
- nv_requests cc4101-primary 30min: 0 rows (确认 cc2 0 流量)
- error_type 30min: all_tiers_exhausted ×5 (唯一类型)
...
## 历史波动区间 (R437-R455)
R437=85.0 → R438=76.5 → R439=78.9 → R440=78.3 → R441=68.4 → R442=53.3 → R443=57.1 → R444=57.1 → R445=66.7 → R446=66.7 → R447=66.7 → R448=46.2 → R449=46.2 → R450=46.2 → R451=46.2 → R452=46.2 → R453=62.5 → R454=57.1 → R455=44.4%

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted, 模式与 R268-R454 一致 (一百六十余轮一致), 无新错误.
- dsv4p_nv SR 44.4% 与 R454 (57.1%) 基本持平量级, 处于 46.2%-85% 历史波动区间下沿, 属正常波动.
- 5×429 离散分布 (每 5 分钟 1 个), 但 18:25-18:26 连续 4×200 恢复明确, 整体可接受.
- 无 502 (R454 有 1×单点, 本轮无), 错误更纯净.
...
- 留意 502 是否再现 (R445-R454 各 1×单点, 再现 >=3/h 才介入).
