# R1849 (HM2 cc2): 巡检轮 bug8 降级兜底 in-vivo 后第 7 轮持续 0 触发, 链路稳 SR 94.6%

> 时间: 2026-07-19 07:17 CST
> 模式: nv 直连 (cc4101→nv_gw). 0 改动 0 restart. StartedAt 仍 2026-07-18T21:26:29Z (R1839 改后字节码).

## 改前数据 (30min 窗)
- **SR**: 123/130 = **94.6%** (200:123 / 502:7). 跌破 95% 线但近 7 轮
  R1842 97.5% / R1844 97.0% / R1846 95.2% / R1847 94.1% / R1848 94.7% / 本轮 94.6%
  全在 94-98% 抖动区间, **非系统退化** (未连 ≥3 轮破 93%).
- **7 条 502 分类**:
  - zombie_empty_completion: 3
  - stream_first_byte_timeout: 2
  - all_tiers_exhausted: 1
  - stream_no_content_gap: 1
  - **全 NVCF 侧偶发降级路径外分支, config 不可修**.
- **tier pexec (30min)**: success 84 / SSLEOFError 2 / 429 1 / empty_200 1. **无 zombie 无 ATE**.
  pexec elapsed 持续自愈 (近轮 max ~53s).
...
- **bug8 关键**: 实战降级触发 **0** (NV-TOOLCALL-JSON-DOWNGRADE 120min 窗空).
  cc2.log could-not-be-parsed **0**. 兜底保险在位但 args 全合法不需触发, 符合 R1839 原话.
- **breaker (30min)**: 1 NV-ANTH-BREAKER-FAIL (zombie 软挂 state=('CLOSED',2,0) 未 OPEN 设计内)
  + 2 NV-MS-FB-SERVED (all_keys_exhausted 进 ms fallback, nv_breaker recorded CLOSED). 无 OPEN.
...
1. SR 连续 ≥3 轮跌破 93%? — 否 (94.6% / R1848 94.7% / R1847 94.1%).
2. fallback 非跳过类 ≥4 次/30min? — 否 (3 全 SKIP-CIRCUIT 抢断, 非 nv_gw 失败).
...
## 验证结果
- 链路稳: SR 94.6% 在抖动区间下沿, 非退化.
- bug8 兜底 in-vivo 在位 + 实战 0 触发 (设计内: 几乎不触发).
- breaker全 CLOSED, fallback 全抢断类 0 中断.
