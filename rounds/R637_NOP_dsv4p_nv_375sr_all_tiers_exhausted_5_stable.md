...
- dsv4p_nv 30min (hermes caller): 8 req, 3×200 + 5×429 (SR=37.5%)
  ...
  - 200 avg_dur=9634ms (健康, 持平 R634/R635/R636 9634ms)
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×5 (avg_dur 1733ms: 429 快速 ABORT)
    → R624→...→R633→R634→R635→R636→R637: 3→3→3→3→3→4→5→5→5→6→5→5→5→5 (持平正常区间 3-6, 本轮持平 R634/R635/R636)
...
## 持续观察点 (沿用 R636, 不改码) — dsv4p_nv 配额型 429 全挂
- 6h SR 波动 29-87%, 04:00 回升 86.8% → 配额型故障模式确认, 非 nv_gw 侧可改
- 01:00 单小时 SR=29.4% 触及 R621 升级阈值 (SR<55%), 但:
  - cc2 视角 0 样本 (铁律1 不满足)
  - 非持续 (04:00 回升至 86.8%)
  - → 不触发回切 PRIMARY glm5_2_nv
- KeyManager 指数退避正确 (429 count decay >300s → reset → count=1→180s, count=2→180s, =3→480s)
  - 日志铁证: 14:20/14:25 [NV-KEYMGR] 429 tier=dsv4p_nv k1-k5 count decayed (>300s) resetting → cooldown=180s
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R636)
- dsv4p_nv SR=37.5% (8req 小样本), 持平 R634/R635/R636 连续 3 轮, 仍在 R617-R636 正常波动区间下沿 (37-91%), 趋势未恶化
- 6h SR 趋势 29-87% 波动, 04:00 回升 86.8% → 配额型故障模式确认
- all_tiers_exhausted ×5 (R624-R637: 3→...→6→5→5→5→5), 持平 R634/R635/R636 仍在正常波动区间 3-6, 无退化
- KeyManager 指数退避正确, 429 ABORT avg 1733ms (快速, 持平 R636 1412ms 量级)
...
- KeyManager 日志铁证: 14:20/14:25 429 count decay + 指数退避正确工作

## 下一步
- dsv4p_nv SR 本轮持平 R634/R635/R636 (37.5%), 仍在正常波动区间下沿, 小样本波动无恶化
- **升级标注解除** (R621 设定 SR<55% 或 exhausted>=8 → 切 PRIMARY 回 glm5_2_nv):
  - 01:00 单小时触及 SR<55% (29.4%), 但非持续 (04:00 回升 86.8%), cc2 视角 0 样本 → 不触发
