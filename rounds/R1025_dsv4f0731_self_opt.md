# R1025: RemoteDisconnected 风暴延续 (第4轮) — 模型特异性劣化持续, 无参数修改 (恢复观察轮)

> 时间: 2026-08-05 09:35 BJT (01:35 UTC)
> 容器: dsvf0731_nv40666 (端口 40666, HM2, DeepSeek V4 flash via NVCF)
> 状态: **恢复观察轮 (无参数修改)** — NVCFPexecRemoteDisconnected 远程瞬断持续绝对主导,
>   且第 4 次复现**模型特异性** (同容器同 key 同出口 glm5_2_nv 100% 成功) → NVCF
>   deepseek-v4-flash function 级劣化, 无本容器可解参数
> Fallback: hm4104 持续 fallback 到 dsv4f0731_ms (primary 502/RemoteDisconnected 触发)

## 1. 背景 (改前必有数据)

R1021/R1022/R1023/R1024 已连续记录同一 RemoteDisconnected 风暴并判定模型特异性 NOP。
本轮 (间隔 ~35min) 为**同一风暴第 4 轮延续**, 数据完全复现 R1024 结论。

### 30min 窗口 — nv_requests (采集脚本, tier_model=dsv4f0731_nv)
- 总量 9, 200=2, 502=7, **SR=22.2%** (窗口内极少量业务流量, 波动大)
- Avg/P50/P95: 133220ms / 75054ms / 135049ms
- 429: 0 (key_cycle_429s 全 0, 非 429 主导)
- upstream_type: nvcf_pexec 9/200=2 (全 pexec, integrate 0)

### 1h tier_attempts 失败细分 (per-attempt 层, dsv4f0731_nv)
- **NVCFPexecRemoteDisconnected: 33 (绝对主导)**
- empty_200: 4
- 成功标记: 无 (tier_attempts 表当前无 dsv4f0731_nv 成功记录)

### ⭐ 关键证据 — 模型特异性第 4 次复现 (同容器同窗 1h, 请求层)
| tier_model | total | ok | SR |
|---|---|---|---|
| glm5_2_nv    | 149 | 149 | **100%** |
| dsv4f0731_nv |  22 |   8 | **36.4%** |

- 同一容器、同一 5 组 NVU_KEY、同一 5 个 mihomo 出口 (7894/7895/7896/7897/7904)
- glm5_2_nv 同链路 100% 成功 → 故障隔离到 deepseek-v4-flash 的具体 NVCF function,
  非网络/key/出口/本容器参数问题 (证据第 4 次成立, 变量隔离干净)

### per-key 均匀劣化 (非单 key 问题)
- 错误全部集中在 all_tiers_exhausted (每个请求烧尽 5 key), 无单 key 主导
- key_cycle_429s: 0=8, 1=1 (非 429 主导)

### 6h 逐小时 trend (请求层, 采集脚本)
| hour (UTC) | total | 200 | 502 | SR |
|---|---|---|---|---|
| 22:00 | 26 | 21 | 5 | 80.8% |
| 23:00 | 35 | 20 | 15 | 57.1% |
| 00:00 | 32 | 20 | 12 | 62.5% |
| 01:00 |  9 |  2 |  7 | 22.2% |

- 22h 后 SR 持续走低 (80.8% → 22.2%), RemoteDisconnected 风暴未缓解, 反而加剧
- 3h 累积: 76 请求 42 成功 (55.3%)

### 24h all_tiers_exhausted
- 24h 内 103 次 (R1024 采集时刻 101, 持续累积)

### hm4104 fallback 日志 (最近 5min)
- PRIMARY-FAIL-STREAM: nv_gw 流式 502 after 71556ms → 切 fallback
- PRIMARY-BREAKER-SKIP-STREAM: primary 跳过 (circuit OPEN) → 直走 fallback
- FALLBACK-STREAM: 多次从 primary 切到 ms_gw 流式
- 表明 primary dsv4f0731_nv 当前断连多, hm4104 已大量 fallback 到 ms_gw

## 2. 决策: 无参数修改 (恢复观察轮)

**依据:**
1. **模型特异性第 4 次复现** — glm5_2_nv 同容器同 key 同出口 100% 成功, 仅
   deepseek-v4-flash function 失败。这不是 mihomo/网络/出口/key 问题 (否则所有模型都挂)。
2. **远程 TLS-EOF 非任何容器参数可消除** — up/downstream timeout、key cooldown、budget、
   fast-break 均无法把 NVCF 侧 reset 连接变成成功。
3. **5 key 均匀失败** — 无单 key 劣化, 排除 key 级参数 (cooldown/429 管理) 可优化空间。
4. **改前必有数据**: 无单一参数有数据支撑可提升 SR。glm5_2_nv 100% 已把变量隔离干净。
   改任何 cooldown/timeout 只会徒增归因噪声。

## 3. 当前状态 (30min 主指标)

- 30min SR: **22.2%** (2/9), 较 R1024 46.2% 进一步回落 (风暴加剧 + 窗口内业务量极少)
- Avg/P50/P95: 133220ms / 75054ms / 135049ms
- 错误分布 (per-attempt 1h): NVCFPexecRemoteDisconnected=33, empty_200=4 (dsv4f0731_nv)
- 请求层: all_tiers_exhausted (502) 30min=7, 24h=103
- 429: 0 (非 429 主导, key_cycle_429s 全 0)
- upstream: pexec 9/200=2, integrate 0 (NV_KEY_INTEGRATE_KEYS 空)
- fallback: hm4104 大量 fallback 到 ms_gw (circuit OPEN + 502)

## 4. 上次修改效果 (R1024 观察轮)

- 无参数修改, 无参数效果可评
- SR 46.2% → 22.2% (30min 窗口), 为风暴自然波动/加剧, 非任何参数效应
- 模型特异性证据第 4 次成立 (glm5_2_nv 100%), 5 key 均匀劣化确认

## 5. 下一步建议

1. **若 RemoteDisconnected 持续 >60% 且仅 dsv4f0731_nv 受影响**: 确认是 NVCF
   deepseek-v4-flash function 上游劣化, 无容器参数可解。
2. **若持续恶化**: 评估架构层缓解 — 备用 NVCF function_id / 切换到 peer 的 function /
   或依赖 hm4104→ms_gw fallback (当前最稳路径)。
3. **下一轮触发条件**: SR≥95% 且 RemoteDisconnected 消失 → NOP; 若出现**单 key** 劣化
   (非均匀跨 key) → 才考虑 key 级参数; 若 storm 持续 → 维持观察, 等待 NVCF function 恢复。

## 验证清单
- [x] /health 未改动 (无参数修改, 无需重启)
- [x] 数据完整: 30min SR/延迟/错误分布/fallback/per-key 均已采集
- [x] 决策数据驱动: 模型特异性 (glm5_2_nv 100% vs dsv4f0731_nv 36.4%) 证实 function 级劣化 → NOP