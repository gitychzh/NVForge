...
## 摘要
- NOP 巡检轮, 0 改动 0 restart. cc2 (cc4101-primary) 30min 无请求 (session 间歇空闲).
- DB 快照 (01:15): dsv4p_nv 全 caller 30min SR=63.6% (7/11), 略低于 R429 的 69.2%, 仍在 NVCF function 配额波动区间.
  - cc2 primary: 30min 0 req (caller × model × status 中无 cc4101-primary 行).
  ...
  - per-egress-IP: 203.10.96.139 6/6=100%, 134.195.101.194 1/1=100%, 空 IP 4req 全 429 (0%).
  - 200 延迟: avg 10439ms, max 15976ms, min 3382ms, avg_ttfb 9740ms (正常波动).
  - finish_reason: tool_calls×4, stop×3.
- 30min fallback: 0 发生 (f×11).
- 30min nv_tier_attempts: 0 行 (buffer caller 不走 mapped-tier).
- 30min buffer/wait/keymanager 日志: 无 (buffer caller 本轮无请求触发).
- 错误类型无新增, 与 R268-R429 一致 (一百五十余轮一致, 仅 all_tiers_exhausted).
- 链路自恢复 (ProbeWorker + KeyManager decayed reset + buffer 5key 轮转) 持测有效.
- 容器健康: nv_gw (23h), cc4101 (11h), ms_gw, logs_db Up.

## 判稳
- **NOP 巡检轮**. cc2 primary 30min 无请求, 无错误无 fallback.
- dsv4p_nv 本轮快照 SR=63.6% (7/11), 在历史波动区间内
  (R420=86.4%, R421=76.5%, R422=76.5%, R423=61.5%, R424=33.3%, R425=25.0%, R426=25.0%, R427=54.5%, R428=54.5%, R429=69.2%, R430=63.6%).
- dsv4p 错误类型无新增, 与 R268-R429 一致 (一百五十余轮一致).
- 切换 PRIMARY_UPSTREAM_MODEL 到 glm5_2_nv 仍是大改: glm5_2_nv 30min 0 req,
  无 buffer 路径数据支撑, 不满足"改前必有数据"铁律 → 暂不切.

## 根因 (沿用 R278-R429, 非代码缺陷)
...
- cc2 缓冲 caller 走 buffer 路径不走 mapped-tier, 不受影响 (本轮 0 req 触发, 无数据).
