# R1022 — cc2 NOP 巡检轮 (不改码)

**轮次**: R1022 | **日期**: 2026-08-07 | **类型**: NOP (主链连续第 130 轮 100% 干净)

## 判定
**NOP**: cc2 主链路 cc4101-primary 连续第 130 轮 (R893-R1022) 100% SR 干净,
主链专属错误 0 rows, fallback 0 次, 无新错误类, 无 key 疲劳. **不改码.**

## 依据 (live 复核 + 注入轮前链路分析 2026-08-07 16:39 CST)

### 注入轮前链路分析 (16:39 CST)
- 30min 链路总览: `cc4101-primary|dsv4f0731_nv|200|111`, `hermes|dsv4f0731_nv|200|19`, `hermes|dsv4f0731_nv|502|4`.
- 30min 按模型: `dsv4f0731_nv SR=97.0% (130/134)`, 主链 111/111 全 200 无 bad.
- 错误分类 (status!=200): `NVStream_IncompleteRead|2`, `all_tiers_exhausted|1`, `stream_absolute_cap|1` — 全属 hermes.
- nv_tier_attempts per-key: 各 key 大量 `pexec_success` (20-24), 偶发 `NVCFPexecRemoteDisconnected` (2-3/key)
  与 `NVCFPexecTimeout` (1/key), 全被 buffer 吸收, 未穿透 caller.
- 无 buffer/WaitQueue/cooldown 异常日志 (buffer attempt=1 全 success).

### live 复核 (本轮)
- `SELECT caller,status,count(*) ... GROUP BY 1,2` → **cc4101-primary 112/112 全 200 = 100% SR, 0 bad**;
  hermes 200=18, 502=4.
- `SELECT caller,error_type,status ... status!=200` → 30min 内全部 bad 全属 hermes
  (NVStream_IncompleteRead 2 / all_tiers_exhausted 1 / stream_absolute_cap 1, 均 502), **主链 0 rows**.
- cc_requests 30min = 112 request, fallback_triggered = **0** (0.0%).
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **112/112 = 100% SR, 0 bad** (live) | ✅ |
...
本轮 4 条 bad (502) 全属 hermes 越界宿主 (NVStream_IncompleteRead 2 / all_tiers_exhausted 1 / stream_absolute_cap 1),
