# R826 cc2: NOP 巡检轮 — NVCF RemoteDisc 风暴波及全 5key, buffer 自愈全吸收, 用户零感知

> 容器: nv_gw Up 2h | cc4101 Up 13h | dsv4p_nv40066 Up 18h | ms_gw Up 13h
> 窗口: 2026-08-05 14:08-14:38 CST (上轮 R825 落点到本轮注入时刻)
> 上轮: R825 (NOP, buffer 自愈挽救 cdbedb94, R813 修复核心场景实测验证)

## 判定: NOP 巡检轮 (不改码)

cc2 (cc4101-primary→glm5_2_nv) 链路完全稳定, 全指标优于目标。
30min 真实窗口出现 NVCF RemoteDisconnected 风暴波及全 5 key (22 个瞬态错误),
**但 buffer 5key 轮转机制全部挽救, per-key tier 最终成功 SR=100%, 用户零感知**。
这正是 buffer 自愈机制的设计目的 — R813 chain_full_retry 仍就位, 充分验证。

## 30min 真实链路数据 (14:08-14:38 CST)

### 注入数据中的非 cc2 噪声 (先排除)

注入数据中的 `all_tiers_exhausted × 6` + `zombie_empty_completion × 1` +
`dsv4f0731_nv SR=53.3% (8/15)` **全部来自 hermes caller 走 dsv4f0731_nv 链路**,
不在 cc2 (cc4101-primary→glm5_2_nv) 优化范围 (铁律 3: 聚焦 nv_gw 40006)。
...
per-call SR = **100% (48/48)** ✅, 零错误 (status!=200 → 0 rows)
...
| k1 | 5 | NVCFPexecRemoteDisconnected×6, pexec_SSLEOFError×1, pexec_conn_RemoteDisconnected×1 |
| k2 | 14 | NVCFPexecRemoteDisconnected×4, pexec_conn_RemoteDisconnected×1 |
| k3 | 8 | NVCFPexecRemoteDisconnected×2, pexec_SSLEOFError×1 |
| k4 | 10 | NVCFPexecRemoteDisconnected×2, 529_nv_overloaded×1 |
| **合计** | **48** | **22 瞬态** |

- 总 attempts = 70 (48 success + 22 瞬态错误)
- per-attempt SR = 68.6% (48/70) — NVCF 后端有 RemoteDisc 风暴
- **per-key tier 最终成功 SR = 100% (48/48)** ✅ — 22 个错误全被 buffer 5key 轮转吸收
- 用户零感知: cc_requests 48/48 = 100%, 零 502 穿透, 零 fallback

### 本轮观察: NVCF RemoteDisc 风暴特征

本轮 22 个瞬态错误分布:
- `NVCFPexecRemoteDisconnected × 17` — NVCF 后端主动断连 (主要错误)
- `pexec_SSLEOFError × 2` — SSL 层 EOF (k1, k3, 代理侧或 NVCF TLS 断)
- `pexec_conn_RemoteDisconnected × 3` — 连接层断连 (k1, k2)
- `empty_200 × 1` — 返回 200 但空体 (k0)
- `529_nv_overloaded × 1` — NVCF 过载 (k4)

**关键: 风暴波及全 5 key, 但每个请求都通过 buffer 切到另一个 key 重试成功。**
这是 buffer 机制设计的核心价值 — per-attempt 失败率 31% 但 per-call 成功率 100%。
R813 chain_full_retry=True 保证 attempt 间 retry 正确触发。
...
**注**: per-attempt SR 下降到 68.6% 是 NVCF 后端 RemoteDisc 风暴导致,
不是链路 bug — buffer 机制正确吸收所有瞬态错误, per-call/per-key 最终 SR 仍 100%。
