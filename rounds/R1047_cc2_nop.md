# R1047 cc2 NOP — cc2 primary 100% clean (107/107), scoped errors 0; bad 3 hermes (zombie_empty_completion×2/NVStream_IncompleteRead×1); fallback 0 (0/107); 155th clean round (R893-R1047)

日期: 2026-08-07 CST
性质: NOP 巡检轮 (不改码)

## 结论
cc2 主链路 (caller=cc4101-primary) 连续第 **155** 轮 100% SR 干净 (R893-R1047)。
主链专属错误 0 rows, fallback 0 次。本轮 3 bad 全属 **hermes** 越界宿主, 与主链 host 分离干净。

## 数据 (live 复核 2026-08-07 CST + 注入轮前链路分析 18:15 CST)
...
- 30min 按模型总 SR dsv4f0731_nv = 98.6% (204/207), 差异 3 条 bad 全属 hermes。
- 30min cc_requests = **0 次 fallback (0.0%)**, 主链 107/107 全 200 (live total=107, ok=107, sr=100.0)。
- nv_tier_attempts (30min 注入): k0-k4 全 pexec_success (22/21/20/20/21) — 主链首代=dsv4f0731_nv,
  无 tier 层错误, 无 key 疲劳。
- buffer 日志 (注入): 无 buffer/wait/keymanager 日志 = 全部 cc4101-primary 请求 attempt=1 success 直接 flush,
  buffer 零吸收需要。
- 容器: nv_gw Up 15h, cc4101 Up 14h, dsv4p_nv40066 Up 2d, /health 40006/4101/40066 全 200。
  当前首代模型 dsv4f0731_nv, 无 tier 降级。

## 关键判断
连续 **155** 轮稳态 (R893-R1047)。主链 107/107 全 200, 专属错误 0 行, fallback 0 (0.0%), buffer 零重试。
