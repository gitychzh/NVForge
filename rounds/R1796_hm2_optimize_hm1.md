# R1796 (HM2→HM1): NOP — false trigger, 零dsv4p_nv post-R1794流量, 改前必有数据铁律触发

**时间**: 2026-07-18 21:15 UTC
**触发**: HM2自提交 R1795 (`这是我提交的, 不触发` — false trigger, cron仍被派遣)
**作者**: opc2_uname (HM2)

## 改前数据 (2026-07-18 21:15 UTC)

### 6h DB (15:15-21:15 UTC)

| request_model | total | ok | fail | avg_dur_ms | sr_pct |
|---------------|-------|-----|------|------------|--------|
| glm5_2_nv     |    24 | 24 |    0 |       9373 |  100.0 |
| dsv4p_nv      |     8 |  7 |    1 |      45958 |   87.5 |

**总**: 32req/31OK (96.9% SR) / 1fail

### 1h DB (20:15-21:15 UTC)
...
- **key_cycle_429s**: glm5_2_nv 100% (24/24, pexec key rotation normal), dsv4p_nv 0%
- **fallback**: 0 (全部 f)
- **peer-fb**: 0 (无peer-fb触发, 日志零 NV-PEER-FB)
- **zombie_empty_completion**: 0
- **NV-INTEGRATE**: 0 (NV_INTEGRATE_MODELS=空, glm5_2 pexec only)

### 日志 (最近6请求, 全部 glm5_2_nv pexec)

```
21:03:20 [NV-GLM52-SUCCESS] k4 pexec 19093ms
21:03:40 [NV-GLM52-SUCCESS] k5 pexec  9107ms
...
→ 全部 pexec SUCCESS, 5 keys cycling, avg=11530ms, 无错误
...
1. **False trigger**: R1795 是 HM2 自提交 NOP, 脚本正确识别 `"这是我提交的, 不触发"` 但 cron 仍被派遣。与 R1795 完全相同的 false trigger 模式。
...
3. **glm5_2_nv: 100% SR (24/24)**: pexec路径完美, avg=9373ms。0 zombie_empty_completion。100% key_cycle_429s (pexec key rotation正常)。

4. **零 fallback, 零 peer-fb, 零 zombie**: 系统干净, 无可配置修复项。

5. **零 post-R1794 dsv4p_nv 流量**: R1794 NOP声明"零dsv4p_nv post-deploy流量, 改前必有数据铁律触发"。R1795确认此模式。当前数据继续验证: 09:31后零dsv4p_nv请求, 仅glm5_2_nv (openclaw) 流量。无法验证peer-fb rescue效果。

6. **所有参数 at floor/optimal**: FASTBREAK=1 (pexec+integrate+empty200), KEY_COOLDOWN=65, TIER_COOLDOWN=65, SSLEOF_RETRY=0.5, MIN_OUTBOUND=0, EMPTY_200=1。无进一步优化空间。

7. **容器漂移: 零**: compose md5一致, 容器env与compose一致。

8. **数据与R1795几乎完全一致**: 6h从30req/29OK变为32req/29OK(+2 glm5_2), SR 96.7%→96.9%。ATE完全相同(同一cluster)。结论不变。

## 决策: NOP (零变更)

**理由**: 零可配置修复故障。dsv4p_nv ATE为NVCF degradation (12-min cluster, 不可配置修复)。glm5_2_nv 100% SR。零 zombie, 零 fallback, 零 peer-fb。零 post-deploy dsv4p_nv流量无法验证。所有参数floor/optimal, 零漂移。数据与R1795几乎完全一致, 结论不变。
