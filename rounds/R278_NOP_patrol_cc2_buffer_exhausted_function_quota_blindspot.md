# R278 — NOP 巡检轮 (2026-08-02 15:38 CST)

## 摘要
- **cc2 (cc4101-primary) 30min 0 req** — session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- **dsv4p_nv 30min SR=37.5% (3/8)** — 5 失败全 hermes caller, 全整 5min 边界点
  (07:10/07:15/07:20/07:25/07:35 UTC = 15:10/15:15/15:20/15:25/15:35 CST),
  全 `all_tiers_exhausted`, duration ~1.6-2.7s 快速失败 (pexec peek path 非 buffer).
- **新发现 (4h 窗口, R277 STATE 未记)**: cc2 primary 在 06:28-06:32 UTC (14:28-14:32 CST)
  经历 **5×502** (3×`all_tiers_exhausted` + 2×`buffer_exhausted`), duration ~165s.
  KEYMGR 日志铁证: 15:25:32 `dsv4p_nv k1/k2/k3/k4/k5 全 429` — **5key 同时被打穿**.
  → **buffer 5key 轮转对 function 级 429 无保护** (根因下述). 06:34 自恢复全 200.
- 0 改动 0 restart. 十一轮一致 R268-R278.

## 本轮数据 (30min 实时 DB 复查 ~15:36 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R277, session 间歇空闲.

### 2. dsv4p_nv 30min 8 req (3×200 + 5×429), 失败全 hermes 边界点
...
### 3. cc2 primary 4h 趋势 (新发现)
- 4h 26 req = 21 ok / 5 fail, 5 失败全集中在 06:28-06:32 UTC 一次性 window:
...
- 这是 cc2 primary **首次** (R268 序列内) 在 buffer 保护下仍打出 502 buffer_exhausted.
...
```
- 1 秒内 dsv4p_nv tier 的 k1/k2/k3/k4/k5 **全** 触发 429 → 全进 cooldown.
- 这是 06:28-06:32 cc2 buffer 5key 轮转全败、打 502 buffer_exhausted 的直接原因.

## 根因分析: buffer 对 function 级 429 无保护 (设计盲区)

### 现象
- dsv4p_nv 5key (k0-k4) 全绑同一 NVCF function `12acbc62-3a9e-461f-8139-142e914b6f16`.
- NVCF 429 配额是 **function 级** (不是 key 级): 当 function 配额耗尽, **5 个 key 同时** 收 429.
- buffer 5key 轮转 (k0→k1→k2→k3→k4) 设计假设 "单 key 429 切下一 key 绕过", 对 function 级 429 失效:
  5 key 全打同一 function → 5 key 同时 429 → buffer_exhausted (165s 总预算耗尽).
- 这是 **设计盲区** 非 nv_gw 代码缺陷. R-nvonly 5key 5IP 设计针对的是 key/IP 级隔离, 未覆盖 function 级配额.

### 为何 hermes 边界 429 是常态, cc2 buffer_exhausted 是罕见
- hermes 走 pexec peek path: 单 key 探测 429 → 一击即败 (~1.6s), 快速释放, 不消耗 buffer.
- cc2 走 buffer 5key 轮转: 5 key 全 429 → 消耗 5×~30s (probe+cooldown) = ~165s → buffer_exhausted.
- 4h 内 hermes 边界 429 稳定 5-9/h (常态), cc2 buffer_exhausted 仅 06:28-06:32 一次性 5 次 (罕见).
- 原因: cc2 流量极低 (4h 26 req), 命中 function 配额边界点的概率远低于 hermes 高频探测.
...
## 下一步
1. 持续监控 cc2 primary buffer_exhausted 是否复发 (>5/h 或蔓延至非边界点才需介入).
2. 若复发频繁, 考察根因层改进: 把 dsv4p_nv 5key 拆到不同 NVCF function (需上游侧, 非 nv_gw 可改),
   或在 nv_gw 侧对 `all_tiers_exhausted`/429-边界点引入 WaitQueue event-driven 短等待 (跨 5min 边界恢复).
   — 均非本轮任务, 记录待后续.
3. 持续监控 dsv4p_nv 5min 边界 429 是否恶化. 现状 5-9/h 可接受.
