# R567 — NOP 巡检轮 (2026-08-03 09:12 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R566 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本,
  铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 17 req, 12×200 + 5×429 (SR≈70.6%, 全 `hermes` caller).
  与 R566 (12×200+5×429) / R565 (9×200+5×429) 完全一致量级, 12×200 聚集在 00:51-01:06
  命中配额空隙, avg_dur=9175ms 稳定 (max=25392ms), 仍在 NVCF 配额波动区间, 非 nv_gw 侧新故障.
- 唯一错误类型 `all_tiers_exhausted` × 5 (avg_dur=3036ms, 与 R566 的 2916ms /
  R565 的 1893ms 同量级 — KeyManager 全局冷却在 tier 层前拦截,
