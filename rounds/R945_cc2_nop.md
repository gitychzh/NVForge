...
| zombie_empty_completion | 1 (R941 首现 → R945 连续第 5 轮保持 1 未扩散) | ⚠️ 观察 |
| tier 瞬态错误 | RemoteDisconnected 18 + empty_200 3 + Timeout 3, 全被吸收 | ✅ |
| scoped health | 4101/40006/40066 全 ok (200) | ✅ |
| 容器 UP | nv_gw 8h (平稳) / cc4101 7h / dsv4p 2d | ✅ |

### 验证
- curl 4101/40006/40066 → 全 ok (200)。
- 30min nv_requests cc4101-primary 实拉 = 125/125 (0 bad); 专属错误 0 rows。
- 30min 所有 bad 分组 (caller 列铁证带 request_id): 5 条全 hermes (105fb55f, 57772fd8, e4f8ea10, 4dd0664d, e17b21b1), cc2 主链 0 bad。
- cc_requests fallback = 0 次。
- docker logs nv_gw buffer 段: cc4101-primary 全 attempt=1 success, 无错误噪音。

### 关键判断
cc2 主链路连续第 **53** 轮 (R893-R945) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
bad 请求 100% 属 hermes (caller 列实拉铁证带 request_id, 未进 cc2 主链), fallback 0 次, 无新 cc2 主链错误类。
zombie_empty_completion 连续第 5 轮保持单例 (R941→R945), 未扩散, 属 hermes 越界。
...
> ⚠️ 观察项 (R945): nv_gw Up = 8h (与 R944 持平, 平稳无重启回归);
> 同机有 `nv_gw_stable Up 5d` 并存, dsv4p_nv40066 Up 2d。zombie_empty_completion 连续第 5 轮保持单例未扩散,
