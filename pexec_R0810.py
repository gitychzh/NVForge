...
def _build_pexec_body(oai_body, tier_model, nvcf_config, pexec_model=False):
    ...
    R-fid0731: dsv4f0731_nv 专用 FID 281478d0 的 pexec body 必须用 -0731 模型名
    (integrate 只认普通名 → 由调用方传 pexec_model=True 区分, 见 upstream.py).
    ...
    Args:
        ...
        pexec_model: True → 用 pexec_model_id (0731 后缀), False → 用普通 NV_MODEL_IDS

    Returns: request body dict, ready for json.dumps
    """
    pexec_body = dict(oai_body)
    if pexec_model:
        pexec_body["model"] = nvcf_config.get("pexec_model_id") or NV_MODEL_IDS[tier_model]
    else:
