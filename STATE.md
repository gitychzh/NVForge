...
> 当前轮: **R989 (NOP 巡检轮/不改码 — cc2 主链路连续第 97 轮 100% 干净; 唯一坏 5: all_tiers_exhausted ×4 + zombie_empty_completion ×1 全属 hermes, 越界非 cc2 范围; fallback 0 次)**
...
> 上轮: R988 (NOP, 主链 126/126=100%)

## 本轮 (R989) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 97 轮 100% 干净, 主专属错误 0 行; 唯一 bad 全属 hermes+已知坏 fid 52e1ddb6 线)

### 依据 (live 复核 2026-08-07 + 注入轮前链路分析)

- 30min cc4101-primary (主 nv_gw:40006) = **121/121 全 200 = 100% SR, 0 bad** (live re-pull)。
- 总 nv_requests bad (非 200) = **5 条, 全 caller=hermes 线** (live 复核 by caller×err_type×fid):
  `hermes | all_tiers_exhausted | 52e1ddb6 | 502 | 4` + `hermes | zombie_empty_completion | 52e1ddb6 | 502 | 1`,
  已知坏 fid 52e1ddb6 hermes 宿主越界容器, non cc2 主链。
- fallback (cc_requests 30min) = **0 次** (live total 1912 req, fb=0)。
- cc_requests 真实 SR = 99.2% (1897/1912), 全部 15 bad 属 hermes 非 cc2 主链。
- nv_tier_attempts 30min: pexec_success 主导 (23-25/key) + 瞬态
  (NVCFPexecRemoteDisconnected/Timeout/empty_200) 散落各 key, 全被多 key round-robin +
  func_health + buffer 吸收, 无泄漏为 200 以外。
- buffer 日志 (nv_gw live): cc2 primary 绝大多数 attempt=1 一次成功 (verdict=success_tool_call,
  elapsed 8-9s, flushed 7505b-18527b), 无 attempt>1 重试, 无 502 泄漏。
...
| 唯一 bad (非 200) | 5 条, 全 caller=hermes 线 (fid 52e1ddb6, all_tiers×4+zombie×1) | ⚠️ 越界 |
| bad caller 归属 | 100% hermes 线; cc2 primary 0 条 | ✅ 隔离 |
| fallback (cc_requests) | 0 次 (1912 req 全未 fb) | ✅ |
| cc_requests 真实 SR | 99.2% (1897/1912); 15 bad 全属 hermes | ✅ |
| tier 瞬态错误 | RemoteDisconnected / Timeout / empty_200, 全被吸收 | ✅ |
| 容器 | nv_gw Up, cc4101 Up, health 全 200 | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 121/121 (0 bad)。
- bad 分组 (by caller×err_type×fid): hermes 线 all_tiers_exhausted ×4 + zombie_empty_completion ×1
  (全 fid 52e1ddb6) — 全 hermes, cc2 主链 0 bad。
- cc_requests fallback = 0 次 (1912 req 全未 fallback_triggered)。
- health: 4101/40006 全 200; 容器 nv_gw/cc4101 皆 Up。
- buffer: cc2 侧全 attempt=1 success_tool_call, 8-9s 一次成功, 无 502 泄漏。

### 关键判断
cc2 主链路连续第 **97** 轮 (R893-R989) 100% SR 干净, 且主链专属错误 0 rows。
...
## 修复链 (沿用, R827+R828+R829+R833+R813 + R869+R876+R891 + R987/R988)
