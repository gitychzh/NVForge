...
> 当前轮: R863 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (128×200) 零错误, buffer attempt=1 一次成交, 30-min 残留 all_tiers_exhausted×5 全为 caller=hermes 外部 cron ~6min 严格周期, 与 cc2 路径无关, 不改码, 2026-08-07 ~05:0x CST)
> 上轮: R862 (NOP — 近窗 125×200 零错误, hermes 周期 all_tiers_exhausted 属外部 cron, 不改码)

## 本轮 (R863) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 128×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (~05:0x CST, 实时拉取, DB UTC)

**最近 30min cc4101-primary (cc2 自己路径) SR = 100% (128×200, 零错误).**

| 指标 | 值 | 状态 |
|---|---|---|
| **最近 30min cc4101-primary SR** | **100% (128×200)** | ✅ |
| **15min 快窗 cc4101-primary** | **100% (63×200)** | ✅ |
| **primary 目标 tier** | **dsv4f0731_nv** (自适应轮转持有, /health 确认) | ✅ |
| **primary fid 281478d0 成功率** | **127/127 = 100%** (5key 均衡 25-26 each) | ✅ |
| **buffer 效果** | 全部 attempt=1 一次成交, flush 7-10s ≪ 90s budget | ✅ 无退化 |
| **error 归属** | all_tiers_exhausted×5 全为 caller=hermes (外部 cron, 严格 ~6min 周期) | ✅ 与 cc2 无关 |
| **failover fid 52e1ddb6** | 全败 (26) 被跨 key round-robin 自适应吸收 | ✅ |
| **三容器 health** | nv_gw / cc4101 / dsv4p 均 ok | ✅ |

### 关键判断: cc2 路径全净, hermes 周期 all_tiers_exhausted 非本链路问题

30min 窗口 5 条 `all_tiers_exhausted` (502) 经 caller 字段核验 **全部 caller=hermes**
(外部客户端, 非 cc4101), 严格 ~6min 周期分布 (20:36/42/48/54/21:00 UTC),
每次 ~180s ≈ 5×90s=450s buffer deadline 全额耗尽 — 属 cron 请求特征而非链路退化
(沿用 R853-R862 判定).

per-key nv_tier_attempts: primary fid 281478d0 (dsv4f0731_nv) 5key 均 25-26 次
**100% 成功 (127/127)**, failover fid 52e1ddb6 全败 (26) 被 KeyManager 跨 key
round-robin 修复链自适应吸收, 未上抛到 cc2 用户请求. cc2 自身路径 128×200 零错误,
buffer 全部 attempt=1 一次成交 (无 NV-BUFFER-REFUSED/WAIT/KeyManager cooldown),
