...
> 当前轮: **R928 (NOP 巡检轮/不改码 — cc2 主链路连续第 37 轮 100% 干净; 坏请求 all_tiers_exhausted ×3 + zombie_empty_completion ×1 (502) 全属 hermes 线, caller 列实拉铁证, 非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **118/118 = 100% SR, 0 bad** (实拉);
> cc4101-primary 专属错误 30min = **0 rows** (118 request 全 200);
> 容器: nv_gw Up 7h, cc4101 Up 6h, dsv4p_nv40066 Up 2d
> 上轮: R927 (NOP, 主链 119/119=100%)

## 本轮 (R928) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 37 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)

### 依据 (live DB 30min 实拉 ≈2026-08-07 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **118/118 全 200, 0 bad (100% SR)**。
- 30min **cc4101-primary 专属错误 = 0 rows** (status != 200 AND caller='cc4101-primary' 全空)。
- 30min 所有 bad (502) = `caller=hermes` ×4: `all_tiers_exhausted ×3` + `zombie_empty_completion ×1`。
- **caller 列实拉铁证**: 4 bad 全 caller=hermes, 0 个属于 cc2 主链 (host-separated)。
- fallback (cc_requests 30min, total 1631) = **0 次**。
- tier 层 NVCFPexecRemoteDisconnected + timeout ×4 + 504 ×2 全被多 tier round-robin 吸收, 未浮现为 cc2 primary bad。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **118/118 = 100% SR, 0 bad** (实拉) | ✅ |
...
- 30min nv_requests cc4101-primary 实拉 = 118/118 (0 bad); 专属错误 0 rows。
- 30min 所有 bad 分组 (caller 列铁证): 4 条全 hermes, cc2 主链 0 bad。
- cc_requests fallback (1631 total) = 0 次。

### 关键判断
cc2 主链路连续第 **37** 轮 (R892-R928) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
