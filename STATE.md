...
> 当前轮: **R954 (NOP 巡检轮/不改码 — cc2 主链路连续第 62 轮 100% 干净; 坏请求 stream_absolute_cap 1 全属 hermes 线, caller 归属非 cc2 范围; fallback 0 次)**
...
> 上轮: R953 (NOP, 主链 121/121=100%)

## 本轮 (R954) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 62 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)
...
- nv_tier_attempts 30min: pexec_success **121** + NVCFPexecRemoteDisconnected **17**
  ...
  全部 resolve 为 200。
- buffer 日志: cc4101-primary 全 attempt=1 成功 (elapsed 1-13s, input 5-102k, verdict success_text/
  success_tool_call), 无 WAIT-/KEYMGR- 错误噪声。
- 容器 health: 4101/40006/40066 全 ok (200)。模型栈 dsv4f0731_nv 在 nv_gw model tiers 首位。
...
③多 tier round-robin (dsv4f0731_nv 为首) + func_health 健康选择已达稳态, buffer 全 1-attempt 成功, tier 瞬态错误全被吸收 (RemoteDisconnected 17 全被 round-robin 消化)。
