...
> 当前轮: **R911 (NOP 巡检轮/不改码 — cc2 主链路连续第 20 轮 100% 干净; 坏请求 stream_absolute_cap ×2 + all_tiers_exhausted ×1 (502) 全属 hermes 线, JOIN 铁证, 非 cc2 范围; bad fid 52e1ddb6 28 次 attempts 全命 hermes, 0 泄漏进 cc2 主链)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **119/119 = 100% SR, 0 bad** (实拉);
> live DB now()≈2026-08-07 09:23 CST
> 上轮: R910 (NOP, 主链 127/127=100%)

## 本轮 (R911) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 20 轮 100% 干净, 无新错误类; bad 请求全属 hermes 非 cc2)

### 依据 (live DB 30min 实拉, ≈2026-08-07 09:23 CST)
...
- fid 级: 健康 fid **281478d0** = pexec_success ×121, 0 错误 (cc2 主链专用)。
  坏 fid **52e1ddb6** = 28 次 attempts 全错误 (RemoteDisconnected / 529_nv_overloaded / Timeout / empty_200),
  JOIN 铁证 `52e1ddb6|hermes|28` → 0 泄漏进 cc2 主链。
- buffer (cc4101-primary): 全 attempt=1/5 成交, 9-10s success_tool_call, 0 重试 / 0 429 / 0 cooldown。
- fallback (cc2 线) 0 次 (0/123)。
- 容器 health: 4101/40006/40066 全 ok (200), nv_gw Up 6h, cc4101 Up 5h。
...
| bad fid 52e1ddb6 泄漏 | 28 attempts 全属 hermes (JOIN), cc2 primary 0 条 | ✅ 隔离 |
| fid 健康 (nv_tier_attempts) | 281478d0 ×121 pexec_success 0 错; 52e1ddb6 全错未选中 | ✅ |
| buffer (cc4101-primary) | 全 attempt=1/5, 9-10s, 0 重试 / 0 429 / 0 cooldown | ✅ |
...
- 52e1ddb6 全部 28 次 attempts JOIN 归属 hermes, 0 进 cc2; func_health 健康选择未选中。

### 关键判断
cc2 主链路连续第 **20** 轮 (R892-R911) 100% SR 干净。bad 请求 100% 属 hermes,
