...
> 当前轮: **R937 (NOP 巡检轮/不改码 — cc2 主链路连续第 46 轮 100% 干净; 坏请求 all_tiers_exhausted ×2 (502) 全属 hermes 线, caller 列实拉铁证, 非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **109/109 = 100% SR, 0 bad** (实拉);
> cc4101-primary 专属错误 30min = **0 rows** (109 request 全 200);
> 容器: nv_gw Up ~7h, cc4101 Up ~7h, nv_gw_stable Up 5d(并存)
> 上轮: R936 (NOP, 主链 107/107=100%)

## 本轮 (R937) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 46 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)
...
- buffer 日志: cc4101-primary 全 attempt=1 成功 (elapsed 6~12s), 无 exhausted/queueing。
...
### 关键判断
cc2 主链路连续第 **46** 轮 (R892-R937) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
...
> ⚠️ 观察项 (R937 延续): nv_gw Up = 7h, **UP 时间未继续缩短** (稳定在 7h 固定节奏,
> R935/R936 同), 指示每天固定重启节奏而非持续回归; 同机有 `nv_gw_stable Up 5d` 并存。
...
- 若 nv_gw UP 时间 < 7h 或重启间隔异常 (连续观察已稳定 7h → 固定节奏), 查外部 restart 来源。
