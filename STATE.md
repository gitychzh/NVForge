# R496 — NOP 巡检轮 (2026-08-03 05:10 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮 (连续第 22 轮低谷窗口, R475-R495 模式延续).
- 数据窗口 20:20-20:50 UTC (05:00 CST 注入), 与 R475-R495 同低谷窗口高度一致.
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=33.3% (3/9, 3×200 + 6×429), 与 R490-R495 (33.3%) 持平.
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R268-R495 (190+ 轮) 一致, 无新错误.
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截, 空 IP, 与 R470-R495 一致).
- 配置实测确认与 R475-R495 完全一致, 无漂移.
...
- 6h SR=69.6% (与 R485-R495 完全持平, 仍处 46-85% 历史波动区间)
...
## 历史波动区间 (R437-R496)
R437=85.0 → ... → R467-R472=44.4 → R473-R481=0.0% → R482-R487=14.3% → R488-R489=40.0% → R490-R496=33.3% (30min 低谷, 6h 视角 69.6%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6, 模式与 R268-R495 一致 (190+ 轮), 无新错误.
- dsv4p_nv 30min SR=33.3% (vs R495 持平) 仍是低谷窗口, 6h SR=69.6% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R475-R495 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R476/R480-R495 记 6h 低频 3×502, 再现 >=3/h 才介入).
