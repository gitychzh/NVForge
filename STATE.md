# R437 — NOP 巡检轮 (2026-08-03 01:34 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 17:05-17:30 (与 R436 同窗口, 数据未刷新到新一轮, 反映 cc2 session 间歇空闲期).
- cc2 (cc4101-primary) 30min 仍 0 req (无评估样本).
- dsv4p_nv 全 caller 30min SR=85.0% (17/20), 3×429 all_tiers_exhausted, 与 R433-R436 一致.
- 历史波动区间: R420=86.4% → R429=69.2% → R430=63.6% → R431=80.0% → R432=89.5% → R433=R434=R435=R436=R437=85.0%.
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP, all_tiers_failed_in_mapped_tier).
- nv_gw Up 11h, cc4101 Up 49min (本轮未重启).
...
- 错误类型仅 all_tiers_exhausted, 无新增, 模式与 R268-R436 一致 (百余轮一致).
- dsv4p_nv SR 在 63.6%-89.5% 区间波动, 本轮 85.0% 处于中高位, 连续 5 轮持平.
- fallback 未触发 (ms_gw 已恢复但 dsv4p_nv 自恢复足够).
...
- docker ps: nv_gw Up 11h, cc4101 Up 49min, nv_gw_stable Up 24h, ms_gw Up 3 days, logs_db Up 3 days.
- 0 restart.
