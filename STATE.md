...
> 当前轮: **R918 (NOP 巡检轮/不改码 — cc2 主链路连续第 27 轮 100% 干净; 坏请求 all_tiers_exhausted ×3 + stream_absolute_cap ×1 (502) 全属 hermes 线, caller 列 + request_id 级 JOIN 双重铁证 (056d2c5e/33516449/5d3afd42/bfcd651d), 非 cc2 范围; fallback 0 次)**
...
> 上轮: R917 (NOP, 主链 120/120=100%)

## 本轮 (R918) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 27 轮 100% 干净, 无新错误类; bad 请求全属 hermes 非 cc2)

### 依据 (live DB 30min 实拉, ≈2026-08-07 09:47 CST)
...
- 30min 所有 bad (502) = `caller=hermes` ×4: `all_tiers_exhausted ×3` + `stream_absolute_cap ×1`。
- **caller 列 + request_id 级 JOIN 双重铁证** (nv_requests ⋈ nv_tier_attempts):
  4 bad request_id = 056d2c5e(5 attempts)/33516449(4)/bfcd651d(5)/5d3afd42(3),
  **全部 caller=hermes, 0 个属于 cc2 主链**。
- fallback (cc_requests 30min total=120) = **0 次**。
- buffer 日志: 主链请求全部 attempt=1 success_tool_call (~2-12s), 无重试, 无全挂。
- per-key 瞬态错误 (NVCFPexecRemoteDisconnected / Timeout) 分散 k0~k4,
  被 multi-tier round-robin + func_health 健康选择吸收, pexec_success 稳定, 未达 cc2 全挂。
- 容器 health: 4101/40006/40066 全 ok (200), nv_gw Up 11h, cc4101 Up 6h, dsv4p_nv40066 Up 2d。
...
| hermes 线 bad (502) | all_tiers_exhausted ×3 + stream_absolute_cap ×1 | ⚠️ 越界 |
| bad caller 归属 | 4 req 全 caller=hermes; cc2 primary 0 条 | ✅ 隔离 |
| bad request_id JOIN | 4/4 条 attempt 均属 hermes 发起 (3~5 attempts), cc2 0 泄漏 | ✅ 隔离 |
| fallback (cc_requests) | 0 次 (0/120) | ✅ |
| per-key tier | pexec_success 主导, 瞬态错误被吸收 | ✅ |
| scoped health | 4101/40006/40066 全 ok (200), nv_gw Up 11h | ✅ |
...
- cc_requests fallback = 0 次 (0/120)。

### 关键判断
cc2 主链路连续第 **27** 轮 (R892-R918) 100% SR 干净。bad 请求 100% 属 hermes,
