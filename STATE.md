...
> 当前轮: **R988 (NOP 巡检轮/不改码 — cc2 主链路连续第 96 轮 100% 干净; 唯一坏 5: all_tiers_exhausted ×4 + zombie_empty_completion ×1 全属 hermes, 越界非 cc2 范围; fallback 0 次)**
...
> 上轮: R987 (NOP, 主链 125/125=100%)

## 本轮 (R988) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 96 轮 100% 干净, 主专属错误 0 行; 唯一 bad 全属 hermes+已知坏 fid 52e1ddb6 线)
...
- 总 nv_requests bad (非 200) = **5 条, 全 caller=hermes 线** (live 复核):
  ...
  宿主越界容器, non cc2 主链。
- fallback (cc_requests 30min) = **0 次** (live total 1904 req, fb=0)。
- nv_tier_attempts 30min: pexec_success 主导 (23-26/key) + 瞬态
  (NVCFPexecRemoteDisconnected/Timeout/empty_200) 散落各 key, 全被多 key round-robin +
  func_health + buffer 吸收, 无泄漏为 200 以外。
- buffer 日志 (nv_gw live): 绝大多数请求 attempt=1 一次成功 (elapsed 3-19s);
  1 例 req=394a6694 attempt=1 key=k5 all_keys_exhausted → BACKOFF 5s → attempt=2 重试被吸收,
...
| fallback (cc_requests) | 0 次 (1904 req 全未 fb) | ✅ |
...
- cc_requests fallback = 0 次 (1904 req 全未 fallback_triggered)。
- health: 4101/40006 全 200; 容器 nv_gw/cc4101 皆 Up。
- buffer: cc2 侧 1 次 all_keys_exhausted (attempt=1) 被 backoff + attempt=2 吸收, 无泄漏 502。

### 关键判断
cc2 主链路连续第 **96** 轮 (R893-R988) 100% SR 干净, 且主链专属错误 0 rows。
