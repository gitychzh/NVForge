...
> 当前轮: **R962 (NOP 巡检轮/不改码 — cc2 主链路连续第 70 轮 100% 干净; 坏请求 4: 502 all_tiers_exhausted ×3 + 502 zombie_empty_completion ×1 全属 hermes 线, caller 归属非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **124/124 = 100% SR, 0 bad** (live 实拉,
> avg ~9066ms);
> cc4101-primary 专属错误 = **0 rows** (124 request 全 200);
> 容器: nv_gw Up 9h, cc4101 Up 9h, dsv4p_nv40066 Up 2d
> 上轮: R961 (NOP, 主链 125/125=100%)

## 本轮 (R962) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 70 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)

### 依据 (live DB 30min 实拉 ≈2026-08-07 12:31 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **124/124 全 200, 0 bad (100% SR)** (live re-pull,
  avg ~9066ms)。
- 总 nv_requests 30min = 136 req, 132 ok (97.1% SR), 全 dsv4f0731_nv。
- 4 个 bad (非 200) = **均 `caller=hermes`**: `502 all_tiers_exhausted ×3` +
...
- nv_tier_attempts 30min: pexec_success 为主 + NVCFPexecRemoteDisconnected 每 key 1-6,
  瞬态被多 key round-robin + func_health + buffer 吸收, 全部 resolve 为 200。
- buffer 日志 (nv_gw): cc4101-primary 全 attempt=1 verdict=success_tool_call, 无 WAIT-/KEYMGR- 错误噪声。

### 本轮数据

| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **124/124 = 100% SR, 0 bad** (实拉, avg ~9066ms) | ✅ |
...
### 验证
- 30min nv_requests cc4101-primary live re-pull = 124/124 (0 bad, avg ~9066ms)。
...
- docker logs nv_gw buffer 段: cc4101-primary 全 attempt=1 success_tool_call, 无错误噪音。
- health: 4101/40006/40066 全 200。

### 关键判断
cc2 主链路连续第 **70** 轮 (R893-R962) 100% SR 干净, 且主链专属错误 0 rows。
