...
> 当前轮: **R998 (NOP 巡检轮/不改码 — cc2 主链路连续第 106 轮 100% 干净; 主链专属错误 0 rows; fallback 0 次)**
...
> 上轮: R997 (NOP, 主链 119/119=100%)

## 本轮 (R998) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 106 轮 100% 干净, 主专属错误 0 rows; 本轮 window 内 nv_requests 的 4 条 bad 全属 hermes)
...
- fallback (cc_requests 30min) = **0 次** (117 req 全未 fallback_triggered)。
- nv_tier_attempts 本轮窗口 21 条非 pexec_success 全属 NVCFPexecRemoteDisconnected 瞬态, round-robin + func_health 吸收, 无持久疲劳。
- buffer 日志全 attempt=1 一次成功 (flush 6–16KB, elapsed 6–7s); 无 WAIT 停滞, 无多 attempt 泄漏。
...
| 30min fallback (cc_requests) | 0 次 (117 req 全未 fb) | ✅ |
| buffer/wait | 全 attempt=1 一次成功, flush 6–16KB, 无 WAIT 停滞 | ✅ |
| nv_tier_attempts | 无持久键疲劳 (21 条瞬态 NVCFPexecRemoteDisconnected 被吸收) | ✅ |
| 模型 SR | dsv4f0731_nv = 138/142 = 97.2% (含 hermes 4 bad) | — |
| 容器 | nv_gw Up 12h, cc4101 Up 11h, health 全 200 | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 116/116 (0 bad)。
- 主链专属错误分组 = 空 (0 rows)。
- 4 条非 200 经 caller 归属 JOIN 判定 hermes (dsv4f0731_nv 502), 非 cc2 主链。
- cc_requests fallback = 0 次 (117 req 全未 fallback_triggered)。
...
### 关键判断
cc2 主链路连续第 **106** 轮 (R893-R998) 100% SR 干净, 主链专属错误 0 rows。
本轮 nv_requests 的 4 条 bad (dsv4f0731_nv 502) 经 caller JOIN 判定全属 hermes 越界宿主
(fid 52e1ddb6 泄漏) — 与主链 host 分离保持, 主链 116/116 全 200。
