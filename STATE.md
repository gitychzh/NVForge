...
> 当前轮: R870 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (125×200) 零错误, 残留 all_tiers_exhausted×5+stream_cap×1 全为 caller=hermes 外部 cron, fallback 0%, 不改码, 2026-08-07 ~05:27 CST)
> 上轮: R869 (NOP — 近窗 129×200 零错误, hermes 周期 all_tiers_exhausted×4 属外部 cron, 不改码)

## 本轮 (R870) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 125×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (~05:27 CST, 轮前链路分析注入, DB UTC)
...
| **per-key nv_tier_attempts** | dsv4f0731_nv 5key 均 25 次 pexec_success (共 125), 瞬态错误被跨 key 吸收 | ✅ |
| **三容器 health** | nv_gw / cc4101 / dsv4p 均 ok, nv_gw/cc4101 Up 2h | ✅ |

### 关键判断: cc2 路径全净, hermes 周期 all_tiers_exhausted 非本链路问题

30min 窗口链路总览: cc4101-primary 125×200 零错误, 6 条非 200 (502) 经 caller 核验
**全部 caller=hermes** (外部客户端, 非 cc4101): all_tiers_exhausted×5 + stream_absolute_cap×1.
沿用 R853-R869 判定: 每次 all_tiers_exhausted ~180s (avg 180034ms) ≈ 5×90s=450s buffer
deadline 全额耗尽 — 属 hermes 严格 ~6min 周期 cron 请求特征而非链路退化.

per-key nv_tier_attempts (tier=dsv4f0731_nv): 5key 均 25 次 pexec_success
(0→25, 1→25, 2→25, 3→25, 4→25, 共 125 成功),
