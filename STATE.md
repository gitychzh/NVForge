...
> 当前轮: **R1170 (NOP 巡检 — 实查 30min cc4101-primary 200|114 = 100% SR, 0 非-200, 总线 0 错误;
> 总线 dsv4f0731_nv SR=100% (196/196) 全 200 无任何非-200; tier 全 pexec_success (114)
...
> 根因: 链上静稳, 全窗 0 错误
> 最新 30min (03:30-04:00 CST): **cc2-primary 全 200 114/114 = 100% SR, 0 非-200**
> fallback: **0%** (cc_requests 直查 0 触发)

## 本轮 (R1170) 改动 + 依据 + 验证

### 改动: 无 (NOP 巡检轮。cc2 整窗 114/114 全 200, 总线 196/196 全 200, 无改码条件)

### 依据 (注入链路分析 2026-08-08 03:59 CST + 实查 30min + 容器健康)

- **实查 30min cc4101-primary**: `200|114` = 100% SR, 0 非-200。
- **实查 30min 总线**: cc4101-primary 114 + hermes 82 = 196 全部 `200`,
  无任何非-200, 总线 SR = **100% (196/196)**。上轮 hermes 瞬时 502 未复发,
  印证其为一次性瞬时 first-byte 抖动非复发。
- **错误分类 (实查)**: `nv_requests` status != 200 → **0 行** (完全无错误)。
- **tier (实查)**: 全 `pexec_success` (24/23/20/24/23 = 114), 无 429/empty/新类型, 无冷却退避。
- **fallback (实查)**: cc_requests 直查 115/115 = **0 触发** (0%)。注入行 `f|196`
  为表头伪字段, 实查确认 0。
- **buffer 日志 (注入)**: (无 buffer/wait/keymanager 异常日志) = 无退避、无 WAIT、无 buffer_exhausted。
- **容器 (实查)**: nv_gw + cc4101 /health 全 ok, nv_gw Up 25h / cc4101 Up 24h /
  nv_gw_stable Up 6d。
...
实查 114/114 全 200, 总线 196/196 全 200 0 错误, 上轮 hermes 502 未复发。
链路跨十三轮全绿无新事件。
