...
> 当前轮: **R1186 (NOP 巡检 — 注入 30min cc4101-primary 200|119 = 100% SR, 0 非-200;
> 总线 dsv4f0731_nv 202/202 全 200 0 错误; tier 全 pexec_success (k0 单次 NVCFPexecTimeout
> 被 attempt-1 后自愈, 同窗 23 success, 连续第 7 轮同型瞬时); fallback 0%;
> buffer 无退避无 WAIT; 整窗全绿跨二十九轮 → NOP 不改码)**
> 主链 fid: **281478d0-f307** 稳定, dsv4f0731_nv 单模式
> 错误分类 (注入, 30min): (无错误 — 0 非-200)
> 根因: 链上静稳, 全窗 0 错误
> 最新 30min (~05:00 CST): **cc2-primary 全 200 119/119 = 100% SR, 0 非-200**
> fallback: **0%**
...
### 依据 (注入链路分析 2026-08-08 05:00 CST)

- **注入 30min cc4101-primary**: `200|119` = 100% SR, 0 非-200, avg_dur 10405ms (~10.4s)。
- **注入 30min 总线**: dsv4f0731_nv 202/202 全 200 = 100% SR (119 cc2 + 83 hermes), 0 非-200。
- **错误分类**: `nv_requests` status != 200 → **0 行** (完全无错误, "(无错误)")。
- **tier**: 全 `pexec_success`, 分布均匀 (k0-k4: 23/24/23/25/24),
...
## 上轮
R1185 (NOP — 注入 117/117 全 200, 总线 198/198 全 200 0 错误) → R1186 确认:
注入 119/119 全 200, 总线 202/202 全 200 0 错误, 链路持续静稳无新事件。
链路跨二十九轮全绿。
