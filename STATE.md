...
> 当前轮: **R1027 (NOP 巡检轮/不改码 — cc2 主链路连续第 135 轮 100% 干净; 主链专属错误 0 rows; fallback 0)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **104/104 = 100% SR, 0 bad** (live 复核);
> cc4101-primary 专属错误 = **0 rows** (104 request 全 200);
> 容器: nv_gw Up 14h, cc4101 Up 13h, dsv4p_nv40066 Up 2d, /health 全 200
> 上轮: R1026 (NOP, 主链 103/103=100%)

## 本轮 (R1027) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 135 轮 100% 干净, 主专属错误 0 rows; 本轮 window 内 5 条 bad 全属 hermes)

### 依据 (live 复核 2026-08-07 + 注入轮前链路分析 16:58 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **104/104 全 200 = 100% SR, 0 bad** (live `SELECT caller,status,count(*) ... GROUP BY 1,2`)。
...
- 本轮 window 内 nv_requests 总 bad = 5 条 (502), 经 DB live `SELECT caller,error_type,status` 判定全属 hermes:
  all_tiers_exhausted x2 / NVStream_IncompleteRead x1 / stream_absolute_cap x1 / zombie_empty_completion x1。
...
### 验证
- docker exec logs_db psql: `SELECT caller,status,count(*) ... GROUP BY 1,2` → cc4101-primary 104 行全 200, 5 行 bad 全 caller=hermes。
...
### 关键判断
cc2 主链路连续第 **135** 轮 (R893-R1027) 100% SR 干净, 主链专属错误 0 rows。
本轮 5 条 bad (502) 归属全属 hermes 越界宿主, 经 caller 铁证与主链 host 分离完全干净 — 主链 104/104 全 200。
