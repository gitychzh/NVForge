...
## 当前轮基线 (2026-08-02 18:12 CST, R319 NOP 巡检轮)
- 本仓 master: 上轮 R318 (6e5dcaa) 已 push. 主仓 hm2 侧 R318 已 push.
- **架构 (主仓)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R319 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=86.4% (19/22), 失败 3 = 2× all_tiers_exhausted
...
- 与 R268-R318 错误类型集合一致 (all_tiers_exhausted + NVStream_IncompleteRead 历史仍存在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R318 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R319 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=86.4% (19/22) (窗口命中两波 429 + 1 IncompleteRead, NVCF function 配额周期; 总 req 22 少, 故 SR 数值偏低但根因不变).
- 错误类型无新增, 与 R268-R318 一致.
- 五十二轮一致 R268-R319.
