...
> 当前轮: **R980 (NOP 巡检轮/不改码 — cc2 主链路连续第 88 轮 100% 干净; 坏请求 2: 502 all_tiers_exhausted ×1 + 502 stream_absolute_cap ×1 全属 hermes, 越界非 cc2 范围; fallback 0 次)**
...
### 改动: 无 (NOP。cc2 主链路连续 88 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (注入轮前链路分析 ≈2026-08-07 13:45 CST + live 复核 bad 归属/fallback)

- 30min cc4101-primary (主 nv_gw:40006) = **108/108 全 200 = 100% SR, 0 bad** (live re-pull;
  注入窗口 13:45 数据)。
...
- fallback (cc_requests 30min) = **0 次** (live total 110 req, fb=0; 注入 `f|124` 行坏显示, 铁证 fb=0)。
- nv_tier_attempts 30min (per-key): pexec_success 21~23 (k0-k4) + NVCFPexecRemoteDisconnected
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **108/108 = 100% SR, 0 bad** (live re-pull) | ✅ |
...
| 全局 nv_requests SR | 122/124 = 98.4% (dsv4f0731_nv) | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 108/108 (0 bad)。
...
### 关键判断
cc2 主链路连续第 **88** 轮 (R893-R980) 100% SR 干净, 且主链专属错误 0 rows。
