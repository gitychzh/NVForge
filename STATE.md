...
> cc4101-primary (主 nv_gw:40006) 实时 30min = **137/137 = 100% SR, 0 bad**;
...
> 主 nv_gw pexec 全程 fid=281478d0 (健康), buffer 全程 attempt=1/5 即 success_* (1~12s)。
> live DB now()≈2026-08-07 00:16 UTC (08:16 CST)
...
### 依据 (live DB now()≈2026-08-07 00:16 UTC)

- 30min cc4101-primary (主 nv_gw:40006, host opc2sname) = **137/137 全 200, 0 bad (100% SR)**。
  实拉 `WHERE status!=200 AND caller='cc4101-primary'` → 0 条。
- buffer 日志 (07:46→08:16 CST): 全程 caller=cc4101-primary attempt=1/5 即
  verdict=success_text/success_tool_call, elapsed ≈ 1~12s, done=True, closed=False。全干净, 零重试/冷却/429。
...
- 30min nv_tier_attempts: 成功全 fid=281478d0 (25~29/key); 52e1ddb6 仅 4~6/key 全是失败
  (NVCFPexecRemoteDisconnected/529_nv_overloaded/Timeout/empty_200) = hermes 线 dsvf0731_nv40666。
- 容器 health: 4101/40006/40066 全 ok; nv_gw 实时日志 fid=281478d0 全程干净。
...
| 主 nv_gw pexec fid | 成功全 281478d0 (0×52e1ddb6) | ✅ |
| buffer (cc4101-primary) | 全 attempt=1/5 success_*, 1-12s, 0 重试 | ✅ |
...
- 30min nv_requests cc4101-primary 实拉 = 137/137 (0 bad); 所有 bad 实拉归 hermes+52e1ddb6。
- 30min nv_tier_attempts 成功全 281478d0, 失败全 52e1ddb6 → 归属 40666 (hermes 线), 未入主 nv_gw。

### 关键判断
cc2 主链路连续 4 轮 (R892 139/139, R893 153/153, R894 143/143, R895 137/137) 100% SR 干净。所有坏请求仍由 hermes 线的
