...
> 当前轮: **R976 (NOP 巡检轮/不改码 — cc2 主链路连续第 84 轮 100% 干净; 坏请求 3: 502 stream_absolute_cap ×2 + 502 all_tiers_exhausted ×1 全属 hermes + fid 52e1ddb6, 越界非 cc2 范围; fallback 0 次)**
...
> 上轮: R975 (NOP, 主链 106/106=100%)

## 本轮 (R976) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 84 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (注入轮前链路分析 ≈2026-08-07 13:26 CST + live 复核 bad 归属/fallback)
...
- nv_tier_attempts 30min (per-key): pexec_success 19~24 (k0-k4) + NVCFPexecRemoteDisconnected
  散落 k0-k4 (~15) + NVCFPexecTimeout k0×1 + empty_200 k3×1 — 全瞬态被多 key round-robin +
  func_health + buffer 吸收, 全部 resolve 为 200。
- buffer 日志 (nv_gw): 全 attempt=1 成功流 (success_tool_call, elapsed 4~9s),
...
| tier 瞬态错误 | RemoteDisconnected ~15 / NVCFPexecTimeout 1 / empty_200 1, 全被吸收 | ✅ |
| 全局 nv_requests SR | 117/120 = 97.5% (dsv4f0731_nv) | ✅ |
...
### 关键判断
cc2 主链路连续第 **84** 轮 (R893-R976) 100% SR 干净, 且主链专属错误 0 rows。
