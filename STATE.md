...
> 当前轮: **R967 (NOP 巡检轮/不改码 — cc2 主链路连续第 75 轮 100% 干净; 坏请求 3: 502 zombie_empty_completion ×2 + 502 all_tiers_exhausted ×1 全属 hermes 线 + fid 52e1ddb6, 双重归属非 cc2 范围; fallback 0 次)**
...
> 上轮: R966 (NOP, 主链 126/126=100%)

## 本轮 (R967) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 75 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (注入轮前链路分析 ≈2026-08-07 12:45 CST + live 复核 health)

- 30min cc4101-primary (主 nv_gw:40006) = **126/126 全 200 = 100% SR, 0 bad**。
...
- fallback (cc_requests 30min) = **0 次** (152 req, fb=0; 全 status=200, SR=100%)。
- nv_tier_attempts 30min (per-key): pexec_success 25/25/25/24/27 (k0-k4) +
  ...
  NVCFPexecTimeout k4×1 + empty_200 k0×1,k2×1 — 全瞬态被多 key round-robin + func_health +
...
| tier 瞬态错误 | RemoteDisconnected 19 / 529 1 / Timeout 1 / empty_200 2, 全被吸收 | ✅ |
...
- cc_requests fallback = 0 次 (152 req 全 200, SR=100%)。
- docker logs nv_gw buffer 段: 无 WAIT-/KEYMGR- 错误噪声, cc4101-primary 全成功流。
- health: 4101/40006/40066 全 200; 容器 nv_gw/cc4101 Up 9h, dsv4p_nv40066 Up 2d。

### 关键判断
cc2 主链路连续第 **75** 轮 (R893-R967) 100% SR 干净, 且主链专属错误 0 rows。
