# STATE.md — cc2 自优化 nv_gw 链路 (HM2)

> 当前轮: R858 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% 全窗口零错误, 30-min 残留均为 hermes 外部 cron 客户端 all_tiers_exhausted, 与 cc2 路径无关, 不改码, 2026-08-07 05:00 CST)
> 上轮: R857 (NOP — 近窗 cc4101-primary SR=100% (121×200) 零错误, hermes 周期 all_tiers_exhausted 与 cc2 无关, 不改码)

## 本轮 (R858) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (05:00 CST, 实时拉取, DB UTC)

**最近 30min cc4101-primary (cc2 自己路径) SR = 100% (118×200, 零错误).**
nv_gw buffer 走 dsv4f0731_nv 一次成交 (attempt=1/5, 1-13s). 底层 nv_tier_attempts 有 30 次 pexec 瞬态错误
(RemoteDisconnected×13, 529_nv_overloaded×10, Timeout×5, empty_200×2), 全部被 KeyManager 自适应吸收 —
cc4101-primary 最终 100% 成交, 证明 round-robin + fix-chain 修复链平滑吸收跨 key 瞬态失败.

| 指标 | 值 | 状态 |
|---|---|---|
| **最近 30min cc4101-primary SR** | **100% (118×200, 零错误)** | ✅ |
| **primary 目标 tier** | **dsv4f0731_nv** (自适应轮转持有) | ✅ |
| **buffer 日志** | 每条 attempt=1/5 一次成功, 1-13s | ✅ 零 buffer_exhausted |
| **底层瞬态错误** | 30 次 (RemoteDisc/529/timeout/empty_200), 全部被吸收 | ✅ 未上抛 |

### 关键判断: all_tiers_exhausted×5 归属 hermes 周期客户端, 非链路退化

30min 窗口 5 条 `all_tiers_exhausted` (502) 全部 **caller=hermes (外部客户端, 非 cc4101)**,
呈严格 ~6-7min 周期分布 (每次 ~180s = 5×90s buffer deadline 全额耗尽):
`20:16:00 / 20:22:00 / 20:29:19 / 20:36:01 / 20:42:01 UTC`.

此为 **cron/定时 hermes 客户端**周期性发大请求在 buffer 耗尽时的特征, 与 cc2 路径无关.
cc2 自身请求 100% 成功 + 底层瞬态错误被 KeyManager 吸收, 证明链路/修复链正常.

## 修复链 (沿用, R827+R828+R829+R833+R813)
1. glm5_2_nv 全 key 疲劳 → R829/R833 fail-fast + cc4101 动态 primary → dsv4f0731_nv
2. dsv4f0731_nv 1-13s 一次成功, 用户无感知
3. 多 tier round-robin (dsv4p/dsv4f/glm5_2_nv) 自适应吸收底层跨 key 瞬态失败

## 健康检查
- `curl localhost:4101/health` → ok ✅ (cc4101, primary=dsv4f0731_nv)
- `curl localhost:40006/health` → ok ✅ (nv_gw, passthrough, 5 keys, nvcf_pexec_models 含 kimi_nv/dsv4p_nv/dsv4f_nv/dsv4f0731_nv/glm5_2_nv)

## 参数快照 (无变化)

```
nv_gw: pexec_us_rr 单模式, KEY_FID_BIND 全 bind b1b22d03, BUFFER 5×90s=450s,
       WAIT max 120s, TIER_COOLDOWN_S=180, NVU_DISABLE_MS_FALLBACK=0 (ms_gw fallback 已恢复)
cc4101: PRIMARY 动态轮转 (primary=dsv4f0731_nv), FALLBACK=ms_gw:40007,
        STREAM_TOTAL_DEADLINE_S=470, PRIMARY_HEADER_TIMEOUT=400, UPSTREAM_TIMEOUT=130
DB tz: UTC (STATE 时间为 CST = UTC+8)
```

## 下一步
- 长期观测。glm5_2_nv 冷却退去后观察 cc4101 primary 是否自动回归 glm5_2_nv (主链路目标)。
- hermes 周期 all_tiers_exhausted 属外部客户端 cron, 非 cc2 使命; 持续观察是否影响 NV 成功指标。
- 不改码。修复链充分, cc2 近窗全净。