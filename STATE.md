...
> 当前轮: **R978 (NOP 巡检轮/不改码 — cc2 主链路连续第 86 轮 100% 干净; 坏请求 3: 502 all_tiers_exhausted ×2 + 502 stream_absolute_cap ×1 全属 hermes + fid 52e1ddb6, 越界非 cc2 范围; fallback 0 次)**
...
### 改动: 无 (NOP。cc2 主链路连续 86 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (注入轮前链路分析 ≈2026-08-07 13:37 CST + live 复核 bad 归属/fallback)
...
- fallback (cc_requests 30min) = **0 次** (total 105 req live, fb=0)。
...
| fallback (cc_requests) | 0 次 (105 req live 全未 fb) | ✅ |
...
- cc_requests fallback = 0 次 (live 窗口 105 req 全未 fallback_triggered)。
- docker logs nv_gw buffer 段: 全 attempt=1 成功流 (elapsed 5~10s), 本窗口无 retry,
  无 WAIT-/KEYMGR- 错误噪声。
- health: 4101/40006 全 200; 容器 nv_gw/cc4101 皆 Up (10h)。

### 关键判断
cc2 主链路连续第 **86** 轮 (R893-R978) 100% SR 干净, 且主链专属错误 0 rows。
...
本窗口连 retry 例都无 — 多 key round-robin + func_health 完全吸收瞬态键错误。
