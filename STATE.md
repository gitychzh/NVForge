...
> 当前轮: R866 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (125×200) 零错误, fallback 0% (130 请求 0 fallback), 残留 all_tiers_exhausted×4+stream_cap×1 全为 caller=hermes 外部 cron ~6min 严格周期, 不改码, 2026-08-07 ~05:2x CST)
> 上轮: R865 (NOP — 近窗 124×200 零错误, hermes 周期 all_tiers_exhausted 属外部 cron, 不改码)

## 本轮 (R866) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 125×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (~05:2x CST, 轮前链路分析注入, DB UTC)
...
| **buffer 效果** | 全 attempt1 一次成交 (4-17s ≪90s, verdict=success_tool_call/text, flushed) | ✅ 无退化 |
| **error 归属** | all_tiers_exhausted×4 + stream_absolute_cap×1 全为 caller=hermes (外部 cron, 严格 ~6min 周期) | ✅ 与 cc2 无关 |
| **per-key round-robin** | 5key 均 25 次 pexec_success (总 125), 瞬态错误被跨 key 吸收 | ✅ |
...
严格 ~6min 周期 (20:48/20:54/21:00/21:06/21:09 UTC, avg ~180033ms≈5×90s=450s buffer deadline
全额耗尽) — 属 cron 请求特征而非链路退化 (沿用 R853-R865 判定).

per-key nv_tier_attempts: 5key 均 25 次 pexec_success (共 125 成功),
...
cc2 自身路径 125×200 零错误, buffer 全 attempt1 一次成交 (4-17s),
