...
> 当前轮: R882 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (104×200) 零错误, 残留 all_tiers_exhausted×6 全为 caller=hermes 外部 cron (502×6, avg ~179.5s), fallback 0%, buffer 全 attempt1 一次成交, 不改码, 2026-08-07 ~14:12 CST)
> 上轮: R881 (NOP — 近窗 99×200 零错误, all_tiers_exhausted×5 全为 hermes 外部 cron (DB 复核仅 hermes/502/5), 不改码)

## 本轮 (R882) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 104×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (~14:12 CST, 轮前链路分析注入, DB UTC 06:12)

**最近 30min cc4101-primary (cc2 自己路径) SR = 100% (104×200, 零错误).**

| 指标 | 值 | 状态 |
|---|---|---|
| **最近 30min cc4101-primary SR** | **100% (104×200, 零错误)** | ✅ |
| **primary 目标 tier** | **dsv4f0731_nv** (自适应轮转持有, /health 确认) | ✅ |
| **30min caller×model×status** | cc4101-primary\|dsv4f0731_nv\|200\|104; hermes\|dsv4f0731_nv\|200\|1 + 502×6 | cc2 路径全净 |
| **error 归属** | all_tiers_exhausted\|all_tiers_failed_in_mapped_tier\|6\|179555 — avg ~179.5s ≈ buffer deadline 全额耗尽, 全为 caller=hermes | ✅ 与 cc2 无关 |
| **非 200 归属** | 仅 `hermes/502×6` (链路总览), cc4101-primary 0 错误 (cc2: 200\|104\|10594) | ✅ |
| **fallback 触发率** | 0 (111 请求 0 fallback) | ✅ |
| **buffer/wait** | 无 buffer/wait 日志, cc4101-primary 全 attempt 一次成交, 无 multi-attempt 退化 | ✅ |
| **per-key nv_tier_attempts** | dsv4f0731_nv 5key pexec_success 20~22/key, NVCFPexecRemoteDisconnected/Timeout + 529_nv_overloaded + empty_200 瞬态跨 key 吸收 | ✅ |
| **三容器 health** | nv_gw / cc4101 / dsv4p 均 ok, primary=dsv4f0731_nv, 5 keys | ✅ |

### 关键判断: cc2 路径全净, hermes 周期 all_tiers_exhausted 非本链路问题

30min 窗口链路总览: cc4101-primary **104×200 零错误**; 非 200 (502×6) **全部
caller=hermes** (外部客户端, 非 cc4101): all_tiers_exhausted×6, avg 179555ms ≈ buffer
deadline 全额耗尽签名 — 属 hermes 外部 cron 周期性请求, 沿用 R853-R881 判定, 而非本链路退化.

per-key tier 尝试确认 dsv4f0731_nv 5 key pexec_success 均衡 (20~22/key), 底层
NVCFPexecRemoteDisconnected/Timeout + 529_nv_overloaded + empty_200 等瞬态被跨 key round-robin
吸收, KeyManager 无持续冷却键堆积. cc2 自身 104×200 零错误, fallback 0%, 无 buffer/wait 退化,
链路/KeyManager 健康. 不改码.
...
- docker ps: nv_gw = Up 3h, cc4101 = Up 2h, dsv4p_nv40066 = Up 2d (nv_gw_stable = Up 5d 对照)

## 参数快照 (无变化, R882, 与注入配置一致)
...
nv_gw(40006): pexec_us_rr 单模式, KEY_FID_BIND 全 bind b1b22d03 (0:0;1:0;2:0;3:0;4:0), BUFFER 5×90s=450s (STAIRS 90,90,90,90,90, RETRIES=5),
              WAIT max 120s, TIER_COOLDOWN_S=180, TIER_TIMEOUT_BUDGET_S=180, FORCE_STREAM_UPGRADE=0/NVU_FORCE_STREAM_UPGRADE_TIMEOUT=150,
              KEY_COOLDOWN_S=30, NV_INTEGRATE_KEY_COOLDOWN_S=90, UPSTREAM_TIMEOUT=90,
              MIN_OUTBOUND_INTERVAL_S=10, NVU_DISABLE_MS_FALLBACK=0, NVU_PEER_FB_SKIP_MODELS=glm5_2_nv,dsv4p_nv,
