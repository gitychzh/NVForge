...
# ─── R40: 5 LiteLLM glm5.1 容器 URL 列表 ───────────────────────────────────
# K1(41101): 直连（无代理）, K2(41102): 直连（无代理）
# K3(41103): SOCKS5 via port 7896, K4(41104): SOCKS5 via port 7897, K5(41105): SOCKS5 via port 7899
# 容器内 HTTP_PROXY 设置在 compose 中，hm40006 通过 HTTP 调用容器
LITELLM_GLM51_URLS = []
for i in range(1, 6):
    url = os.environ.get(f"LITELLM_GLM51_URL{i}", "")
    if url:
        LITELLM_GLM51_URLS.append(url)
LITELLM_GLM51_COUNT = len(LITELLM_GLM51_URLS)
if LITELLM_GLM51_COUNT < 5:
    print(f"[HM-CONFIG] WARN: only {LITELLM_GLM51_COUNT} LiteLLM glm5.1 URLs configured (expected 5)",
          file=sys.stderr, flush=True)
else:
    print(f"[HM-CONFIG] R40: {LITELLM_GLM51_COUNT} LiteLLM glm5.1 URLs ready (K1-K2 direct, K3-K5 SOCKS5 by container)",
          file=sys.stderr, flush=True)

# ─── R40: LiteLLM glm5.1 HTTP 专用轮询计数器 ──────────────────────────────
_HTTP_RR_COUNTER_FILE = os.path.join(LOG_DIR, "rr_litellm_glm51_counter.json")
_http_rr_counter = {}
_http_rr_lock = threading.Lock()

def _load_http_rr() -> None:
    "Restore LiteLLM glm5.1 HTTP counter from disk."
    try:
        with open(_HTTP_RR_COUNTER_FILE, "r") as f:
            raw = f.read().strip()
        if not raw:
            return
        saved = json.loads(raw)
        if isinstance(saved, dict) and "llm_glm51" in saved:
            v = saved.get("llm_glm51", 0)
            if isinstance(v, int) and v >= 0:
                _http_rr_counter["llm_glm51"] = v
                print(f"[HM-HTTP-RR] restored from {_HTTP_RR_COUNTER_FILE}: v={v}", file=sys.stderr, flush=True)
    except FileNotFoundError:
        pass
    except Exception as e:
        print(f"[HM-HTTP-RR] WARN: {e}", file=sys.stderr, flush=True)

def _save_http_rr() -> None:
    "Persist HTTP counter atomically."
    try:
        tmp = f"{_HTTP_RR_COUNTER_FILE}.tmp.{os.getpid()}.{threading.get_ident()}"
        with open(tmp, "w") as f:
            json.dump(_http_rr_counter, f)
        os.replace(tmp, _HTTP_RR_COUNTER_FILE)
    except Exception as e:
        print(f"[HM-HTTP-RR] WARN: {e}", file=sys.stderr, flush=True)
...
def _next_litellm_glm51() -> int:
    "Per-K sequential round-robin for HTTP containers (0..LITELLM_GLM51_COUNT-1)."
    with _http_rr_lock:
        counter = _http_rr_counter.get("llm_glm51", 0)
        key_idx = counter % LITELLM_GLM51_COUNT
        _http_rr_counter["llm_glm51"] = counter + 1
        _save_http_rr()
        return key_idx
...
# AB-test: glm5.1 primary tier (matches HM2 config for comparison).
# Rule #8: glm5.1_hm_nv must ALWAYS be the primary tier.
...
DEFAULT_NV_MODEL = "glm5.1_hm_nv"  # AB-test: glm5.1 primary
