...
def _try_tier_http(oai_body, tier_model, request_id, metrics, t_start,
                   is_stream, prior_cycle_attempts):
    """R40: glm5.1 走 HTTP 容器路径 — 顺序调用 5 个 LiteLLM 容器.

    K1/K2 直连(无代理), K3/K4/K5 走 SOCKS5 出口(由容器内部处理).
    Deepseek/kimi 仍走 NVCF pexec, 不受影响.
    """
    from .config import LITELLM_GLM51_URLS, LITELLM_GLM51_COUNT, _next_litellm_glm51

    result = UpstreamResult()
    result.is_stream = is_stream
    result.tier_model = tier_model
    result.upstream_type = "litellm_glm51_http"
    key_cycle_attempts = list(prior_cycle_attempts)

    if LITELLM_GLM51_COUNT == 0:
        _log("HM-LITELLM-ERR", "No LiteLLM glm5.1 URLs configured — skipping HTTP tier")
        result.all_keys_exhausted = True
        return result

    _log("HM-HTTP-TIER", f"Starting glm5.1 HTTP tier: {LITELLM_GLM51_COUNT} containers "
                         f"(K1-K2 direct, K3-K5 SOCKS5)")

    start_key_idx = _next_litellm_glm51()
    tier_budget_start = time.time()

    for attempt_idx in range(LITELLM_GLM51_COUNT + 2):
        key_idx = (start_key_idx + attempt_idx) % LITELLM_GLM51_COUNT
        ...
        if elapsed_in_tier >= TIER_TIMEOUT_BUDGET_S:
            _log("HM-HTTP-BUDGET", f"HTTP tier budget {TIER_TIMEOUT_BUDGET_S}s "
        ...
        per_attempt_timeout = max(MIN_ATTEMPT_TIMEOUT,
                                  min(UPSTREAM_TIMEOUT, remaining_budget))

        container_url = LITELLM_GLM51_URLS[key_idx]
        if not container_url:
            key_cycle_attempts.append({
                "tier": tier_model,
                "key_idx": key_idx,
                "error_type": "no_container_url",
                "upstream_type": "litellm_glm51_http",
            })
            continue

        _log("HM-HTTP-KEY", f"glm5.1 k{key_idx+1} → {container_url} "
                             f"(attempt {attempt_idx+1}/{LITELLM_GLM51_COUNT + 2})")

        try:
            # Throttle before outbound
            if attempt_idx == 0:
                throttle_outbound()
            ...
            # HTTP POST to container (it forwards to NV API)
            body = json.dumps(oai_body).encode("utf-8")
            parsed = urllib.parse.urlparse(container_url)
            host = parsed.hostname
            port = parsed.port or 41101

            conn = http.client.HTTPConnection(host, port, timeout=per_attempt_timeout)
            ...
            if resp.status >= 400:
                ...
                try:
                    error_json = json.loads(error_body.decode("utf-8", errors="replace"))
                except Exception:
                    error_json = {"error": error_body.decode("utf-8", errors="replace")[:500]}
                conn.close()
                key_cycle_attempts.append({
                    "tier": tier_model,
                    "key_idx": key_idx,
                    "error_type": f"http_{resp.status}",
                    "error_body": str(error_json)[:500],
                    "upstream_type": "litellm_glm51_http",
                    "container_url": container_url,
                })
                _log("HM-HTTP-ERR", f"glm5.1 k{key_idx+1} → {resp.status} "
                                     f"from {container_url}")

                should_cycle = resp.status in (429, 500, 502, 503)
                if should_cycle:
                    _log("HM-HTTP-CYCLE", f"glm5.1 k{key_idx+1} {resp.status} — "
                                          f"cycling to next container")
                    continue
            ...
            if not resp_body or len(resp_body) == 0:
                ...
                try:
                    ...
                    pass
                _log("HM-HTTP-EMPTY", f"glm5.1 k{key_idx+1} empty 200 from {container_url}")
                continue

            # ─── Valid 200 success ───
            result.success = True
            result.nv_key_idx = key_idx
            result.nv_model_label = f"litellm_{container_url.split(':')[-1].split('/')[0]}_k{key_idx+1}"
            result.key_cycle_attempts = key_cycle_attempts
            ...
            if key_cycle_attempts:
                ...
                _log("HM-HTTP-SUCCESS", f"glm5.1 k{key_idx+1} succeeded via {container_url} "
                                        f"after {len(key_cycle_attempts)} cycle attempts")
            else:
                _log("HM-HTTP-SUCCESS", f"glm5.1 k{key_idx+1} succeeded on first attempt via {container_url}")
            ...
            _log("HM-HTTP-TIMEOUT", f"glm5.1 k{key_idx+1} timeout from {container_url}: "
                                    f"{attempt_elapsed_ms}ms")
            key_cycle_attempts.append({
                "tier": tier_model,
                "key_idx": key_idx,
            ...
            _log("HM-HTTP-CONN", f"glm5.1 k{key_idx+1} connection error to {container_url}: {e}")
            key_cycle_attempts.append({
                "tier": tier_model,
                "key_idx": key_idx,
                "error_type": f"http_conn_{type(e).__name__}",
                "upstream_type": "litellm_glm51_http",
                "container_url": container_url,
            })
            continue
            ...
            _log("HM-HTTP-ERR", f"glm5.1 k{key_idx+1} {error_class} from {container_url}: {e}")
            key_cycle_attempts.append({
                "tier": tier_model,
                "key_idx": key_idx,
                "error_type": f"http_{error_class}",
                "error": str(e)[:200],
                "upstream_type": "litellm_glm51_http",
                "container_url": container_url,
            })
    ...
    result.all_keys_exhausted = True
    result.all_429 = all(a.get("error_type", "").startswith("http_429")
                         for a in key_cycle_attempts
                         if a.get("upstream_type") == "litellm_glm51_http")
    result.empty_200 = all(a.get("error_type") == "empty_200"
                           for a in key_cycle_attempts
                           if a.get("upstream_type") == "litellm_glm51_http")
    result.key_cycle_attempts = key_cycle_attempts
    result.elapsed_ms = int((time.time() - t_start) * 1000)

    fail_summary = (f"http_errors={sum(1 for a in key_cycle_attempts if a.get('upstream_type')=='litellm_glm51_http')}, "
                   f"empty200={sum(1 for a in key_cycle_attempts if a.get('error_type')=='empty_200')}, "
                   f"timeout={sum(1 for a in key_cycle_attempts if a.get('error_type')=='http_timeout')}")
    _log("HM-HTTP-FAIL", f"glm5.1 all {LITELLM_GLM51_COUNT} containers failed: {fail_summary}, "
...
def _try_tier_keys(oai_body, tier_model, request_id, metrics, t_start,
    ...
    # ─── R40: glm5.1 走 HTTP 容器路径（不走 NVCF pexec）───
    if tier_model == "glm5.1_hm_nv":
        return _try_tier_http(oai_body, tier_model, request_id, metrics, t_start,
                              is_stream, prior_cycle_attempts)
