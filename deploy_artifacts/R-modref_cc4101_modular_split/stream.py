...
def passthrough_stream(handler, resp, conn, metrics, t_start):
    ...
    try:
        ...
        # R-bugfix-J: HTTP/1.1 SSE 流必须用 Transfer-Encoding: chunked, 否则
        # zcode SDK 无法界定响应体边界 (无 Content-Length, 无 chunked = 非法).
        # HM1 ms-gateway 用 chunked 正常工作, cc4101 之前缺少 → "模型未返回任何内容".
    ...
    def _write_bytes(b):
        ...
        if not b:
            return True
        try:
            # R-bugfix-J: chunked transfer encoding — 每个数据块用 hex_size\r\n data \r\n 包装.
            chunk_size = len(b)
            handler.wfile.write(f"{chunk_size:x}\r\n".encode("ascii"))
            handler.wfile.write(b)
            handler.wfile.write(b"\r\n")
            handler.wfile.flush()
            _bytes_written += chunk_size
            return True
        except (BrokenPipeError, ConnectionResetError, OSError) as e:
            _disc_exc = e
    ...
    # R-bugfix-E: 取底层 socket, 设 timeout=None (无限阻塞), 用 select 检查可读后再 read.
    # 旧方案设 socket timeout=30s, resp.read 30s 超时后 http.client fp 进入 "timed out"
    # 状态, 后续 resp.read 永远抛 "cannot read from timed out object" -> 死循环, 收不到
    # nv_gw flush 的数据 -> 返回空 200 (模型未返回任何内容).
    try:
        _raw_sock = resp.fp.raw._sock
        _raw_sock.settimeout(None)  # 无限阻塞, 由 select + deadline 控制
    except Exception:
        _raw_sock = None
    try:
        while True:
            ...
            # R-bugfix-E: ttfb 前也要有兜底, 防 nv_gw buffer 重试期间无限等待.
            if not ttfb_recorded and stream_total_deadline is None:
                stream_total_deadline = time.time() + CC4101_STREAM_TOTAL_DEADLINE_S
            if not ttfb_recorded and time.time() > stream_total_deadline:
                metrics["error_type"] = "ttfb_total_deadline"
                _log("STREAM-DEADLINE", f"passthrough ttfb deadline "
                    f"{CC4101_STREAM_TOTAL_DEADLINE_S}s exceeded (nv_gw buffer retrying too long)")
                raise socket.timeout("ttfb_total_deadline")
            # R-bugfix-E: 用 select 等 socket 可读 (5s 一次检查 deadline), 不让 resp.read
            # 无限阻塞也不让 http.client fp 进入 timed-out 状态.
            if _raw_sock is not None:
                try:
                    _rfds, _, _ = select.select([_raw_sock], [], [], 5.0)
                except Exception:
                    _rfds = []
                if not _rfds:
                    # 5s 内无数据, 回循环顶部检查 deadline, 然后继续等.
                    continue
                # socket 可读: 有数据到达, 或对端 FIN (EOF). resp.read 不会阻塞.
            try:
            ...
            if not chunk:
                ...
                break
            # R-bugfix-G: 过滤 nv_gw buffer 发的 event: ping, 不透传给 zcode.
            # zcode SDK 收到 message_start 之前的 event: ping 会报错 (Provider returned a server error).
            # ping 是 nv_gw buffer 重试时防止 cc4101 超时的占位符, cc4101 消费即可, 不转发.
            if b"event: ping" in chunk:
                chunk = chunk.replace(b"event: ping\ndata: {}\n\n", b"")
                if not chunk:
                    continue  # 整个 chunk 都是 ping, 跳过
            ...
            if not _write_bytes(chunk):
                ...
                return
        # 正常结束
        # R-bugfix-J: chunked encoding 终止标记
        try:
