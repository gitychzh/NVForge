...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"  # R-http11: BaseHTTP 默认 1.0+Connection:close, zcode 等客户端需 1.1+keep-alive
    ...
    def _handle_messages(self):
        ...
        # R2417: remove R-cc_s3 stair logic (fixed-key binding removed in R2416).
        # Stair override forced 60s on ALL requests, overriding R2154 input-size
        # header_timeout (150-180s for 50K+ input).  Every large request failed on
        # stair#1.  Plus _try_primary never set result.error_kind on timeout, so
        # stair loop broke immediately ("non-timeout error_kind=" -> stop stair).
        # Restore direct execute_request call — R2154 input-size header_timeout
        # and nv_gw buffer layer handle retries properly.
    ...
    def _send_raw(self, code, body_bytes, content_type="application/json", extra_headers=None):
        ...
        # R-bugfix1: HTTP/1.1 keep-alive — 不再强制 Connection: close + close_connection.
        # 旧: 强制 close 导致 zcode SDK 连接池每次都断, 间歇性读空响应
        # "模型未返回任何内容". HTTP/1.1 + Content-Length 足以界定响应边界,
        # 客户端连接池可复用连接.
        if extra_headers:
