...
def execute_request(oai_body, request_id, metrics, t_start, header_timeout_override=None):
    ...
    def _try_primary(stage_label):
        ...
        try:
            ...
            # R cc_s3: stepped-timeout override (来自 handlers 重试循环). 非 None 时
            # 强制用此值覆盖 R2154 分档 _hdr_to, 用于阶梯超时探测 (60->120->240s).
            if header_timeout_override is not None:
                _hdr_to = header_timeout_override
    ...
    def _try_fallback(stage_label):
        ...
        try:
            ...
            # R cc_s3: stepped-timeout override (同 primary, 用于阶梯重试).
