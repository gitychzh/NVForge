...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_messages(self):
        ...
        # R cc_s3: 阶梯超时重试循环. 3 次 attempt, header_timeout 60->120->240s,
        # 每次间隔 RETRY_INTERVAL_S=2s sleep. 仅当上游 timeout 失败时重试 (非超时
        # 错误如 client_4xx/server_5xx 不重试, 直接返回原错误). 3 次都 timeout
        # -> 504 'error:time out3' 告诉 CC.
        ...
        for _stair_attempt, _hdr_to in enumerate(TIMEOUT_STAIRS, 1):
            ...
            if result.success:
                break
            # 仅 timeout 连续才走阶梯. 其他错误 (client_4xx/server_5xx/conn) 直接返回.
            _ek = result.error_kind or ""
            if _ek != "timeout":
                _log("REQ-STAIR", f"req={request_id} stair#{_stair_attempt} hdr_to={_hdr_to}s "
                                   f"non-timeout error_kind={_ek} -> stop stair, return original error")
                break
            _log("REQ-STAIR", f"req={request_id} stair#{_stair_attempt} hdr_to={_hdr_to}s "
                               f"timeout (elapsed={result.elapsed_ms}ms), "
                               f"{'sleep ' + str(RETRY_INTERVAL_S) + 's -> next stair' if _stair_attempt < len(TIMEOUT_STAIRS) else 'all stairs exhausted -> error:time out3'}")
            if _stair_attempt < len(TIMEOUT_STAIRS):
                time.sleep(RETRY_INTERVAL_S)
        else:
            # for-else: 3 次都 timeout 未 break (success) -> 强制 error:time out3
            _log("REQ-STAIR", f"req={request_id} 3 consecutive timeouts -> 504 error:time out3")
            err_payload = {"type": "error", "error": {
                "type": "timeout",
                "message": "error:time out3",
                "model": request_model}}
            metrics["status"] = 504
            metrics["error_type"] = "stepped_timeout_3"
            metrics["error_message"] = "error:time out3"
            metrics["stair_attempts"] = _stair_attempt
            metrics["duration_ms"] = int((time.time() - t_start) * 1000)
            _log_metrics(metrics); enqueue_metrics(metrics)
            self._send_json(504, err_payload, extra_headers={"retry-after": "5"})
            return
