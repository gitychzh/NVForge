...
from .upstream import execute_request, UpstreamResult, _ms_fallback_request, _peek_retry_next_key, _try_glm52_mode_chain
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_messages_anthropic(self):
        ...
        # R-buf2key: for buffer callers, call NVCF directly (no ms_gw fallback).
        # BufferStreamSession will handle key2→key5 retry + ms_gw fallback internally.
        _buf_caller = metrics.get("caller", "")
        if _buf_caller in NVU_BUFFER_CALLERS and is_stream:
            from .buffer_stream import BufferStreamSession
            from .format.oai_to_anth import OaiSseToAnthropicConverter
            _log("NV-BUF2KEY-INTERCEPT", f"({request_model}) caller={_buf_caller} stream=True, "
                                         f"buffer handles NVCF directly (req={request_id})")
            # Send 200 + SSE headers, then let buffer run
            self.send_response(200)
            ...
            self.end_headers()
            _buf_converter = OaiSseToAnthropicConverter(
                request_model, request_id=request_id)
            _buf_session = BufferStreamSession(
                handler=self, resp=None, conn=None, oai_body=oai_body,
                metrics=metrics, t_start=t_start, request_model=request_model,
                converter=_buf_converter)
            _buf_session.run()
            return
    ...
    def _stream_openai_to_anth(self, resp, conn, metrics, t_start, request_model, oai_body=None):
        ...
        """
        # ── R-buffer (2026-07-27): cc2 buffer-then-flush 模式 ──
        # 对 cc4101-primary caller, 不立即转发流, buffer 到判定成功后一次性 flush.
        # 每 30s 发 event:ping 重置 SDK idle timer. 失败同 key 重试 3 次. 总预算 580s.
        # 回滚: NVU_BUFFER_CALLERS="" (env, 无需改代码) 或回退 handlers.py.bak.R-buffer.
