...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _relay_stream(self, result, metrics, t_start, request_id, agent_model):
        ...
        try:
            ...
            # R813b ms_gw [DONE] close: ModelScope upstream often keeps connection open
            # after data: [DONE], causing resp.read(8192) to block until UPSTREAM_TIMEOUT
            # (300s). Break after forwarding [DONE] so downstream consumers
            # (agent collect_stream) can finish immediately.
            done_seen = False
            while not done_seen:
                try:
