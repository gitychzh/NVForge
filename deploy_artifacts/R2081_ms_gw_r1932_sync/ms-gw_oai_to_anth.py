...
class OaiSseToAnthropicConverter:
    ...
    def finish(self, interrupted=False, zombie=False, input_tokens_real=0,
               flushed_content_chars=0):
        """Emit terminal events (content_block_stop, message_delta, message_stop).

        caller contract (R1932+R1774 synced from nv_gw, see [[r1932-ms-gw-missing-parsecall-fix]]):
          zombie=True        -> 若已有真内容 flush 给 CC (flushed_content_chars>0 或
                              self.content_chars>0 或 message_start_sent), 不发 event: error
                              (CC SDK 当致命错中断 session), 改发 graceful
                              message_delta(stop_reason=tool_use/end_turn)+message_stop,
                              让 CC 把已收内容当完整响应收尾不中断。零内容仍发 api_error 让 CC 重试。
        ...
        R1932 bug8 root-cause fix (2026-07-20 cc2 session d87568cd interruption):
        backend (ZHIPUAI/glm-5.2 etc) declares finish_reason=tool_calls but sends no
        real tool_call delta with id+args (saw_real_tool_call=False). pending_stop_reason
        was set to "tool_use" by the finish_reason chunk but CC SDK sees no tool_use block
        -> parse failure "no visible output" -> session terminated. Fix: declared tool_use
        but no real block emitted -> force end_turn, so CC SDK skips tool_use parse path.
        Mirrors nv_gw finish() same logic. Normal tool_call (id+args complete)
        saw_real_tool_call=True is unaffected.
        ...
        _has_flushed_content = (
            flushed_content_chars > 0
            or getattr(self, "content_chars", 0) > 0
            or getattr(self, "reasoning_chars", 0) > 0
        )

        if zombie:
            if _has_flushed_content or self.message_start_sent:
