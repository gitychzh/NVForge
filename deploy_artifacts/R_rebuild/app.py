...
def create_and_start_server():
    ...
    # R-rebuild Phase 3: 启动后台 ProbeWorker
    try:
        from gateway.probe_worker import start_probe_worker
        start_probe_worker()
    except Exception as e:
        print(f"[NV-PROXY] ProbeWorker start failed: {e}", file=sys.stderr, flush=True)
