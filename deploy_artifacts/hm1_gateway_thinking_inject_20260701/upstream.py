#!/usr/bin/env python3
"""Upstream request executor for NV proxy (hm40006) — 三 agent 通用.
...
from .config import (
    ...
    UPSTREAM_TIMEOUT, TIER_TIMEOUT_BUDGET_S, HM_FORCE_STREAM_UPGRADE_TIMEOUT,
    _next_nv_key,
    throttle_outbound,
    ...
    for attempt_idx in range(HM_NUM_KEYS + 2):
        ...
        try:
            ...
            if consecutive_pexec_timeout >= PEXEC_TIMEOUT_FASTBREAK:
                _log("HM-PEXEC-FASTBREAK", f"tier={tier_model} {consecutive_pexec_timeout} consecutive "
            ...
            if is_ssl_err:
                _log("HM-SSL-CYCLE", f"tier={tier_model} k{key_idx+1} SSL error ({elapsed_ms}ms) — "
...
def execute_request(handler, oai_body, mapped_model, request_id, metrics, t_start, upstream_timeout_override=None):
    ...
    # 3model (2026-07-01): tier_order=[mapped_model] 单元素, 删跨 tier ring fallback.
    # 各 agent 各后端语义 (hermes→kimi / opencode→dsv4p / openclaw→glm5.1), 不允许跨 model fallback
    # (否则 opencode 的 deepseek 悄悄变 glm5.1 违背"各对应各"). 循环体不变, 只跑目标 tier 单次.
    # 全 key 用尽 → all_keys_exhausted 直接报错, 靠 nvcf-func-monitor(10min)+人工换 id 兜底下架.
    tier_order = [mapped_model]

    _log("HM-REQ", f"mapped_model={mapped_model} start_tier={mapped_model} "
